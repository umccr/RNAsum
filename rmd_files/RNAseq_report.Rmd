...
pca <- function(data, targets, plot_mode = "static" ) {
  ...
  pca_plot <- plot_ly(data.subset_pca.df, x = ~PC1, y = ~PC2, color = ~Target, text=paste(target$Target, rownames(data.subset_pca.df), sep=": "), colors = targets.colour[[1]], type='scatter', mode = "markers", marker = list(size=10, opacity = 0.7), width = 600, height = 400) %>%
...
colnames(data.subset_scree.df) <- c("PC", "Variances")
  ...
  scree_plot <- plot_ly(data.subset_scree.df, x = ~PC, y = ~Variances, type = 'bar', width = 600, height = 400) %>%
...
if ( file.exists(paste(params$batch, "pcgr", paste0(batch, "-somatic.pcgr_acmg.grch37.snvs_indels.tiers.tsv"), sep = "/")) ) {
  
pcgr_file <- paste(params$batch, "pcgr", paste0(batch, "-somatic.pcgr_acmg.grch37.snvs_indels.tiers.tsv"), sep = "/")

} else if ( file.exists(paste(params$batch, "pcgr", paste0(batch, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")) ) {
  
  pcgr_file <- paste(params$batch, "pcgr", paste0(batch, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")
}
...
sv_file_1 <- paste(params$batch, "structural", paste0(batch, "-sv-prioritize-manta-pass.tsv"), sep = "/")
sv_file_2 <- paste(params$batch, "structural", paste0(batch, "-manta.tsv"), sep = "/")
runSVsChunk <- TRUE
...
sampleID.col <- grep(params$sample_id, ref_datasets.list[[tissue]][["clinical_info"]])

if ( length(sampleID.col) > 0 ) {
  
  ##### Identify column and row with patients details
  sampleID.row <- grep(params$sample_id, ref_datasets.list[[tissue]][["clinical_info"]][, sampleID.col])
...
mRNA expression levels of genes containing single nucleotide variants (SNVs) or insertions/deletions (indels), obtained from the [PCGR](https://github.com/sigven/pcgr){target="_blank"} report, in patient's sample and their average mRNA expression in samples from cancer cohorts. NOTE, only PCGR [tier 1-3](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} variants are reported.
