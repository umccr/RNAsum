...
targets.colours <- c("darkseagreen1", "red", "gray50")
...
glanceExprPlot <- function(genes, data, targets, sampleName, normal, cancer, plot_mode = "static", hexcode) {
  ...
  ##### Reorder groups
  gene.expr.df$Group <- factor(gene.expr.df$Group, levels=c("Patient", cancer, normal))
    
  p <- plot_ly( gene.expr.df, x = ~Gene, y = ~Expression, color = ~Group, type = "box", colors = c(I("black"), "red", "green"), opacity=0.2, showlegend = TRUE ) %>% 
...
if ( params$filter ) {
  ...
  legend("topright", legend=c(normal_group, cancer_group, "Patient"), fill=targets.colour[[1]], bty="n", bg = "transparent")
...
legend("topright", legend=c(normal_group, cancer_group, "Patient"), fill=targets.colour[[1]], horiz=TRUE, bg = "transparent", box.col="transparent")
...
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, plot_mode = params$plots_mode, hexcode = "immune_genes")
...
##### Generate overview boxplot
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, plot_mode = params$plots_mode, hexcode = "hrd_genes")
