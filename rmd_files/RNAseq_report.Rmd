...
output:
  ...
  rmdformats::material:
    highlight: kate
params:
  sample_name: 'MDX230101'
  dataset: 'PANCAN'
  bcbio_rnaseq: NULL
  dragen_rnaseq: '/Users/kanwals/UMCCR/data/wts/RNAsum/final/SBJ02805_PRJ222043_L2201319/dragen'
  arriba_rnaseq: '/Users/kanwals/UMCCR/data/wts/RNAsum/final/SBJ02805_PRJ222043_L2201319/dragen/arriba'
  report_dir: '/Users/kanwals/UMCCR/data/wts/RNAsum/reports'
  ref_data_dir: '../data'
  transform: 'CPM'
  filter: TRUE
  norm: 'TMM'
  ...
  drugs: FALSE
  immunogram: FALSE
  umccrise: '/Users/kanwals/UMCCR/data/wts/RNAsum/umccrised/SBJ03162__MDX230100'
...
exprTable <- function(genes, keep_all = FALSE, data, cn_data = NULL, sv_data = NULL, cn_decrease = TRUE, targets, sampleName, int_cancer, ext_cancer, comp_cancer, add_cancer = NULL, genes_annot = NULL, oncokb_annot = NULL, cancer_genes = NULL, mut_annot = NULL, fusion_genes = NULL, ext_links = FALSE, type = "z", scaling = "gene-wise") {
  ...
  ##### Attach links to GeneCards and Ensembl (if provided). Here we assume that gene names are unique
  for ( gene in genes ) {
    if ( "ENSEMBL" %in% names(group.z) ) {
      check1 <- unique(group.z$ENSEMBL[group.z$Gene == gene])
      if (length(check1) > 0) {
        # if ( !is.na(unique(group.z$ENSEMBL[group.z$Gene == gene ]) )) {
...
##### Read in gene fusion data for sample under investigation
...
if ( !is.null(params$dragen_rnaseq) && file.exists(dragen_fusion_file) ) {
  ...
  ##### Check if Dragen detected any fusions
  if (nrow(dragen_fusion) > 0) {
