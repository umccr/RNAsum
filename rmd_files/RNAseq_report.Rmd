...
params:
  ...
  tissue:
  ref_data_dir: '../data'
  genes_pmcc: 'Genes_PMCC.txt'
  genes_immune: 'Genes_immune.txt'
...
ref_tissue <- list( "pancreas" = c(paste(params$ref_data_dir, "Datasets_list_pancreas.txt", sep="/"), "Normal (pancreas)", "PDAC"), "cervix" = c(paste(params$ref_data_dir, "Datasets_list_cervix.txt", sep="/"), "Normal (cervix uteri)", "Cervical squamous cell carcinoma") )
...
ref_tissues <- c(tissue)
ref_tissues.list <- vector("list", length(ref_tissues))
names(ref_tissues.list) <- ref_tissues
...
ref_tissues.list[[tissue]] <- combineDatasets(params$sample_name, params$count_file, ref_tissue[[tissue]][1])

##### Read in selected genes list
ref_genes.list[["genes_pmcc"]] <- read.table(paste(params$ref_data_dir, params$genes_pmcc, sep="/"), sep="\t", as.is=TRUE, header=FALSE, row.names=NULL, quote="")
...
for ( ref in names(ref_tissues.list) ) {
  
  counts <- ref_tissues.list[[ref]][[1]]
  target <- ref_tissues.list[[ref]][[2]]
  ...
  ref_tissues.list[[ref]][[3]] <- y.filtered
}
...
for ( ref in names(ref_tissues.list) ) {
  ...
  y.filtered <- ref_tissues.list[[ref]][[3]]
  ...
  ref_tissues.list[[ref]][[3]] <- y.filtered.norm.cpm
}
```
...
for ( ref in names(ref_tissues.list) ) {
  ...
  target <- ref_tissues.list[[ref]][[2]]
  data <- ref_tissues.list[[ref]][[3]]
  
  ref_tissues.list[[ref]][[4]] <- pca(data, target)
}
```
...
for ( ref in names(ref_tissues.list) ) {
  ...
  ref_tissues.list[[ref]][[3]] <- apply(data.annot[, colnames(data)], 2, as.numeric)
  rownames(ref_tissues.list[[ref]][[3]]) <- data.annot$SYMBOL
  ref_tissues.list[[ref]][[5]] <- data.annot[, c("SYMBOL", "GENEBIOTYPE", "ENSEMBL", "ENTREZID", "SEQNAME", "GENESEQSTART", "GENESEQEND")]
}
```
...
fusion_genes_annot <- ref_tissues.list[[ref]][[5]][ , c("SYMBOL", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND") ]
...
From the `r length(unique(ref_genes.list[["pcgr"]]$SYMBOL))` mutated genes the expression of **`r length(which(unique(ref_genes.list[["pcgr"]]$SYMBOL) %in% rownames(ref_tissues.list[[ref]][[3]])))`** was reliably measured in patient's sample. NOTE, all variants annotated by [PCGR](https://github.com/sigven/pcgr){target="_blank"} are reported, including non-coding variants.
...
mut_genes.expr <- exprTable(unique(ref_genes.list[["pcgr"]]$SYMBOL), data, target, params$sample_name, normal_group , cancer_group, ref_tissues.list[[ref]][[5]][, c(1,3)], ref_genes.list[["oncokb_genes"]])
...
From the `r nrow(ref_genes.list[["oncokb_genes"]])` cancer genes the expression of **`r length(which(rownames(ref_genes.list[["oncokb_genes"]]) %in% rownames(ref_tissues.list[[ref]][[3]])))`** was reliably measured in patient's sample.
...
cancer_genes.expr <- exprTable(rownames(ref_genes.list[["oncokb_genes"]]), data, target, params$sample_name, normal_group , cancer_group, ref_tissues.list[[ref]][[5]][, c(1,3)], ref_genes.list[["oncokb_genes"]])
...
##### Generate bezier curves-like plot representing gene fusion events
fusionsBezierPlot(fusion_annot_top, ref_tissues.list[[ref]][[5]])
```
...
fusion.df <- ref_tissues.list[[ref]][[5]]
fusion.df <- rbind(fusion.df[fusion.df$SYMBOL==geneA, ], fusion.df[fusion.df$SYMBOL==geneB, ])
exprTable(fusion.df$SYMBOL, data, target, params$sample_name, normal_group, cancer_group, ref_tissues.list[[ref]][[5]][, c(1,3)], ref_genes.list[["oncokb_genes"]][,c(1,2,4)])[[1]]
```
...
fuiosnExprProfile <- exprProfilePlot(c(as.vector(geneA), as.vector(geneB)), data, target, params$sample_name, normal_group, cancer_group, ref_tissues.list[[ref]][[5]])
...
immune_genes.expr <- exprTable(unlist(ref_genes.list[["genes_immune"]]), data, target, params$sample_name, normal_group , cancer_group, ref_tissues.list[[ref]][[5]][, c(1,3)])
