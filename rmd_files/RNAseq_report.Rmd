...
pizzly.fusions <- read.table(file = paste(dataDir, "pizzly", paste0(sampleName.orig, "-flat-filtered.tsv"), sep = "/"), header = TRUE)
...
mRNA expression levels of mutated genes, obtained from the [PCGR](https://github.com/sigven/pcgr){target="_blank"} report, in patient's sample and their average mRNA expression in samples from cancer patients and healthy individuals.

### Expression summary

From the `r length(unique(ref_genes.list[["pcgr"]]$SYMBOL))` mutated genes the expression of **`r length(which(unique(ref_genes.list[["pcgr"]]$SYMBOL) %in% rownames(ref_datasets.list[[ref]][[3]])))`** was reliably measured in patient's sample. NOTE, all variants annotated by [PCGR](https://github.com/sigven/pcgr){target="_blank"} are reported, including non-coding variants.
...
##### Report only mutations calssified within Tiers 1-4
#ref_genes.list[["pcgr"]] <- ref_genes.list[["pcgr"]][ ref_genes.list[["pcgr"]]$TIER %in% c("TIER 1", "TIER 2", "TIER 3", "TIER 4" ), ]

mut_genes.expr <- exprTable(unique(ref_genes.list[["pcgr"]]$SYMBOL), data, target, params$sample_name, normal_group , cancer_group, ref_datasets.list[[tissue]][[5]][, c(1,3)], ref_genes.list[["oncokb_genes"]])
...
```{r cdf_plot_mut_genes_8, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
...
```{r cdf_plot_mut_genes_9, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
...
```{r cdf_plot_mut_genes_10, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
...
<span style="color:#ff0000">NOTE</span>: this section is in progress. The scatterplot presents per-gene mRNA expression values (y-axis, Z-score), copy-number values (x-axis, from PRUPLE), and mutation status information (colours, from PCGR). To do:

- Limit genes to those with detected variants classified within tier 1-3
- Add summary table
