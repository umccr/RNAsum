...
if ( runPurpleChunk ) {
  ...
  ##### Keep only altered genes with CN values below loss threshold (default 2nd percentile) and above gain threshold (default 98th percentile)
  if ( params$cn_loss == 2 && params$cn_gain == 98 ) {
    
    cn_data.all.percent <- quantile(ref_genes.list[["summary"]]$CN$MeanCopyNumber, probs = seq(0, 1, .02), na.rm = TRUE)
    
    cn_bottom <- round(cn_data.all.percent[2], digits = 0)
    cn_top <- round(cn_data.all.percent[50], digits = 0)
...
cn_data.all.percent <- quantile(cn_data.all$MeanCopyNumber, probs = seq(0, 1, .02), na.rm = TRUE)
...
cn_dist_plot <- plot_ly(x = cn_data.all$MeanCopyNumber, type = 'histogram', name = "CN data", width = 800, height = 500) %>%
  ...
  add_lines(y = seq(0,1000, 100), x = rep(cn_data.all.percent[2],11), 
              ...
              name = "2nd percentile", showlegend = TRUE) %>%
  
  ##### Add 50th percentile
  add_lines(y = seq(0,1000, 100), x = rep(cn_data.all.percent[26],11), 
              line = list(color = "black", dash = "dash"), opacity = 0.7,
              name = "50th percentile", showlegend = TRUE) %>%
  
  ##### Add 95th percentile threshold
  add_lines(y = seq(0,1000, 100), x = rep(cn_data.all.percent[50],11), 
              line = list(color = "black", dash = "dash"), opacity = 1,
              name = "98th percentile", showlegend = TRUE) %>%
