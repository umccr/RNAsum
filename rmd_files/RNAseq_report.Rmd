...
caner_genes_annot.list[["civic_gene_summaries"]] <- read.table(params$civic_gene_summaries, sep="\t", as.is=TRUE, header=TRUE, row.names=NULL, fill = TRUE)
caner_genes_annot.list[["civic_var_summaries"]] <- read.table(params$civic_var_summaries, sep="\t", as.is=TRUE, header=TRUE, row.names=NULL, fill = TRUE)
caner_genes_annot.list[["civic_clin_evid"]] <- read.table(params$civic_clin_evid, sep="\t", as.is=TRUE, header=TRUE, row.names=NULL, fill = TRUE)
caner_genes_annot.list[["civic_variant_group"]] <- read.table(params$civic_variant_group, sep="\t", as.is=TRUE, header=TRUE, row.names=NULL, fill = TRUE)
```
...
Section reporting drugs that target dysregulated genes, fusion events (dysregulated pathways to be added) based on data from *[CIViC](https://civicdb.org/home){target="_blank"}* ([Griffith *et al.* 

reports suitable drugs based on identified actionable targets, which can be considered in the treatment decision making process.
...
cancer_genes.drug <- drugTable(unlist(ref_genes.list[["genes_immune"]]), data, target, params$sample_name, params$normal_group , params$cancer_group, ref_datasets.list[[ref]][[5]][, c(1,3)])
