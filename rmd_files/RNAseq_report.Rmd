...
fusionsBezierPlot <- function(fusion_annot, genes_annot) {
  ...
  ##### Keep only fusions for which both genes are annotated and have genomic coordinates available
  fusion_annot <- fusion_annot[ fusion_annot$SYMBOL %in% genes_annot$SYMBOL, ]
...
} else if ( !is.na(params$clinical_id) ) {
  subjectID <- params$clinical_id
}
...
if ( !is.null(params$umccrise) ) {
  ...
  if ( file.exists(purple_file) ) {
    ref_genes.list[["purple"]] <- read.table(purple_file, sep="\t", as.is=TRUE, header=TRUE, row.names=NULL, fill = TRUE, quote = "")
  } else {
...
if ( length(sampleID.col) > 0 ) {
  ...
  sampleID.col <- grep(subjectID, ref_dataset.list[[dataset]][["clinical_info"]][, sampleID.col])
}
