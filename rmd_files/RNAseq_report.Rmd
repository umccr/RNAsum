...
params:
  ...
  dataset_name_incl: ''
  bcbio_rnaseq: NULL
  dragen_rnaseq: '../data/test_data/stratus/test_sample_WTS'
  report_dir: '../data/test_data/stratus/test_sample_WTS/RNAsum'
  ref_data_dir: '../data'
  transform: 'CPM'
  filter: TRUE
  norm: 'TMM'
...
Transcriptome summary for patient sample **`r params$sample_name`**.
...
sv_prioritize <- function(sv_file) {
  ...
  if (length(readLines(con = sv_file, n = 2)) > 1) {
    ...
    ##### Due to changes in PURPLE output format there are two expected coumn names combinations
    if ( all(c("AF_BPI", "AF_PURPLE", "CN_PURPLE", "CN_change_PURPLE", "Ploidy_PURPLE") %in% names(read_tsv(sv_file, col_names = TRUE, col_types = col_types))) ) {
...
if ( !is.null(params$bcbio_rnaseq) ) {
  ...
  ##### Check if Dragen fusion output file exists
  dragen_fusion_file <- paste(dataDir, paste0(params$sample_name, ".fusion_candidates.final"), sep = "/")
  if ( file.exists(dragen_fusion_file) ) {
    #Dragen's fusion output file header starts with '#' hence first reading without header and then assigning the correct one
    dragen_fusion_file <- read.table(file = dragen_fusion_file, header = FALSE, quote = "")
    colnames(dragen_fusion_file) <- c("FusionGene", "Score", "LeftBreakpoint", "RightBreakpoint", "ReadNames")
    dragen_fusion <- dragen_fusion_file %>%
      tidyr::separate(col = FusionGene, into = c("gene1", "gene2"), sep = "--")
    ref_genes.list[["dragenFusion"]] <- dragen_fusion
...
ref_dataset.list[[dataset]] <- combineDatasets(sample_name=params$sample_name, sample_counts=counts, ref_data=ref_dataset, report_dir = paste0(params$report_dir, "/results/"), dataset = dataset, dataset_name_incl = params$dataset_name_incl)
...
# ARRIBA, PIZZLY and DRAGEN annotation of gene fusion events detected in given patient based on fusion results
if ( runFusionChunk ) {
...
for ( dataset in names(ref_dataset.list) ) {
  ...
  write.table(prepare2write(ref_dataset.list[[dataset]][["combined_data"]]), file = paste0(params$report_dir, "/results/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.combined_data.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(ref_dataset.list[[dataset]][["gene_annot_all"]]), file = paste0(params$report_dir, "/results/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.gene_annot_all.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
}
...
##### Percentiles
genes.expr.perc <- exprTable( genes = rownames(data), keep_all = TRUE, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "perc", scaling = scaling)

##### Z-scores
genes.expr.z <- exprTable( genes = rownames(data), keep_all = TRUE, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "z", scaling = scaling)
...
##### ...percerntiles
expr_data.perc <- exprTable( genes = rownames(expr_data), keep_all = TRUE, data = expr_data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, type = "perc", scaling = scaling)[[2]]

expr_genes <- expr_data.perc$SYMBOL

##### Get the "Diff" (Patient vs [comp_cancer]) Z-scores using exprTable function
expr_data.z <- exprTable( genes = expr_genes, keep_all = TRUE, data = expr_data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, type = "z", scaling = scaling)[[2]]
...
##### Add columns for info about reported fusions
fusions <- cbind(dragen.fusions, data.frame(matrix("", ncol = 5, nrow = nrow(dragen.fusions)), stringsAsFactors = FALSE))
colnames(fusions)[(ncol(fusions)-4):ncol(fusions)] <- c("FGID", "reported_fusion", "reported_fusion_geneA", "reported_fusion_geneB", "effector_gene")
...
if ( nrow(dragen.cancer_genes) > 0 ) {
  fusions$fusions_cancer[ fusions$geneA %in% dragen.cancer_genes$geneA ] <- "Yes"
  fusions$fusions_cancer[ fusions$geneB %in% dragen.cancer_genes$geneB ] <- "Yes"
}
...
fusions <- fusions[ order(fusions$reported_fusion, fusions$Score, fusions$fusions_cancer, fusions$reported_fusion_geneA, fusions$reported_fusion_geneB, decreasing = TRUE), ]
#fusions <- fusions[order(factor(fusions$confidence, levels=c("high", "medium", "low"))), ]

##### Keep only key columns
fusions <- fusions[ colnames(fusions) %in% c("geneA", "geneB", "Score", "LeftBreakpoint", "RightBreakpoint", "FGID", "reported_fusion", "reported_fusion_geneA", "reported_fusion_geneB", "effector_gene", "fusions_cancer")]
...
##### Clean the space and return output
rm(dragen.fusions, dragen.fusion.transcripts, dragen.cancer_genes, dragen.other_genes)
```
...
if ( runArribaChunk ) {
  ...
  fusion_annot <- cbind(fusion_annot1, fusion_annot2, fusions.annot[, c("Score", "LeftBreakpoint", "RightBreakpoint", "reported_fusion", "fusions_cancer", "reported_fusion_geneA", "reported_fusion_geneB")])
}
...
##### Compare FUSIONS and MANTA called gene fusion events
...
##### Compare fusion genes called by FUSION CALLER and MANTA
...
if ( runArribaChunk ) {
  ...
  fusion_annot <- fusion_annot %>% dplyr::filter( split_readsA > 1 | split_readsB > 1 | discordant_mates > 1 | geneA_dna_support != "-" | geneB_dna_support != "-" | reported_fusion != "-" | fusions_cancer != "-")
} else if (runPizzlyChunk) {
  ...
  fusion_annot <- fusion_annot %>% dplyr::filter(split_readsA > 1 | discordant_mates > 1 | geneA_dna_support != "-" | geneB_dna_support != "-" | reported_fusion != "-" | fusions_cancer != "-")
} else if (runDragenFusionChunk) {
  ...
  fusions <- fusions %>% dplyr::filter(Score > 1 | geneA_dna_support != "-" | geneB_dna_support != "-" | reported_fusion != "-" | fusions_cancer != "-")
...
##### Take into account only reported fusions or those with or cancer gene(s) involved
if ( runSVsChunk ) {
...
```{r fusions_summary_table_Arriba, comment = NA, message=FALSE, warning=FALSE, eval = runArribaChunk}
...
if ( runArribaChunk ) {
  ...
  fusions.table <- fusions.table[ , c("geneA", "geneB", "split_readsA", "split_readsB", "discordant_mates", "geneA_dna_support", "geneB_dna_support", "reported_fusion", "fusions_cancer", "reported_fusion_geneA", "reported_fusion_geneB", "confidence", "breakpointA", "breakpointB", "siteA", "siteB", "type", "circos")]
  names(fusions.table) <- c("Gene A", "Gene B", "Split reads (A)", "Split reads (B)", "Pair reads", "DNA support (A)", "DNA support (B)", "Reported fusion", "Cancer gene(s)", "Fusion gene (A)", "Fusion gene (B)", "Confidence (Arriba)", "Breakpoint (A)", "Breakpoint (B)", "Site (A)", "Site (B)", "Type", "Genomic view")
...
```{r fusions_summary_table_Dragen, comment = NA, message=FALSE, warning=FALSE, eval = runDragenFusionChunk}
##### Create a nice table output (with dataTable)
if ( runDragenFusionChunk ) {
  ...
  fusions.table <- fusions.table[ , names(fusions.table) %!in% "FGID" ]
  fusions.table <- fusions.table[ , c("geneA", "geneB", "Score", "LeftBreakpoint", "RightBreakpoint", "geneA_dna_support", "geneB_dna_support", "reported_fusion", "fusions_cancer", "reported_fusion_geneA", "reported_fusion_geneB", "circos")]
  names(fusions.table) <- c("Gene A", "Gene B", "Score", "Left breakpoint", "Right breakpoint", "DNA support (A)", "DNA support (B)", "Reported fusion", "Cancer gene(s)", "Fusion gene (A)", "Fusion gene (B)", "Genomic view")
...
`r if ( !runSVsChunk ) { c("Genomic data this sample is **NOT AVAILABLE**.") }`
...
if ( nrow(fusion_annot_top) > 0 && runArribaChunk ) {
...
} else if ( nrow(fusion_annot_top) > 0 && runDragenFusionChunk ) {
  
  ##### Clean the table for better presentation
  fusion_annot_top.clean <- fusion_annot_top[, c("SYMBOL", "SEQNAME", "GENESEQSTART", "GENESEQEND", "SYMBOL.1", "SEQNAME.1", "GENESEQSTART.1", "GENESEQEND.1", "LeftBreakpoint", "RightBreakpoint", "Score", "geneA_dna_support", "geneB_dna_support", "reported_fusion", "fusions_cancer") ]
  ...
  names(fusion_annot_top.clean) <- c("Gene A", "Chrom (A)", "Start (A)", "End (A)", "Gene B", "Chrom (B)", "Start (B)", "End (B)", "Breakpoint (A)", "Breakpoint (B)", "Score", "DNA support (A)", "DNA support (B)", "Reported fusion", "Cancer gene(s)")
...
if ( genes_no > 0 ) {
  for( i in 1:genes_no ) {
    ...
    if ( runArribaChunk && file.exists(gsub(":", ".", paste0(params$report_dir, "/results/arriba/", make.names(paste(geneA, geneB, sep = "__")), "_", breakpointA, "-", breakpointB, ".png"))) ) {
...
Section presenting expression levels of homologous recombination DNA damage repair (DDR) deficiency (HRD) genes to assess how many of these demonstrate low expression, which may indicate potential promoter methylation events. Their mRNA expression levels are presented in patient's sample along their average mRNA expression in samples from cancer cohorts.
...
mysql_populate <- paste0(mysql_populate, "\", \"Transcriptome summary for sample ", params$sample_name, " generated on ", Sys.Date(), "\"", ", \"", Sys.Date(), "\" )")
mysql_populate_update <- paste0(mysql_populate_update, "\", Summary=\"Transcriptome summary for sample ", params$sample_name, " generated on ", Sys.Date(), "\"", ", Date=\"", Sys.Date(), "\";")
