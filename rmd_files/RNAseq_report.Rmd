...
kallisto_quant_pizzly_file <- paste(dataDir, "kallisto/quant_pizzly_post/abundance.tsv", sep = "/")
kallisto_file <- paste(dataDir, "kallisto/abundance.tsv", sep = "/")
runKallistoChunk <- TRUE
runFusionChunk <- TRUE

if ( file.exists(kallisto_quant_pizzly_file) ) {

  ref_genes.list[["kallisto"]] <- read.table(kallisto_quant_pizzly_file, header = TRUE)
...
genes2keep <- unique( c(ref_genes.list[["genes_cancer"]]$symbol, ref_genes.list[["genes_oncokb"]]$Hugo.Symbol, ref_genes.list[["genes_immune"]]$SYMBOL, ref_genes.list[["genes_hrd"]]$SYMBOL, as.character(ref_genes.list[["pizzly"]]$geneA.name), as.character(ref_genes.list[["pizzly"]]$geneB.name), ref_genes.list[["pcgr"]]$SYMBOL, ref_genes.list[["purple"]]$Gene, unlist(strsplit(ref_genes.list[["manta"]]$Gene, split='&', fixed=TRUE))))
