...
##### Clean the space and return output
rm(quant, deduped.result, pizzly.fusion.transcripts, pizzly.fusion.candidates, quant.sorted.filtered, quant.fusions.only.transcripts, known_translocations.CGI, known_translocations.FusionGDB, result, result.cancer_genes, result.other_genes)
```
...
if ( !is.na(gene) && gene %in% rownames(data) ) {
  ...
  cdfPlot(gene = gene, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = TRUE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
...
##### Save the expression table as html file
if ( c(params$plots_mode == "interactive" || params$plots_mode == "semi-interactive") && params$save_tables ) {
  saveWidgetFix(widget=mut_cn_expr_genes.expr.losses.perc[[1]], file=paste(exprTableDir, "mut_cn_expr_genes.expr.losses.perc.html", sep = "/"), selfcontained=TRUE)
}
```
...
##### Save the expression table as html file
if ( c(params$plots_mode == "interactive" || params$plots_mode == "semi-interactive") && params$save_tables ) {
  saveWidgetFix(widget=immune_genes.expr.z[[1]], file=paste(exprTableDir, "immune_genes.expr.z.html", sep = "/"), selfcontained=TRUE)
}
```
...
##### Save the expression table as html file
if ( c(params$plots_mode == "interactive" || params$plots_mode == "semi-interactive") && params$save_tables ) {
  saveWidgetFix(widget=immune_genes.expr.perc[[1]], file=paste(exprTableDir, "immune_genes.expr.perc.html", sep = "/"), selfcontained=TRUE)
}
```
