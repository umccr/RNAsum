...
exprTable <- function(genes, data, targets, genes_annot = NULL) {
  ...
  genes <- as.data.frame(genes[ unlist(genes) %in% rownames(data) , ])
  names(genes) <- "gene"
  ...
  group.z <- group.z[ rownames(group.z) %in% unlist(genes), ]
  ...
  for ( group in targets.list ) {
    ...
    brks.q[[group]] <- quantile(group.z[, targets.list[1]], probs = seq(.05, .95, .0005), na.rm = TRUE)
