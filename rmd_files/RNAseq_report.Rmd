...
sv_prioritize <- function(sv_file) {
  ...
  if (length(readLines(con = sv_file, n = 2)) > 1) {
    ...
    if ( ncol(read_tsv(sv_file, col_names = TRUE)) == 22 ) {
      col_types <- "ccciiccccciiccccdc"
    } else if ( ncol(read_tsv(sv_file, col_names = TRUE)) == 23 ) {
      col_types <- "ccciiccccciiccccdcc"
    }
...
ref_dataset.list[[dataset]] <- combineDatasets(sample_name=sample_name, sample_counts=counts, ref_data=ref_dataset, report_dir = paste0(params$report_dir, "/results/"), dataset = dataset)
