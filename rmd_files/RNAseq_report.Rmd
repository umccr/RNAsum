...
Scatterplot comparing the per-gene **mRNA expression** values (*y-axis*, Z-scores), **CN values** (*x-axis*, from [PURPLE](https://anaconda.org/bioconda/hmftools-purple)). If the mutation status information is available then the genes's colours correspond to the variant(s) consequence (from [PCGR](https://github.com/sigven/pcgr)). Genes with CN values > 10 or < 0.5 are annotated.
...
Table summarising the per-gene **mRNA expression** values in normal, cancer and patient samples, the **CN values** based on patient's genomic data (from [PURPLE](https://anaconda.org/bioconda/hmftools-purple)), and mutation status if available (from [PCGR](https://github.com/sigven/pcgr)).
