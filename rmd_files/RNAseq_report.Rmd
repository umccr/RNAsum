...
exprTable <- function(genes, data, cn_data = NULL, targets, sampleName, normal, cancer, genes_annot = NULL, OncoKB_genes = NULL, mut_annot = NULL, type = "z") {
  ...
  if ( !is.null(cn_data) ) {
    ...
    ##### Order the data by decreasing CN values (to allow filtering based on CN information) and then by the highest absolute values for Patient vs Normal difference (to allow filtering based on z-score differences)
    group.z <- group.z[ order(abs(group.z[, "Patient vs Normal"]),  decreasing = TRUE), ]
...
##### Keep only altered genes with CN values below 1 (loss) and above 4 (gain)
  cn_data <- cn_data[ cn_data$MeanCopyNumber < 1 | cn_data$MeanCopyNumber > 4, ]
...
Summary of biological classification of detected features in patient’s sample, as well as in samples from cancer patients and healthy individuals. This may facilitate identification of potential abnormal contamination in the data. In RNA-seq data it is expected that most of the genes will be protein-coding so detecting an enrichment in the sample of any other biotype could point to a potential contamination.
...
data <- ref_datasets.list[[tissue]][["combined_data"]]
target <- ref_datasets.list[[tissue]][["sample_annot"]][, c("Dataset", "Target")]
target$Target[target$Target==normal_group] <- "Normal"
...
NOISeq_data <- readData( data = data, biotype = gene_biotypes, factors = target)
...
mut_genes.expr <- exprTable( genes = genes, data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], type = "z")
...
mut_genes.expr <- exprTable( genes = genes, data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], type = "perc")
...
The aim of this section is to overlay the per-gene somatic copy-number (CN) data with mRNA expression data and, where available, mutation status. <span style="color:#ff0000">NOTE</span>: the work on this section is in progress. 
...
if ( !is.null(ref_genes.list[["pcgr"]]) && !is.null(ref_genes.list[["purple"]]) ) {
  ...
  plot_ly(data, x = ~CN, y = ~mRNA, color = ~Mutation, text=~Gene, type='scatter', mode = "markers", marker = list(size=10, symbol="circle"), width = 800, height = 600) %>%
...
##### Generate scatterplot with per-gene expression values (y-axis) and CN values (x-axis)
  ...
  plot_ly(data, x = ~CN, y = ~mRNA, text=~Gene, type='scatter', mode = "markers", marker = list(size=10, symbol="circle"), width = 800, height = 600) %>%
...
Table summarising the per-gene **mRNA expression** values in normal, cancer and patient samples, the **CN values** based on patient's genomic data (from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}), and mutation status if available (from [PCGR](https://github.com/sigven/pcgr){target="_blank"}).

#### Z-scores

```{r mut_cn_expr_data_table, comment = NA, message=FALSE, warning=FALSE}
##### Generate expression summary table for per-gene expression values CN values and mutation status info (colours)
if ( !is.null(ref_genes.list[["pcgr"]]) && !is.null(ref_genes.list[["purple"]]) ) {
  
  cn_data <- as.data.frame(ref_datasets.list[[tissue]][["expr_mut_cn_data"]]$CN)
  rownames(cn_data) <- ref_datasets.list[[tissue]][["expr_mut_cn_data"]]$Gene
  names(cn_data) <- "CN"
  
  data <- ref_datasets.list[[tissue]][["combined_data_processed"]]
  
  mut_cn_expr_genes.expr <- exprTable( genes = rownames(cn_data), data = data, cn_data = cn_data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], type = "z")
...
##### Generate expression summary table for per-gene expression values and CN values
  ...
  mut_cn_expr_genes.expr <- exprTable( genes = rownames(cn_data), data = data, cn_data = cn_data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c("SYMBOL", "ENSEMBL")], type = "z")
...
```{r mut_cn_expr_data_table_perc, comment = NA, message=FALSE, warning=FALSE}
##### Generate expression summary table for per-gene expression values CN values and mutation status info (colours)
if ( !is.null(ref_genes.list[["pcgr"]]) && !is.null(ref_genes.list[["purple"]]) ) {
  ...
  mut_cn_expr_genes.expr <- exprTable( genes = rownames(cn_data), data = data, cn_data = cn_data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], type = "perc")
...
##### Generate expression summary table for per-gene expression values and CN values
  ...
  mut_cn_expr_genes.expr <- exprTable( genes = rownames(cn_data), data = data, cn_data = cn_data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c("SYMBOL", "ENSEMBL")], type = "perc")
