...
if ( duplicated(pizzly.fusions.annot) != 0 ) {
...
pizzly.fusions.annot <- pizzly.fusions.annot[ , names(pizzly.fusions.annot) %!in% c("geneA.id", "geneB.id", "transcripts.list") ]
pizzly.fusions.annot <- pizzly.fusions.annot[ , c("geneA.name", "geneB.name", "paircount", "splitcount", "fusions_abundant", "fusions_cancer", "Reported", "Effector_gene", "Source", "Cancer_acronym") ]

##### Create a nice table output (with dataTable)
names(pizzly.fusions.annot) <- c("Gene A", "Gene B", "Pair count", "Split count", "Abundant transscript(s)", "Cancer gene(s)", "Reported fusion gene(s)", "Effector gene", "Source", "Cancer acronym")

DT::datatable( data = pizzly.fusions.annot, filter="none", rownames = FALSE, options = list(pageLength = 10, scrollX = TRUE), width = 800,  escape = FALSE) %>%
    DT::formatStyle( columns = names(pizzly.fusions.annot), `font-size` = '12px', 'text-align' = 'center' )
```
...
* *In_silico_predicted*: gene identified by an ensemble of bioinformatics tools to have signals of positive selection in its mutational pattern across tumors (obtained from the [IntOGen](https://www.intogen.org/search){target="_blank"} repository)
