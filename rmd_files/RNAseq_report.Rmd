...
# 1. ref_genes.list[["genes_cancer"]] = list of cancer genes derived from UMCCR Cancer Gene list (https://trello.com/c/7j3KFMiL/184-umccr-cancer-genes) and OncoKB portal (http://oncokb.org/#/cancerGenes) 
...
for ( tissue in names(ref_datasets.list) ) {
  ...
  ##### Kep only immune response marjers for which there is available annotation
...
for ( tissue in names(ref_datasets.list) ) {
  ...
  ##### Save the combined expression matrix and associated targets into txt files
...
for ( i in 1:nrow(pizzly.fusions) ) {
  ...
  ##### Now check if any ofthe pizzly detected fusion genes are reported
    ...
    ##### Now check the fusion genes in the UMCCR Cancer Gene list
    ##### Both genes are reported
    if ( geneA %in% rownames(ref_genes.list[["genes_cancer"]])[ ref_genes.list[["genes_cancer"]]$Fusion=="Yes" ] && geneB %in% rownames(ref_genes.list[["genes_cancer"]])[ ref_genes.list[["genes_cancer"]]$Fusion=="Yes" ] ) {
      
      pizzly.fusions$Reported[i] <- paste(as.vector(geneA), as.vector(geneB), sep=",")
      pizzly.fusions$Source[i] <- paste("UMCCR", pizzly.fusions$Source[i], sep = ",")
      
    ##### Gene A is reported
    } else if ( geneA %in% rownames(ref_genes.list[["genes_cancer"]])[ ref_genes.list[["genes_cancer"]]$Fusion=="Yes" ] ) {
      
      pizzly.fusions$Reported[i] <- as.vector(geneA)
      pizzly.fusions$Source[i] <- paste("UMCCR", pizzly.fusions$Source[i], sep = ",")
      
    ##### Gene B is reported
    } else if ( geneB %in% rownames(ref_genes.list[["genes_cancer"]])[ ref_genes.list[["genes_cancer"]]$Fusion=="Yes" ] ) {
      
      pizzly.fusions$Reported[i] <- as.vector(geneB)
      pizzly.fusions$Source[i] <- paste("UMCCR", pizzly.fusions$Source[i], sep = ",")
    }
  }
}

##### remove redundant commas in the Source column
pizzly.fusions$Source[ pizzly.fusions$Source == "UMCCR," ] <- "UMCCR"
...
2. In the second step, fusion genes reported by [pizzly](https://github.com/pmelsted/pizzly){target="_blank"} (transcriptome-based evidence) are compared with fusion events reported by [manta](https://github.com/Illumina/manta){target="_blank"} (DNA-based evidence), if available.
...
pizzly.fusions.summary <- DT::datatable( data = pizzly.fusions.table, filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE) %>%
...
* *UMCCR*: fusion genes listed among the UMCCR Cancer Genes
...
**`r nrow(fusion_annot[ fusion_annot$geneA_dna_support == "Yes" | fusion_annot$geneB_dna_support == "Yes" , ])`** fusion events involving `r length(unique(c(fusion_annot$SYMBOL[ fusion_annot$geneA_dna_support == "Yes"], fusion_annot$SYMBOL.1[ fusion_annot$geneB_dna_support == "Yes"])))` genes located within detected [Structural variants] (obtained from [Manta](https://github.com/Illumina/manta){target="_blank"}) are presented in the genomic context. The table at the bottom contains genomic coordingates of individual fusion genes sorted based on their genomic location.
...
exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Germline") ], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "z")[[1]]
```
...
mRNA expression levels of genes within detected structural variants (SVs), obtained from [Manta](https://github.com/Illumina/manta){target="_blank"} structural variant caller, in patient's sample and their average mRNA expression in samples from cancer patients and healthy individuals.
...
sv_genes.expr.z <- exprTable( genes = genes, data = data, sv_data = manta_sv[ , names(manta_sv) %!in% c("lof", "SR", "PR") ], targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z")
...
##### Generate expression summary table for cancer genes from OncoKB and PMCC
sv_genes.expr.perc <- exprTable( genes = genes, data = data, sv_data = manta_sv[ , names(manta_sv) %!in% c("lof", "SR", "PR") ], targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc")
...
Scatterplot comparing the per-gene **mRNA expression** values (*y-axis*, Z-scores), **CN values** (*x-axis*, from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}). If the mutation status information is available then the genes's colours correspond to the variant(s) consequence (from [PCGR](https://github.com/sigven/pcgr){target="_blank"}). Genes with CN values > 3 or < 0.5 are annotated. NOTE: only [Cancer genes], [Mutated genes] with [tier 1-3](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} variants (if mutation information is available), genes involved in [Gene fusion events] and [Structural variants] (if SV information is available) are presented.
...
Out of the `r length(ref_datasets.list[[tissue]][["expr_mut_cn_data"]][,1])` genes within gained (CN values > `r params$cn_gain`) or lossed (CN values < `r params$cn_loss`) regions `r if (runPurpleChunk) { length(cn_genes) } else { length(NULL) }` are either [Cancer genes], [Mutated genes] with [tier 1-3](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} variants (if mutation information is available) or are involved in [Gene fusion events] or [Structural variants] (if SV information is available). The expression of **`r if (runPurpleChunk) { length(which(cn_genes %in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]]))) } else { length(NULL) }`** of these genes was reliably measured in patient's sample. The remaining `r if (runPurpleChunk) { length(which(cn_genes %!in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]]))) } else { length(NULL) }` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
Section presenting expression levels of immune response markers to assess pre-existing anti-cancer immunity and likelihood of response to immunotherapy. Their mRNA expression levels are presented in patient's sample along their average mRNA expression in samples from cancer patients and healthy individuals.
...
<summary>Table legend</summary>
<font size="2">
The <span style="color:#ff0000">RED</span> colour range indicate relatively **high expression** (Z-score) values and <span style="color:#0000ff">BLUE</span> colour range indicate relatively **low expression** (Z-score) values in individual sample group. The <span style="color:#808080">BLANK</span> cells with missing values indicate genes with **no/low expression**. The **Patient vs Normal** column illustrates the difference between Z-scores in patient sample and cohort of healthy individuals for each immune response marker. Genes are ordered by **decreasing** absolute values in the **Patient vs Normal** column. SD - standard deviation
</font>
</details>
...
**`r nrow(fusion_annot_top)`** gene fusion events involving DNA-supported fusion genes (see [Structural variants] section), abundant transcripts or cancer genes (see [Cancer genes] section), indicated in <span style="color:#02d653">green</span>, <span style="color:#ff0000">red</span> and <span style="color:#0000ff">blue</span> columns in the [Gene fusion events] table, respectively, and with the highest *Split count* and *Pair count* values were screened for suitable drugs.
...
drugsTable.fusion_genes_abundant <- civicDrugTable(genes = unique(c(genesA, genesB)), civic_var_summaries  = caner_genes_annot.list[["civic_var_summaries"]], civic_clin_evid  = caner_genes_annot.list[["civic_clin_evid"]],  evid_type = "Predictive", var_type = "fusion")

drugsTable.fusion_genes_abundant[[1]]

##### Save the expression table as html file
if ( params$plots_mode == "interactive" ) {
  DT::saveWidget(widget=drugsTable.fusion_genes_abundant[[1]], file=paste(drugsTableDir, "drugsTable.fusion_genes_abundant.html", sep = "/"), selfcontained=TRUE)
}
```
...
drugsTable.fusion_genes_cancer <- civicDrugTable(genes = unique(c(genesA, genesB)), civic_var_summaries  = caner_genes_annot.list[["civic_var_summaries"]], civic_clin_evid  = caner_genes_annot.list[["civic_clin_evid"]],  evid_type = "Predictive", var_type = "fusion")

drugsTable.fusion_genes_cancer[[1]]

##### Save the expression table as html file
if ( params$plots_mode == "interactive" ) {
  DT::saveWidget(widget=drugsTable.fusion_genes_cancer[[1]], file=paste(drugsTableDir, "drugsTable.fusion_genes_cancer.html", sep = "/"), selfcontained=TRUE)
}
```
...
**`r if ( runPurpleChunk ) { length(cn_genes) } else { length(NULL) }`** genes with CN values > `r params$cn_gain` (**gains**) or < `r params$cn_loss` (**losses**, see [Copy-number altered genes] section), and reported as either [Cancer genes], [Mutated genes] with [tier 1-3](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} variants (if mutation information is available) or involved in [Gene fusion events] or [Structural variants] (if SV information is available) were screened for suitable drugs.
