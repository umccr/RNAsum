...
getColours <- function(elements) {
  ...
  if ( length(unique(elements)) == 3 ) {
    ...
    elements.colours <- c("powderblue", "red", "forestgreen", "gray50")
  } else {
    elements.colours <- rainbow(length(elements))
  }
  
  f.elements <- factor(elements)
...
densityPlot <- function(gene, data, main_title, x_title, sampleName, distributions = NULL, plot_mode = "static", scaling = "gene-wise", report_dir) {
  ...
  p <- plot_ly(data.df, x = ~expr, y = ~dens, type = 'scatter', mode = 'lines', color = ~gene, colors = genes.colour[[1]], width = 800, height = 200) %>%
  ...
  ##### Save interactive plot as html file
    ...
    saveWidgetFix(p, file = paste(PlotsDir, paste0(gene, "_expr_distr_plot.html"), sep = "/"))
...
for ( dataset in names(ref_dataset.list) ) {
  ...
  if ( params$batch_rm ) {
    ref_dataset.list[[dataset]][["pca_combined_data_processed"]] <- pca(data = ref_dataset.list[[dataset]][["combined_data_processed"]], target = target, title = "Before batch-effects correction", plot_mode = params$plots_mode, report_dir = params$report_dir)
    
    ref_dataset.list[[dataset]][["pca_batch_effect_corrected"]] <- pca(data = ref_dataset.list[[dataset]][["batch_effect_corrected"]], target = target, title = "After batch-effects correction",  plot_mode = params$plots_mode, report_dir = params$report_dir)
    
    ref_dataset.list[[dataset]][["data_to_report"]] <- ref_dataset.list[[dataset]][["batch_effect_corrected"]]
    
  } else {
    ref_dataset.list[[dataset]][["pca_combined_data_processed"]] <- pca(data = ref_dataset.list[[dataset]][["combined_data_processed"]], target = target, plot_mode = params$plots_mode, report_dir = params$report_dir)
...
for( i in 1:genes_no ) {
  if ( genes[i] %in% rownames(data) ) {
    output_cdf[[length(output_cdf) + 1]] <- cdfPlot(gene = genes[i], data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = TRUE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
    
    output_density[[length(output_density) + 1]] <- densityPlot(gene = genes[i], data = data, main_title= "", x_title = "Z-score", sampleName = params$sample_name, distributions = c("normal", "bimodal"), plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir ) 
  }
}
...
for( i in 1:genes_no ) {
  ...
  fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
    ...
    output_transcripts[[length(output_transcripts) + 1]] <- fusion_reads.selected %>% 
  ...
  if ( geneA %in% rownames(data) && geneB %in% rownames(data) ) {
    ...
    ##### Gene A
    output_cdf_A[[length(output_cdf_A) + 1]] <- cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
    
    output_density_A[[length(output_density_A) + 1]] <- densityPlot(gene = geneA, data = data, main_title= "", x_title = "Z-score", sampleName = params$sample_name, distributions = c("normal", "bimodal"), plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir ) 
    
    ##### Gene B
    output_cdf_B[[length(output_cdf_B) + 1]] <- cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
    
    output_density_B[[length(output_density_B) + 1]] <- densityPlot(gene = geneB, data = data, main_title= "", x_title = "Z-score", sampleName = params$sample_name, distributions = c("normal", "bimodal"), plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir ) 
