...
if ( file.exists(dragen_fusion_file) ) {
  ...
  dragen_fusion <- read.table(file = dragen_fusion_file, header = FALSE, quote = "")
...
if ( runArribaChunk && runDragenFusionChunk ) {
  ...
  fusion_annot <- fusion_annot %>% dplyr::filter(split_reads > 1 | Score > 1 | geneA_dna_support != "-" | geneB_dna_support != "-" | reported_fusion != "-" | fusions_cancer != "-")
