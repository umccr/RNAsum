...
params:
  ...
  count_file:
  tissue: 'pancreas'
  datasets: '../data/Datasets_list_cervix.txt'
  normal_group: 'Cervix_uteri (normal)'
  cancer_group: 'Cervical squamous cell carcinoma'
...
for ( ref in names(ref_datasets.list) ) {
  ...
  ref_datasets.list[[ref]][[5]] <- data.annot[, c("SYMBOL", "GENEBIOTYPE", "ENSEMBL", "ENTREZID", "SEQNAME", "GENESEQSTART", "GENESEQEND")]
}
```
...
fusion_genes_annot <- ref_datasets.list[[ref]][[5]][ , c("SYMBOL", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND") ]
...
* **`r paste (ref_normal, params$normal_group, sep=" ")`** samples from [The Genotype-Tissue Expression](https://commonfund.nih.gov/gtex){target="_blank"} (GTEx) program
* **`r paste (ref_cancer, params$cancer_group, sep=" ")`** samples from [The Cancer Genome Atlas](https://cancergenome.nih.gov/abouttcga){target="_blank"} (TCGA) project
...
fusion_annot_top <- fusion_annot[ fusion_annot$pizzly.fusions.annot.fusions_abundant == "Yes" | fusion_annot$pizzly.fusions.annot.fusions_abundant == "Yes" , ]

##### Generate bezier curves-like plot representing gene fusion events
fusionsBezierPlot(fusion_annot_top, ref_datasets.list[[ref]][[5]])
```
...
names(fusion_annot_top.clean) <- c("Gene A", "Chrom (A)", "Start (gene A)", "End (gene A)", "Gene B", "Chrom (gene B)", "Start (gene B)", "End (gene B)", "Abundant transcript(s)", "Cancer gene(s)")
...
<details>
<summary>mRNA expression levels</summary>

```{r expr_plot_fusions, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5, eval = FALSE}
suppressMessages(library(plotly))
##### Generate plots with genome-wide expression profile (Z-score) for each group

fuiosnExprProfile <- exprProfilePlot(unique(c(fusion_annot_top$SYMBOL, fusion_annot_top$SYMBOL.1)), data, target, params$sample_name, params$normal_group, params$cancer_group, ref_datasets.list[[ref]][[5]])
...
<span style="color:#ff0000">NOTE</span>: currently, displaying the top hits from pizzly filtering output (<span style="color:#ff0000">red</span> and <span style="color:#0000ff">blue</span> sections in [Gene fusion events] table). Also, in the following visualisation  section, the missing hits from the prioritised results are the ones for which no junctions are found by [clinker](https://github.com/Oshlack/Clinker){target="_blank"}.
