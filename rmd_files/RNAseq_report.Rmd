...
fusionsBezierPlot <- function(fusion_annot, genes_annot) {
  ...
  ##### ... and the events type
  type <- paste(fusion_annot$pizzly.fusions.annot.fusions_abundant, fusion_annot$pizzly.fusions.annot.fusions_cancer, fusion_annot$geneA_dna_evidence, fusion_annot$geneB_dna_evidence, sep = "-") 
  type <- gsub("Yes-Yes-Yes-Yes","Abundant, cancer and SVs-affected gene(s)", type)
  type <- gsub("Yes-Yes---Yes","Abundant, cancer and SVs-affected gene(s)", type)
  type <- gsub("Yes-Yes-Yes--","Abundant, cancer and SVs-affected gene(s)", type)
  type <- gsub("Yes-Yes----","Abundant and cancer gene(s)", type)
  type <- gsub("Yes---Yes-Yes","Abundant and SVs-affected gene(s)", type)
  type <- gsub("Yes---Yes--","Abundant and SVs-affected gene(s)", type)
  type <- gsub("Yes-----Yes","Abundant and SVs-affected gene(s)", type)
  type <- gsub("Yes------","Abundant transcript(s)", type)
  type <- gsub("--Yes-Yes-Yes","Cancer and SVs-affected gene(s)", type)
  type <- gsub("--Yes-Yes--","Cancer and SVs-affected gene(s)", type)
  type <- gsub("--Yes---Yes","Cancer and SVs-affected gene(s)", type)
  type <- gsub("--Yes----","Cancer gene(s)", type)
  type <- gsub("----Yes-Yes","SVs-affected gene(s)", type)
  type <- gsub("----Yes--","SVs-affected gene(s)", type)
  type <- gsub("------Yes","SVs-affected gene(s)", type)
...
ref_genes.list[["pizzly"]] <- read.table(file = paste(dataDir, "pizzly", paste0(sampleName, "-flat-filtered.tsv"), sep = "/"), header = TRUE)
...
fusion_annot$geneA_dna_evidence <- "-"
fusion_annot$geneB_dna_evidence <- "-"
```
...
##### Add column to flag fusions supported by WGS data (from MANTA), if available
pizzly.fusions.cut$geneA_dna_evidence <- "-"
pizzly.fusions.cut$geneB_dna_evidence <- "-"

pizzly.fusions.cut <- pizzly.fusions.cut[ , c("geneA.name", "geneB.name", "paircount", "splitcount", "fusions_abundant", "fusions_cancer", "geneA_dna_evidence", "geneB_dna_evidence", "Reported", "Effector_gene", "Source", "Cancer_acronym") ]
...
if ( length(manta_pizzly.fusions) > 0 ) {
  ...
  pizzly.fusions.cut$geneA_dna_evidence[ sort( match( manta_pizzly.fusions , pizzly.fusions.cut$geneA.name ), na.last = NA ) ] <- "Yes"
  pizzly.fusions.cut$geneB_dna_evidence[ sort( match( manta_pizzly.fusions , pizzly.fusions.cut$geneB.name ), na.last = NA ) ] <- "Yes"
    
  fusion_annot$geneA_dna_evidence[ sort( match( manta_pizzly.fusions , fusion_annot$SYMBOL ), na.last = NA ) ] <- "Yes"
  fusion_annot$geneB_dna_evidence[ sort( match( manta_pizzly.fusions , fusion_annot$SYMBOL.1 ), na.last = NA ) ] <- "Yes"

  ##### Re-order pizzly dataframe with MANTA supporting fusions on top
  idx <- order(pizzly.fusions.cut$geneA_dna_evidence, pizzly.fusions.cut$geneB_dna_evidence, decreasing = TRUE)
...
```{r biotype_detection, comment=NA, message=FALSE, warning=FALSE, results='hide', eval = FALSE}
...
```{r biotype_detection_per_sample, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 4, eval = FALSE}
...
```{r biotype_detection_distribution, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 4, eval = FALSE}
##### Generate per-sample biodetection plot
for ( sample in c("Patient", cancer_group, "Normal") ) {
...
```{r biotype_detection_comparison_1, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 4, eval = FALSE}
...
```{r biotype_detection_comparison_2, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 4, eval = FALSE}
...
```{r biotype_detection_comparison_3, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 4, eval = FALSE}
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[2] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[3] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[4] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[5] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[6] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[7] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[8] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[9] }`
...
#### `r if ( runPcgrChunk) { mut_genes.expr.z[[2]]$SYMBOL[10] }`
...
names(pizzly.fusions.cut) <- c("Gene A", "Gene B", "Pair count", "Split count", "Abundant transcript(s)", "Cancer gene(s)", "DNA evidence (gene A)", "DNA evidence (gene B)", "Fusion gene(s)", "Effector gene", "Source", "Cancer acronym")

##### Present gene fusion events in a table
DT::datatable( data = pizzly.fusions.cut, filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE) %>%
    DT::formatStyle( columns = names(pizzly.fusions.cut), `font-size` = '12px', 'text-align' = 'center' ) %>%
  
    ##### Highlight rows with fusions involving hihgly abundant transcripts (red), cancer genes (blue) or DNA evidence (from MANTA)
    DT::formatStyle( columns = colnames(pizzly.fusions.cut) %in% "Abundant transcript(s)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'coral')) ) %>%
    DT::formatStyle( columns = colnames(pizzly.fusions.cut) %in% "Cancer gene(s)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightblue')) ) %>%
    DT::formatStyle( columns = colnames(pizzly.fusions.cut) %in% "DNA evidence (gene A)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) ) %>%
    DT::formatStyle( columns = colnames(pizzly.fusions.cut) %in% "DNA evidence (gene B)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) )
```

<details>
<summary>Table legend</summary>
<font size="2">
Cell in <span style="color:#ff0000">RED</span> indicate **highly abundant** fusion transcript(s), cells in <span style="color:#0000ff">BLUE</span> indicate fusions containing **[Cancer genes]** and those hihglighted in <span style="color:#02d653">GREEN</span> indicate genes affected by detected DNA **[Structural variants]**. Genes known to be involved in gene fusions are flagged based on information provided in [Cancer Genome Interpreter](https://www.cancergenomeinterpreter.org/biomarkers){target="_blank"} (CGI) database.
...
**DNA evidence (gene A/B)**: gene(s) affected by DNA [Structural variants] detected by [Manta](https://github.com/sigven/pcgr){target="_blank"}
...
fusion_annot_top <- fusion_annot[ fusion_annot$pizzly.fusions.annot.fusions_abundant == "Yes" | fusion_annot$pizzly.fusions.annot.fusions_cancer == "Yes" | fusion_annot$geneA_dna_evidence == "Yes" | fusion_annot$geneB_dna_evidence == "Yes" , ]
...
fusion_annot_top.clean <- fusion_annot_top[, c("SYMBOL", "SEQNAME", "GENESEQSTART", "GENESEQEND", "SYMBOL.1", "SEQNAME.1", "GENESEQSTART.1", "GENESEQEND.1", "pizzly.fusions.annot.fusions_abundant", "pizzly.fusions.annot.fusions_cancer", "geneA_dna_evidence", "geneB_dna_evidence") ]
...
names(fusion_annot_top.clean) <- c("Gene A", "Chrom (A)", "Start (A)", "End (A)", "Gene B", "Chrom (B)", "Start (B)", "End (B)", "Abundant transcript(s)", "Cancer gene(s)", "DNA evidence (gene A)", "DNA evidence (gene B)")
...
DT::datatable( data = fusion_annot_top.clean, filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE) %>%
    ...
    DT::formatStyle( columns = colnames(fusion_annot_top.clean) %in% "DNA evidence (gene A)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) ) %>%
    DT::formatStyle( columns = colnames(fusion_annot_top.clean) %in% "DNA evidence (gene B)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) )
```
...
Expression profiles for the top 5 gene fusion events detected in patient's sample and listed within <span style="color:#ff0000">red</span> (abundant transcripts and <span style="color:#0000ff">blue</span> (cancer genes) sections in the [Gene fusion events] table). 
...
if ( !is.na(geneA) && !is.na(geneB) ) {
  ...
  cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
    ...
    cat(paste("There are", nrow(pizzly.fusions), " gene fusion events detected!", sep=" "))
}
...
#### `r ( !is.na(pizzly.fusions$geneA.name[2]) ) { paste(pizzly.fusions$geneA.name[2], pizzly.fusions$geneB.name[2], sep="-") }`
...
#### `r ( !is.na(pizzly.fusions$geneA.name[3]) ) { paste(pizzly.fusions$geneA.name[3], pizzly.fusions$geneB.name[3], sep="-") }`
...
#### `r ( !is.na(pizzly.fusions$geneA.name[4]) ) { paste(pizzly.fusions$geneA.name[4], pizzly.fusions$geneB.name[4], sep="-") }`
...
#### `r ( !is.na(pizzly.fusions$geneA.name[5]) ) { paste(pizzly.fusions$geneA.name[5], pizzly.fusions$geneB.name[5], sep="-") }`
...
```{r cdf_plot_fusions_5, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
if ( !is.na(geneA) && !is.na(geneB) ) {
...
```{r gene_table_fusions_5_perc, comment = NA, message=FALSE, warning=FALSE}
exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "perc")[[1]]
```
...
Out of the `r length(unique(manta_sv$gene))` genes affected by `r nrow(manta_sv)` SVs, the expression of **`r length(which(unique(manta_sv$gene) %in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]])))`** was reliably measured in patient's sample. The remaining `r length(which(unique(manta_sv$gene) %!in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]])))` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
Expression profiles for the top 10 SVs-affected genes demonstrating the greatest difference in standarised (Z-score) mRNA expression values between patient's sample and the average mRNA expression in samples from healthy individuals.

#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[1] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[2] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[3] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[4] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[5] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[6] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[7] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[8] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[9] }`
...
#### `r if ( runSVsChunk) { unique(sv_genes.expr.z[[2]]$SYMBOL)[10] }`
...
Scatterplot comparing the per-gene **mRNA expression** values (*y-axis*, Z-scores), **CN values** (*x-axis*, from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}). If the mutation status information is available then the genes's colours correspond to the variant(s) consequence (from [PCGR](https://github.com/sigven/pcgr){target="_blank"}). Genes with CN values > `r params$cn_gain` or < `r params$cn_loss` are annotated. NOTE: only [Cancer genes] and [Mutated genes] with [tier 1-3](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} variants (if mutation data is available) are presented.
...
if ( !is.null(ref_genes.list[["pcgr"]]) ) {
  ...
  mutCNexprPlot(data = data, mut_data = TRUE, cn_bottom = params$cn_gain, cn_top = params$cn_loss, plot_mode = params$plots_mode)
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.gains.z[[2]]$SYMBOL[1] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.gains.z[[2]]$SYMBOL[2] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.gains.z[[2]]$SYMBOL[3] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.gains.z[[2]]$SYMBOL[4] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.gains.z[[2]]$SYMBOL[5] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.losses.z[[2]]$SYMBOL[1] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.losses.z[[2]]$SYMBOL[2] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.losses.z[[2]]$SYMBOL[3] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.losses.z[[2]]$SYMBOL[4] }`
...
##### `r if ( runPurpleChunk) { mut_cn_expr_genes.expr.losses.z[[2]]$SYMBOL[5] }`
...
**`r nrow(fusion_annot_top.clean)`** gene fusion events involving highly abundant transcripts and/or cancer genes (<span style="color:#ff0000">red</span> and <span style="color:#0000ff">blue</span> sections in [Gene fusion events] table) were screened for suitable drugs.
...
**`r length(unique(manta_sv$gene))`** genes affected by structural variants (SVs) were screened for suitable drugs (see [Structural variants] section).
