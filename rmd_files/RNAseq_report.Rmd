...
cdfPlot <- function(gene, data, targets, sampleName, int_cancer, ext_cancer, comp_cancer, add_cancer = NULL, addBoxPlot = FALSE, plot_mode = "static", scaling = "gene-wise") {
  ...
  if ( addBoxPlot ) {
    ...
    if ( !is.null(add_cancer) ) {
      ...
      gene.expr.df$Group <- factor(gene.expr.df$Group, levels=c(ext_cancer, int_cancer, add_cancer, "Patient"))
      group.colors <- c("cornflowerblue", "red", "forestgreen", "black")
    } else {
      gene.expr.df$Group <- factor(gene.expr.df$Group, levels=c(ext_cancer, int_cancer, "Patient"))
      group.colors <- c("cornflowerblue", "red", "black")
    }
    
    p2 <- plot_ly(gene.expr.df, x= ~Expression, color = ~Group, type = 'box', jitter = 0.3, pointpos = 0, boxpoints = 'all', colors = group.colors, opacity = 0.5, orientation = 'h', width = 800, height = 400, showlegend=FALSE)
  }
...
glanceExprPlot <- function(genes, data, targets, sampleName, int_cancer, ext_cancer, comp_cancer, add_cancer = NULL, plot_mode = "static", hexcode, scaling = "gene-wise") {
  ...
  if ( !is.null(add_cancer) ) {
    ...
    group.colours <- c(I("black"), "cornflowerblue", "forestgreen")
...
if ( params$filter ) {
  ...
  ##### Keep only genes with read counts below the 90th percentile
  data.df <- data.df[ data.df$Counts < quantile(data.df$Counts, 0.9), ]
  ...
  counts_vs_transformed <- plot_ly( data.df, x = ~Transformed, y = ~Counts, width = 800, height = 300, color = I('black'), marker = list(size = 5), type="scatter", mode = "markers", name = paste0(params$transform, " / Counts (Patient)") ) %>% 
    ...
    layout(title = "", xaxis = list(title = paste0(params$transform, "s")), yaxis = list(title = "Read counts"), showlegend=TRUE)
...
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "immune_genes", scaling = scaling)
...
##### Generate overview boxplot
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "hrd_genes", scaling = scaling)
