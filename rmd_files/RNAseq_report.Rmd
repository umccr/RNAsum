...
getDatasetsColours <- function(datasets) {
  ...
  if ( length(unique(datasets)) == 3 ) {
    datasets.colours <- c("powderblue", "red", "gray50")
  } else if ( length(unique(datasets)) == 4 ) {
    datasets.colours <- c("powderblue", "red", "forestgreen", "gray50")
  }
...
pca <- function(data, targets, title = "", plot_mode = "static", report_dir ) {
  ...
  targets.colour <- getDatasetsColours(targets$Target)
...
colnames(data.subset_scree.df) <- c("PC", "Variances")
  ...
  if ( plot_mode == "static" ) {
      ...
      PlotsDir <- paste(report_dir, "InputDataPlots", sep = "/")
...
cdfPlot <- function(gene, data, targets, sampleName, int_cancer, ext_cancer, comp_cancer, add_cancer = NULL, addBoxPlot = FALSE, plot_mode = "static", scaling = "gene-wise", report_dir) {
  ...
  layout(xaxis = list(title = "mRNA expression (Z-score)", zeroline = FALSE, range = c(min(group.z[[ sampleName ]]$z)-1.5, max(group.z[[ sampleName ]]$z)+1.5)),
    ...
    if ( plot_mode == "static" || plot_mode == "semi-interactive" ) {
      ...
      PlotsDir <- paste(report_dir, "CDF_plots", sep = "/")
      if ( !file.exists(PlotsDir) ) {
...
glanceExprPlot <- function(genes, data, targets, sampleName, int_cancer, ext_cancer, comp_cancer, add_cancer = NULL, plot_mode = "static", hexcode, type = "z", sort = "diff", scaling = "gene-wise", report_dir) {
...
mutCNexprPlot <- function(data, alt_data = FALSE, cn_bottom = cn_bottom, cn_top = cn_top, comp_cancer, type = "z", plot_mode = "static", report_dir) {
...
ref_dataset.list[[dataset]] <- combineDatasets(sample_name=params$sample_name, sample_counts=kallisto.counts, ref_data=ref_dataset, report_dir = params$report_dir, dataset = dataset, dataset_name_incl = params$dataset_name_incl)
...
targets.colour <- getDatasetsColours(target$Target)
...
if ( params$filter ) {
  ...
  if ( !is.null(add_cancer_group) ) {
    legend <- c(ext_cancer_group, int_cancer_group, add_cancer_group, "Patient")
  } else {
...
for ( dataset in names(ref_dataset.list) ) {
  ...
  if ( params$batch_rm ) {
    ref_dataset.list[[dataset]][["pca_combined_data_processed"]] <- pca(data = ref_dataset.list[[dataset]][["combined_data_processed"]], target = target, title = "Before batch-effects correction", plot_mode = params$plots_mode)
    
    ref_dataset.list[[dataset]][["pca_batch_effect_corrected"]] <- pca(data = ref_dataset.list[[dataset]][["batch_effect_corrected"]], target = target, title = "After batch-effects correction",  plot_mode = params$plots_mode)
    
    ref_dataset.list[[dataset]][["data_to_report"]] <- ref_dataset.list[[dataset]][["batch_effect_corrected"]]
    
  } else {
    ref_dataset.list[[dataset]][["pca_combined_data_processed"]] <- pca(data = ref_dataset.list[[dataset]][["combined_data_processed"]], target = target, plot_mode = params$plots_mode)
...
for ( dataset in names(ref_dataset.list) ) {
  ...
  write.table(prepare2write(ref_dataset.list[[dataset]][["data_to_report"]]), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.combined_data_processed.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(rownames(ref_dataset.list[[dataset]][["data_to_report"]])), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.combined_data_processed.genes.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(ref_dataset.list[[dataset]][["sample_annot"]]), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.sample_annot.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
}
...
##### Limit the data to include only cancer genes
cn_data <- cn_data[ cn_data$Gene %in% rownames(ref_genes.list[["genes_cancer"]]), ]
...
for( i in 1:genes_no ) {
  if ( genes[i] %in% rownames(data) ) {
    output[[length(output) + 1]] <- cdfPlot(gene = genes[i], data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = TRUE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
  }
}
...
for( i in 1:genes_no ) {
  ...
  fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
    ...
    fusion_reads.selected <- fusion_reads.selected %>% 
      transform(transcripts.list = strsplit(as.character(transcripts.list), ";")) %>%
      unnest(transcripts.list) %>%
      dplyr::select(transcripts.list)
    
    output_transcripts[[length(output_transcripts) + 1]] <- DT::datatable( data = fusion_reads.selected, filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 400, height = 260, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE) %>%
      DT::formatStyle( columns = names(fusion_reads.selected), `font-size` = '12px', 'text-align' = 'center' )
  ...
  if ( geneA %in% rownames(data) && geneB %in% rownames(data) ) {
    ...
    ##### Gene A
    output_A[[length(output_A) + 1]] <- cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
    
    ##### Gene B
    output_B[[length(output_B) + 1]] <- cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
    
    ##### Generate expression summary tables
    genes <- c(geneA, geneB)
    
    ##### Z-scores
    output_table_Z[[length(output_table_Z) + 1]] <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Germline") ], fusion_genes = unique(known_translocations$geneA, known_translocations$geneB ), ext_links = TRUE, type = "z", scaling = scaling)[[1]]
    
    ##### Percentiles
    output_table_perc[[length(output_table_perc) + 1]] <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Germline") ], fusion_genes = unique(known_translocations$geneA, known_translocations$geneB ), ext_links = TRUE, type = "perc", scaling = scaling)[[1]]
  }
}
```
...
for( i in 1:genes_no ) {
  ...
  if ( file.exists(paste0(dataDir, "/clinker/", geneA, "_", geneB, ".pdf")) ) {
    ...
    #pdf(paste0(paste(dataDir, "clinker", paste0(geneA, "-", geneB, ".pdf"), sep="/")) , useDingbats=FALSE)
  ...
  cat(renderTags(output_transcripts[[i]])$html)
  cat("</details>\n")
  cat("\n***\n")
  ...
  if ( geneA %in% rownames(data) && geneB %in% rownames(data) ) {
    cat(renderTags(output_A[[i]])$html)
    cat(renderTags(output_B[[i]])$html)
    
    cat("\n###### Z-scores\n")
    cat(renderTags(output_table_Z[[i]])$html)
    cat("\n<details>")
    cat("<summary>Table legend</summary>")
    cat("<font size=\"2\">")
...
write.table(prepare2write(clinker_fusions), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.clinker_fusions.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE, append = FALSE )
...
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling, report_dir = params$report_dir)
...
##### Consider only SVs with known genes
...
Section overlaying the mRNA expression data for **[Cancer genes]** with per-gene somatic copy-number (CN) data (from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}) and mutation status, if available.
...
if ( runPcgrChunk && length(cn_genes) > 0 ) {
  ...
  mutCNexprPlot(data = data, alt_data = TRUE, cn_bottom = cn_bottom, cn_top = cn_top, comp_cancer = comp_cancer_group, type = "z", plot_mode = params$plots_mode, report_dir = params$report_dir)
  
} else if ( length(cn_genes) > 0) {
  data <- data[ data$Gene %in% cn_genes, ]
  mutCNexprPlot(data = data, alt_data = FALSE, cn_bottom = cn_bottom, cn_top = cn_top, comp_cancer = comp_cancer_group, type = "z", plot_mode = params$plots_mode, report_dir = params$report_dir)
...
if ( runPcgrChunk && length(cn_genes) > 0 ) {
  mutCNexprPlot(data = data, alt_data = TRUE, cn_bottom = cn_bottom, cn_top = cn_top, comp_cancer = comp_cancer_group, type = "perc", plot_mode = params$plots_mode, report_dir = params$report_dir)
  
} else if ( length(cn_genes) > 0) {
  data <- data[ data$Gene %in% cn_genes, ]
  mutCNexprPlot(data = data, alt_data = FALSE, cn_bottom = cn_bottom, cn_top = cn_top, comp_cancer = comp_cancer_group, type = "perc", plot_mode = params$plots_mode, report_dir = params$report_dir)
...
Out of the `r length(ref_dataset.list[[dataset]][["expr_mut_cn_data"]][,1])` genes within gained `r if ( runPurpleChunk ) { paste0("(CN values > ", cn_top, ")") }` or lost `r if ( runPurpleChunk ) { paste0("(CN values < ", cn_bottom, ")") }` regions `r if (runPurpleChunk) { length(cn_genes) } else { length(NULL) }` are [Cancer genes]. The expression of **`r if (runPurpleChunk) { length(which(cn_genes %in% rownames(ref_dataset.list[[dataset]][["data_to_report"]]))) } else { length(NULL) }`** of these genes was reliably measured in patient's sample. The remaining `r if (runPurpleChunk) { length(which(cn_genes %!in% rownames(ref_dataset.list[[dataset]][["data_to_report"]]))) } else { length(NULL) }` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "immune_genes", type = "z", sort = "alphabetically", scaling = scaling, report_dir = params$report_dir)
} else {
...
##### Generate overview boxplot
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "immune_genes", type = "perc", sort = "alphabetically", scaling = scaling, report_dir = params$report_dir)
} else {
  cat("No expression data is available for immune markers!")
}
...
##### Generate overview boxplot
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "hrd_genes", type = "z", sort = "alphabetically", scaling = scaling, report_dir = params$report_dir)
} else {
...
##### Generate overview boxplot
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "hrd_genes", type = "perc", sort = "alphabetically", scaling = scaling, report_dir = params$report_dir)
} else {
  cat("No expression data is available for HRD genes!")
}
...
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "cancer_genes", type = "z", sort = "none", scaling = scaling, report_dir = params$report_dir)
} else {
  cat("No expression data is available for cancer genes!")
}
...
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, plot_mode = params$plots_mode, hexcode = "cancer_genes", type = "perc", sort = "none", scaling = scaling, report_dir = params$report_dir)
} else {
  cat("No expression data is available for cancer genes!")
}
...
**`r length(mut_genes.expr.z[[2]]$SYMBOL)`** genes with [PCGR](https://github.com/sigven/pcgr){target="_blank"} [tier](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} 1-`r params$pcgr_tier` variants were screened for suitable drugs (see [Mutated genes] section).
