...
exprTable <- function(genes, data, targets, sampleName, normal, cancer, genes_annot = NULL, OncoKB_genes = NULL, tiers = NULL) {
  ...
  if( !is.null(tiers) ){
    ...
    ##### Order the data by increasing TIER category and then by the highest absolute values for Patient vs Normal difference
    group.z <- group.z[ order(abs(group.z[, "Patient vs Normal"]),  decreasing = TRUE), ]
...
ref_genes.list[["pcgr"]] <- ref_genes.list[["pcgr"]][ ref_genes.list[["pcgr"]]$TIER %in% c("TIER 1", "TIER 2", "TIER 3" ), ]
...
mut_genes.expr <- exprTable( genes = unique(ref_genes.list[["pcgr"]]$SYMBOL), data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][[5]][, c("SYMBOL", "ENSEMBL")], tiers = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER")])
...
cancer_genes.expr <- exprTable( genes = rownames(ref_genes.list[["oncokb_genes"]]), data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][[5]][, c(1,3)], OncoKB_genes = ref_genes.list[["oncokb_genes"]])
...
exprTable( genes = fusion.df$SYMBOL, data = data, targets = target, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][[5]][, c(1,3)], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c(1,2,4)])[[1]]
```
...
immune_genes.expr <- exprTable( genes = unlist(ref_genes.list[["genes_immune"]]), data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][[5]][, c(1,3)])
