...
for ( ref in names(ref_datasets.list) ) {
  ...
  cat(nrow(y.filtered$counts), "genes remained after annotating with gene symbols.\n\n")
}
```
...
mRNA expression levels of fusion genes in sample **`r params$sample_name`** and average mRNA expression in samples from cancer patients and healthy individuals.
...
mRNA expression levels of genes included in the Peter Mac comprehensive cancer (PMCC) panel in sample **`r params$sample_name`** and average mRNA expression in samples from cancer patients and healthy individuals.
