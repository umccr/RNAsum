...
Out of the `r nrow(y$counts)` input genes:

* **`r nrow(data.annot)` genes** have reliably detected expression
* `r if (params$filter) { nrow(y$counts) - nrow(y$filtered$counts) } else { ("0") }` are either not expressed or their expression level is too low to be detected
* `r if (params$filter) { nrow(y$filtered$counts) - nrow(data.annot) } else { nrow(y$counts) - nrow(data.annot) }` genes were ignored due to lack of [HGNC](https://www.genenames.org/){target="_blank"}-approved gene symbol

NOTE: the `r if (params$filter) { nrow(y$counts) - nrow(y$filtered$counts) } else { ("0") }` genes with no/low expression are indicated in <span style="color:#808080">BLANK</span> cells with missing values in *expression summary tables* in [Mutated genes], [Cancer genes], [Structural variants], [CN altered genes] and [Immune markers] sections.
