...
for ( group in targets_mod.list ) {
  ...
  if ( params$filter && params$transform == "CPM" ) {
    ...
    filter.min <- min(filter.min, round(min(as.numeric(colSums(data)*1e-6)), digits=0))
    filter.max <- max(filter.max, round(max(as.numeric(colSums(data)*1e-6)), digits=0))
    #cat("The CPM of 1 (cut-off for removing low expressed genes) corresponds to",filter.min, "reads in sample with the lowest sequencing depth, and", filter.max, "reads in sample with the greatest sequencing depth\n")
  ...
  ##### TPM transformation with filtering
    ...
    if ( params$log ) {
      ...
      keep <- rowSums(y[[group]]$transformed>(1+0.25)) >= ncol(y[[group]]$transformed)/10
      ...
      ##### Remove genes with TPM of at least 1 in less than 10% of samples
      keep <- rowSums(y[[group]]$transformed>1) >= ncol(y[[group]]$transformed)/10
...
plot(exp(y[["comb"]]$transformed[,ncol(y[["comb"]]$transformed)]),ref_datasets.list[[dataset]][["combined_data"]][,ncol(ref_datasets.list[[dataset]][["combined_data"]])])

##### Limit the x and y-axis so we can see what is happening at the smaller counts
plot(exp(y[["comb"]]$transformed[,ncol(y[["comb"]]$transformed)]),ref_datasets.list[[dataset]][["combined_data"]][,ncol(ref_datasets.list[[dataset]][["combined_data"]])], ylim=c(0,50),xlim=c(0,3))
