...
params:
  ...
  dataset: 'TEST'
  bcbio_rnaseq: NULL
  dragen_rnaseq: '../data/test_data/stratus/test_sample_WTS_dragen_v3.7.5'
  report_dir: '../data/test_data/stratus/test_sample_WTS_dragen_v3.7.5/RNAsum'
  ref_data_dir: '../data'
  transform: 'CPM'
  filter: TRUE
  norm: 'TMM'
...
if ( runArribaChunk ) {
  ...
  #####  Dragen's fusion format version 3.7.5
  } else if ( "Score" %in% colnames(dragen.fusions) ) {
...
##### Otherwise add empty columns expected from Aribba results
  ...
  fusions$split_readsA <-  c(rep("-", nrow(fusions)))
...
if ( runArribaChunk && runDragenFusionChunk ) {
  ...
  fusion_annot <- cbind(fusion_annot1, fusion_annot2, fusions.annot[, c("reported_fusion", "fusions_cancer", "reported_fusion_geneA", "reported_fusion_geneB")])
}
...
if ( runFusionChunk ) {
  ...
  } else if ( runDragenFusionChunk && runArribaChunk == FALSE) {
    fusions.table <- fusions.table[ , names(fusions.table) %!in% "FGID" ]
    fusions.table <- fusions.table[ , c("geneA", "geneB", "geneA_dna_support", "geneB_dna_support", "reported_fusion", "fusions_cancer", "reported_fusion_geneA", "reported_fusion_geneB", "confidence", "Score", "breakpointA", "breakpointB", "siteA", "siteB", "type", "circos")]
  names(fusions.table) <- c("Gene A", "Gene B", "DNA support (A)", "DNA support (B)", "Reported fusion", "Cancer gene(s)", "Fusion gene (A)", "Fusion gene (B)", "Confidence (Arriba)", "Score (Dragen)", "Breakpoint (A)", "Breakpoint (B)", "Site (A)", "Site (B)", "Type", "Genomic view")
...
if ( nrow(fusion_annot_top) > 0 ) {
  ...
  if ( runDragenFusionChunk && runArribaChunk) {
    ...
    fusion_annot_top.clean <- fusion_annot_top[, c("SYMBOL", "SEQNAME", "GENESEQSTART", "GENESEQEND", "SYMBOL.1", "SEQNAME.1", "GENESEQSTART.1", "GENESEQEND.1", "geneA_dna_support", "geneB_dna_support", "reported_fusion", "fusions_cancer") ]
  }
  ...
  if ( runDragenFusionChunk && runArribaChunk) {
    ...
    names(fusion_annot_top.clean) <- c("Gene A", "Chrom (A)", "Start (A)", "End (A)", "Gene B", "Chrom (B)", "Start (B)", "End (B)", "DNA support (A)", "DNA support (B)", "Reported fusion", "Cancer gene(s)")
  }
