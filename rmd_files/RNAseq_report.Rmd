...
Table with of ***`r geneA`*** and ***`r geneB`*** annotation and mRNA expression levels in sample **`r params$sample_name`** and average mRNA expression across samples representing different pancreas-related lesions and tissues.
...
kable(fusion.df, row.names = FALSE) %>%
  kable_styling(font_size = 10, "striped", "bordered")
```
...
Empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level of ***`r pizzly.fusions2$geneA.name[4]`*** and ***`r pizzly.fusions2$geneB.name[4]`*** in the context of the overall mRNA expression distribution in sample **`r params$sample_name`** and samples representing various pancreas-related lesions/tissues.
...
kable(ref_datasets.list[[ref]][[5]][gene, ], row.names = FALSE) %>%
  kable_styling(font_size = 10, "striped", "bordered")
```
