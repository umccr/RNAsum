...
arriba_plots <- function(arriba_file, arriba_results, results_dir) {
  ...
  for ( i in 1:nrow(arriba_results) ) {
    arriba_plots.png <- gsub(":", ".", paste0(results_dir, "/", paste0(paste(arriba_results$X.gene1[i], arriba_results$gene2[i], sep = "__"), "_", arriba_results$breakpoint1[i], "-", arriba_results$breakpoint2[i]), ".png"))
...
if ( file.exists(arriba_file) ) {
  ...
  fusion <- gsub(":", ".", c("", paste0(paste(ref_genes.list[["arriba"]]$X.gene1, ref_genes.list[["arriba"]]$gene2, sep = "__"), "_", ref_genes.list[["arriba"]]$breakpoint1, "-", ref_genes.list[["arriba"]]$breakpoint2)))
  
  write.table(prepare2write(fusion), file = paste0(params$report_dir, "/results/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.arriba_fusions.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE, append = FALSE )
...
for ( dataset in names(ref_dataset.list) ) {
  ...
  write.table(prepare2write(toupper(rownames(ref_dataset.list[[dataset]][["data_to_report"]]))), file = paste0(params$report_dir, "/results/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.combined_data_processed.genes.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
