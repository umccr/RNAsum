...
exprGroupsStats_sampleWise <- function(data, targets) {
  ...
  ##### Perform mean values for each group
  ...
  for ( group in targets.list ) {
    ...
    if ( sum(c(targets$Target %in% group), na.rm = TRUE) > 1 && nrow(data) > 1 )  {
      ...
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], rowMeans(data.cum[ , colnames(data)[ targets$Target %in% group ] ]))
      ...
      ##### Extract the mean cumulative fraction
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], mean(data.cum[ , colnames(data)[ targets$Target %in% group, drop = FALSE ] ]))
...
cdfPlot <- function(gene, data, targets, sampleName, int_cancer, ext_cancer, addBoxPlot = FALSE, plot_mode = "static", scaling = "sample-wise") {
  ...
  if ( scaling == "sample-wise" ) {
    ...
    for ( group in targets.list ) {
        ...
        group.z[[ sampleName]] <- rbind(group.z[[ sampleName]], cbind(t(gene.stats[[ group]]$mean), t(gene.stats[[ group]]$z), t(gene.stats[[ group]]$q), t(gene.stats[[ group]]$cum) ))
        
        group.z[[ group]] <- group.z[[ sampleName]]
