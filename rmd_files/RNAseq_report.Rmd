...
exprTable <- function(genes, data, targets, sampleName, normal, cancer, genes_annot = NULL, OncoKB_genes = NULL, tiers = NULL, expr_type = "z") {
  ...
  for ( group in targets.list ) {
    ...
    if ( expr_type == "perc" ) {
      ...
      group.z[, group] <- round(group.stats$quantile, digits=2)
      expr_type <- "(percentile)"
      
    } else {
      
      group.z[, group] <- round(group.stats$z, digits=1)
      expr_type <- "(z-score)"
    }
  } 
