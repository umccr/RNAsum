...
# 3. ref_datasets.list[[tissue]][["clinical_info"]] = clinical information (survival and treatment info)

# 4. ref_datasets.list[[tissue]][["combined_data_processed"]] = transformed, filtered and normalised data (see "data_transformation" and "data_normalisation" chunks)

# 5. ref_datasets.list[[tissue]][["pca"]] = PCA results

# 6. ref_datasets.list[[tissue]][["gene_annot_all"]] = gene annotation for combined read count data, containing all input genes. The annotation includes "SYMBOL", "GENEBIOTYPE", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND". "ENSEMBL" is used for rownames

# 7. ref_datasets.list[[tissue]][["gene_annot"]] = gene annotation for transformed, filtered and normalised data. The annotation includes "SYMBOL", "GENEBIOTYPE", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND". "SYMBOL" is used for rownames

# 8. ref_datasets.list[[tissue]][["expr_mut_cn_data"]] = combined expression, mutation and copy-number data
...
combineDatasets <- function(sample_name, sample_counts, ref_dataset) {
  ...
  if ( length(gene_list.missing) > 0 ) {
    write.table(prepare2write(gene_list.missing), file = paste0(params$report_dir, "/", sample_name, ".", params$tissue, ".missing_genes.txt"), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
  }
...
ref_tissue <- list( "pancreas" = c(paste(params$ref_data_dir, "Datasets_list_PDAC.txt", sep="/"), "PDAC (UMCCR)", "PDAC (TCGA)"), "cervix" = c(paste(params$ref_data_dir, "Datasets_list_cervical_SCC.txt", sep="/"), "Cervical squamous cell carcinoma", NULL) )

tissue <- params$tissue
int_cancer_group <- ref_tissue[[tissue]][2]
ext_cancer_group <- ref_tissue[[tissue]][3]
...
if ( file.exists(clinical_info_file) ) {
  ...
  ref_datasets.list[[tissue]][["clinical_info"]] <- read.xlsx(xlsxFile = clinical_info_file, sheet = 1, colNames = TRUE, rowNames = FALSE, detectDates = TRUE, skipEmptyRows = TRUE, skipEmptyCols = TRUE, check.names = TRUE)
...
sampleID.col <- grep(params$subject_id, ref_datasets.list[[tissue]][["clinical_info"]])

if ( length(sampleID.col) > 0 ) {
  
  ##### Identify column and row with patients details
  sampleID.row <- grep(params$subject_id, ref_datasets.list[[tissue]][["clinical_info"]][, sampleID.col])

  clinical_info <- ref_datasets.list[[tissue]][["clinical_info"]][ sampleID.row, ]
...
ref_datasets.list[[tissue]][["combined_data_processed"]] <- y$norm
```
...
for ( tissue in names(ref_datasets.list) ) {
  ...
  ref_datasets.list[[tissue]][["pca"]] <- pca(data, target, plot_mode = params$plots_mode)
}
...
for ( tissue in names(ref_datasets.list) ) {
  ...
  write.table(prepare2write(ref_datasets.list[[tissue]][["combined_data_processed"]]), file = paste0(params$report_dir, "/", params$sample_name, ".", params$tissue, ".combined_data_processed.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(rownames(ref_datasets.list[[tissue]][["combined_data_processed"]])), file = paste0(params$report_dir, "/", params$sample_name, ".", params$tissue,  ".combined_data_processed.genes.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(ref_datasets.list[[tissue]][["sample_annot"]]), file = paste0(params$report_dir, "/", params$sample_name, ".", params$tissue, ".sample_annot.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
}
```
...
Out of the `r nrow(ref_datasets.list[[tissue]][["combined_data"]])` input genes:

* **`r nrow(data.annot)` genes** have reliably detected expression
* `r if (params$filter) { nrow(ref_datasets.list[[tissue]][["combined_data"]]) - nrow(ref_datasets.list[[tissue]][["combined_data_processed"]]) } else { ("0") }` are either not expressed or their expression level is too low to be detected
* `r if (params$filter) { nrow(ref_datasets.list[[tissue]][["combined_data_processed"]]) - nrow(data.annot) } else { nrow(ref_datasets.list[[tissue]][["combined_data"]]) - nrow(data.annot) }` genes were ignored due to lack of [HGNC](https://www.genenames.org/){target="_blank"}-approved gene symbol

NOTE: the `r if (params$filter) { nrow(ref_datasets.list[[tissue]][["combined_data"]]) - nrow(ref_datasets.list[[tissue]][["combined_data_processed"]]) } else { ("0") }` genes with no/low expression are indicated in <span style="color:#808080">BLANK</span> cells with missing values in *expression summary tables* in [Mutated genes], [Cancer genes], [Structural variants], [CN altered genes] and [Immune markers] sections.
...
ref_datasets.list[[tissue]][["pca"]][[2]]
```
...
ref_datasets.list[[tissue]][["pca"]][[3]]
```
...
mut_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
...
if ( !is.na(gene) && gene %in% rownames(data) ) {
  ...
  } else if ( gene %in% ref_datasets.list[[tissue]][["gene_annot_all"]]$SYMBOL ) {
...
write.table(prepare2write(clinker_fusions), file = paste0(params$report_dir, "/", params$sample_name, ".", params$tissue, ".clinker_fusions.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
```
...
genes <- unique(manta_sv$Gene)
genes <- genes[ genes %in% ref_datasets.list[[tissue]][["gene_annot_all"]]$SYMBOL ]
...
immune_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL", "Immune_Cycle_Role")], ext_links = TRUE, type = "z", scaling = scaling)
...
##### Generate expression summary table for cancer genes from OncoKB and PMCC
immune_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL", "Immune_Cycle_Role")], ext_links = TRUE, type = "perc", scaling = scaling)
...
hrd_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], ext_links = TRUE, type = "z", scaling = scaling)
...
hrd_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], ext_links = TRUE, type = "perc", scaling = scaling)
