...
if ( !is.null(params$umccrise) ) {
  ...
  if ( runPcgrChunk ) {
    ...
    mysql_populate <- paste0(mysql_populate, ", \"", subjectID, "\", \"", params$sample_name, "\", \"", params$dataset , "\", \"", params$sample_source , "\", \"", params$project , "\", \"", paste0(params$sample_name, params$dataset_name_incl, ".RNAseq_report.html"), "\", \"", params$sample_name, "\", \""  )
    mysql_populate_update <-  paste0(mysql_populate_update, ", PatientID=\"", subjectID, "\", SampleID=\"", params$sample_name, "\", Cancer=\"", params$dataset , "\", Source=\"", params$sample_source , "\", Project=\"", params$project , "\", Report=\"", paste0(params$sample_name, params$dataset_name_incl, ".RNAseq_report.html"),"\", PMID=\"", params$sample_name, "\", Analysis=\""  )
    
  } else {
    ref_genes.list[["pcgr"]] <- NULL
    
    ##### Update MySQL commend to populate RNA-seq data portal
    mysql_populate <- paste0(mysql_populate, ", \"\", \"", params$sample_name, "\", \"", params$dataset , "\", \"", params$sample_source , "\", \"", params$project , "\", \"", paste0(params$sample_name, params$dataset_name_incl, ".RNAseq_report.html"), "\", \"", params$sample_name, "\", \""  )
    mysql_populate_update <-  paste0(mysql_populate_update, ", PatientID=\"\", SampleID=\"", params$sample_name, "\", Cancer=\"", params$dataset , "\", Source=\"", params$sample_source , "\", Project=\"", params$project , "\", Report=\"", paste0(params$sample_name, params$dataset_name_incl, ".RNAseq_report.html"),"\", PMID=\"", params$sample_name, "\", Analysis=\""  )
  }
...
##### Otherwise add empty columns expected from Aribba results
  ...
  fusions$geneA_dna_support <- c(rep("-", nrow(fusions)))
  fusions$geneB_dna_support <- c(rep("-", nrow(fusions)))
...
if ( !is.na(sunburst_plot[[2]]) ) {
  ...
  sunburst_plot[[2]] <- as.data.frame(c("No genes are listed in more then one section. Expand the \"All altered genes\" to view the summary plot for all altered genes."))
  #rownames(sunburst_plot[[2]]) <- ""
  #colnames(sunburst_plot[[2]]) <- ""
}
...
for( i in 1:genes_no ) {
  ...
  if ( geneB %in% rownames(data) ) {
    ...
    cat(paste0("\nExpression data is not available for ", geneB, ".\n"))
  }
