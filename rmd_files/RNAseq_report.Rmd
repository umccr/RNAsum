...
colnames(pizzly.fusions)[-c(1:7)] <- c("FGID", "reported_fusion", "reported_fusion_genes", "effector_gene")
...
for ( i in 1:nrow(pizzly.fusions) ) {
  ...
  if ( paste(geneA, geneB, sep="-") %in% trans.pairs ) {
    ...
    pizzly.fusions$reported_fusion_genes[i] <- paste(geneA, geneB, sep = "  ")
  ...
  ##### Now check if any ofthe pizzly detected fusion genes are reported
    ...
    if ( geneA %in% known_translocations$geneA ) {
      ...
      pizzly.fusions$reported_fusion_genes[i] <- paste(pizzly.fusions$reported_fusion_genes[i], geneA, sep = "  ")
    ...
    if ( geneB %in% known_translocations$geneA ) {
      ...
      pizzly.fusions$reported_fusion_genes[i] <- paste(pizzly.fusions$reported_fusion_genes[i], geneB, sep = " ")
    ...
    if ( geneA %in% known_translocations$effector_gene  ) {
      ...
      pizzly.fusions$effector_gene[i] <- paste(pizzly.fusions$effector_gene[i], geneA, sep = " ")
      
    } else if ( geneB == known_translocations$effector_gene  ) {
          
        pizzly.fusions$effector_gene[i] <- paste(pizzly.fusions$effector_gene[i], geneB, sep = " ")
    }
  }
}
...
pizzly.fusions <- pizzly.fusions[ order(pizzly.fusions$reported_fusion, decreasing = TRUE), ]
```
...
fusion_annot <- cbind(fusion_annot1, fusion_annot2, pizzly.fusions.annot[, c("reported_fusion", "fusions_abundant", "fusions_cancer")])
...
pizzly.fusions.cut <- pizzly.fusions.cut[ , c("geneA.name", "geneB.name", "paircount", "splitcount", "reported_fusion", "FGID", "geneA_dna_support", "geneB_dna_support", "fusions_abundant", "fusions_cancer", "reported_fusion_genes") ]
```
...
for ( i in 1:nrow(pizzly.fusions.table) ) {
    ...
    if ( pizzly.fusions.table$reported_fusion[i] == "Yes" ) {
      ...
      pizzly.fusions.table$geneA.name[i] <- paste0("<a href='<a href='https://ccsm.uth.edu/FusionGDB/gene_search_result.cgi?page=page&type=quick_search&quick_search=", pizzly.fusions.table$FGID[i], "' target='_blank'>", pizzly.fusions.table$geneA.name[i], "</a>")

      pizzly.fusions.table$geneB.name[i] <- paste0("<a href='<a href='https://ccsm.uth.edu/FusionGDB/gene_search_result.cgi?page=page&type=quick_search&quick_search=", pizzly.fusions.table$FGID[i], "' target='_blank'>", pizzly.fusions.table$geneB.name[i], "</a>")
    }
}

pizzly.fusions.table <- pizzly.fusions.table[ , names(pizzly.fusions.table) %!in% "FGID" ]

names(pizzly.fusions.table) <- c("Gene A", "Gene B", "Pair count", "Split count", "Reported fusion", "DNA support (gene A)", "DNA support (gene B)", "Abundant transcript(s)", "Cancer gene(s)", "Fusion gene(s)")
