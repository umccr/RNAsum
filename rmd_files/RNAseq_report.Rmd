...
params:
  ...
  oncokb_genes: '../data/CancerGenesList.txt'
  oncokb_clin_vars: '../data/allActionableVariants.txt'
  oncokb_all_vars: '../data/allAnnotatedVariants.txt'
---
...
cdfPlot <- function(gene, data, targets, sampleName, normal, cancer) {
  ...
  p <- plot_ly(sample.expr.cum, x = ~z, color = I("black")) %>%
    ...
    ##### Prepare data frame
    #gene.expr.df <- data.frame(targets$Target, data[gene, ])
    ...
    #p2 <- plot_ly(gene.expr.df, y= ~Expression, color = ~Group, type = 'box', jitter = 0.3, pointpos = 0, boxpoints = 'all', colors = targets.colour[[1]], width = 800, height = 600) %>%
    ...
    layout(xaxis = list(title = "mRNA expression (Z-score)", zeroline = FALSE, range = c(min(sample.expr.cum$z)-1, max(sample.expr.cum$z)+1)),
...
exprTable <- function(genes, data, targets, sampleName, normal, genes_annot = NULL, cancer_genes_resources = NULL) {
  ...
  dt.table <- DT::datatable( data = group.z, filter="none", rownames = FALSE, options = list(pageLength = 10), caption = htmltools::tags$caption(
          style = 'caption-side: top; text-align: left; color:grey; 
          font-size:100% ;', "Standardised (Z-score) mRNA expression levels in individual groups.")) %>%
...
ref_genes <- c("genes_fusions","genes_pmcc", "oncokb_genes")
...
The following refenece patient cohorts were used for the analysis:
...
### Prioritisation of gene fusion events
...
kable(pizzly.fusions2) %>%
  ...
  group_rows("Filtered results using cancer genes list", (nrow(deduped.result)+1), (nrow(deduped.result) + nrow(result.pmcc)), label_row_css = "background-color: #2980B9; color: #fff;") %>%
...
### Fusion events visualization {.tabset}
...
mRNA expression levels of fusion genes in patient sample **`r params$sample_name`** and their average mRNA expression in samples from cancer patients and healthy individuals.
...
exprTable(fusion.df$SYMBOL, data, target, params$sample_name, params$normal_group, ref_datasets.list[[ref]][[5]][, c(1,3)])[[1]]
```
...
cancer_genes.expr <- exprTable(rownames(ref_genes.list[["cancer_genes"]]), data, target, params$sample_name, params$normal_group, ref_datasets.list[[ref]][[5]][, c(1,3)], ref_genes.list[["cancer_genes"]])
...
NOTE: currently, displaying the top 6 cancer genes with the greatest difference in standarised (Z-score) mRNA expression between patient sample **`r params$sample_name`** and the average mRNA expression in samples from normal individuals.
...
cdfPlot(gene, data, target, params$sample_name, params$normal_group, params$cancer_group)
