...
params:
  ...
  subject_id: ''
  sample_source: "-"
  sample_name_mysql: 'test_sample_WTS'
  dataset_name_incl: ''
  project: "-"
  top_genes: 5
  save_tables: TRUE
...
combineDatasets <- function(sample_name, sample_counts, ref_data, report_dir, dataset, dataset_name_incl = FALSE) {
  ...
  if ( length(gene_list.missing) > 0 ) {
    if ( isFALSE(dataset_name_incl) || dataset_name_incl == "" ) {
      ...
      write.table(prepare2write(gene_list.missing), file = paste0(report_dir, "/", sample_name, "_",  dataset, ".RNAseq_report.missing_genes.txt"), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
    }
  }
...
mysql_populate <- paste0(mysql_populate, ", \"", subjectID, "\", \"", params$sample_name_mysql, "\", \"", params$dataset , "\", \"", params$sample_source , "\", \"", params$project , "\", \"", paste0(params$sample_name, params$dataset_name_incl, ".RNAseq_report.html"), "\", \"", params$sample_name_mysql, "\", \""  )
mysql_populate_update <-  paste0(mysql_populate_update, ", PatientID=\"", subjectID, "\", SampleID=\"", params$sample_name_mysql, "\", Cancer=\"", params$dataset , "\", Source=\"", params$sample_source , "\", Project=\"", params$project , "\", Report=\"", paste0(params$sample_name, params$dataset_name_incl, ".RNAseq_report.html"),"\", PMID=\"", params$sample_name_mysql, "\", Analysis=\""  )
...
mysql_populate <- paste0(mysql_populate, "\", \"Transcriptome summary for sample ", params$sample_name_mysql, " generated on ", Sys.Date(), "\"", ", \"", Sys.Date(), "\" )")
mysql_populate_update <- paste0(mysql_populate_update, "\", Summary=\"Transcriptome summary for sample ", params$sample_name_mysql, " generated on ", Sys.Date(), "\"", ", Date=\"", Sys.Date(), "\";")
mysql_populate <- paste0(mysql_populate, "\n  ", mysql_populate_update, "\nSET @ID := 0;\nUPDATE RNAseq_reports SET ID = ( SELECT @ID := @ID + 1 );")
writeLines(mysql_populate, con = paste0(params$report_dir, "/results/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.sql"))
```
