...
### Copy-number variation {.tabset}

#### Plot

<span style="color:#ff0000">NOTE</span>: this section is in progress. The aim of this section is to overlay the per-gene somatic copy-number variants (CNVs) with mRNA expression data and, where available, mutation status. The scatterplot compares the per-gene *mRNA expression* values (*y-axis*, Z-score) with *CN values* (*x-axis*, from [PURPLE](https://anaconda.org/bioconda/hmftools-purple)). Genes's colours correspond to their mutation status if available  (from [PCGR](https://github.com/sigven/pcgr)).
