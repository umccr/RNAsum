...
# 5. ref_datasets.list[[tissue]][["gene_annot"]] = transformed, filtered and normalised data annotated with "SYMBOL", "GENEBIOTYPE", "ENSEMBL", "ENTREZID", "SEQNAME", "GENESEQSTART", "GENESEQEND"

# 6. ref_datasets.list[[tissue]][["expr_mut_cn_data"]] = combined expression, mutation and copy-number data
...
mut_genes.expr <- exprTable( genes = unique(ref_genes.list[["pcgr"]]$SYMBOL), data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c("SYMBOL", "ENSEMBL")], tiers = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER")])
...
plot_ly(ref_datasets.list[[tissue]][["expr_mut_cn_data"]], x = ~CN, y = ~mRNA, color = ~Mutation, text=~Gene, type='scatter', mode = "markers", marker = list(size=10, symbol="circle"), width = 800, height = 600) %>%
...
From the `r nrow(ref_genes.list[["oncokb_genes"]])` cancer genes the expression of **`r length(which(rownames(ref_genes.list[["oncokb_genes"]]) %in% rownames(ref_datasets.list[[tissue]][[3]])))`** was reliably measured in patient's sample.
...
cancer_genes.expr <- exprTable( genes = rownames(ref_genes.list[["oncokb_genes"]]), data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c(1,3)], OncoKB_genes = ref_genes.list[["oncokb_genes"]])
...
exprTable( genes = fusion.df$SYMBOL, data = data, targets = target, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c(1,3)], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c(1,2,4)])[[1]]
```
...
fuiosnExprProfile <- exprProfilePlot(c(as.vector(geneA), as.vector(geneB)), data, target, params$sample_name, normal_group, cancer_group, ref_datasets.list[[tissue]][["gene_annot"]])
...
immune_genes.expr <- exprTable( genes = unlist(ref_genes.list[["genes_immune"]]), data = data, targets = target, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot"]][, c(1,3)])
