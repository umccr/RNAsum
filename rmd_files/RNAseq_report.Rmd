...
pca <- function(data, targets, plot_mode) {
...
cdfPlot <- function(gene, data, targets, sampleName, int_cancer, ext_cancer, addBoxPlot = FALSE, plot_mode = "static") {
  
  ##### Get expression-related stats for each group
  sample.expr.cum <- exprGroupStats(data, targets, sampleName)
  int_cancer.expr.cum <- exprGroupStats(data, targets, int_cancer)
  ext_cancer.expr.cum <- exprGroupStats(data, targets, ext_cancer)
  
  ##### Extract expression for selected genes
  sample.expr.cum.selected <- sample.expr.cum[ rownames(sample.expr.cum) %in% gene, ]
  int_cancer.expr.cum.selected <- int_cancer.expr.cum[ rownames(int_cancer.expr.cum) %in% gene, ]
  ext_cancer.expr.cum.selected <- ext_cancer.expr.cum[ rownames(ext_cancer.expr.cum) %in% gene, ]
  ...
  if ( addBoxPlot ) {
    ...
    p2 <- plot_ly(gene.expr.df, x= ~Expression, color = ~Group, type = 'box', jitter = 0.3, pointpos = 0, boxpoints = 'all', colors = c("powderblue", "red", "black"), opacity = 0.5, orientation = 'h', width = 800, height = 600, showlegend=FALSE)
  }
  ...
  p1 <- plot_ly(sample.expr.cum, x = ~z, color = I("black")) %>%
    ...
    ##### Add int_cancer data
    add_markers(y = int_cancer.expr.cum.selected$cumulative_fraction, x =  int_cancer.expr.cum.selected$z,
                text = rownames( int_cancer.expr.cum.selected),
                name = int_cancer,
                marker = list(size = 12, opacity = 0.5, color = "red"),
                showlegend = TRUE) %>%
  
    add_lines(y = int_cancer.expr.cum$cumulative_fraction, x = int_cancer.expr.cum$z, opacity = 0.5,
              line = list(color = "red", dash = "dash"),
              text = rownames( int_cancer.expr.cum ),
              name = int_cancer, showlegend = FALSE) %>%
    
    ##### Add ext_cancer data
    add_markers(y = ext_cancer.expr.cum.selected$cumulative_fraction, x =  ext_cancer.expr.cum.selected$z,
                text = rownames( ext_cancer.expr.cum.selected ),
                name = ext_cancer,
                marker = list(size = 12, opacity = 0.1, color = "cornflowerblue"),
                showlegend = TRUE) %>%
  
    add_lines(y = ext_cancer.expr.cum$cumulative_fraction, x = ext_cancer.expr.cum$z, opacity = 0.1,
              line = list(color = "cornflowerblue", dash = "dash"),
              text = rownames( ext_cancer.expr.cum ),
...
glanceExprPlot <- function(genes, data, targets, sampleName, int_cancer, ext_cancer, plot_mode = "static", hexcode) {
  ...
  p <- plot_ly( gene.expr.df, x = ~Gene, y = ~Expression, color = ~Group, type = "box", colors = c(I("black"), "red", "cornflowerblue"), opacity=0.2, showlegend = TRUE ) %>% 
...
exprTable <- function(genes, data, cn_data = NULL, sv_data = NULL, cn_decrease = TRUE, targets, sampleName, int_cancer, ext_cancer, genes_annot = NULL, cancer_genes = NULL, mut_annot = NULL, fusion_genes = NULL, ext_links = FALSE, type = "z") {
  ...
  ##### Change sample ID to "Patient" and ext_cancer sample to "Cancer (TCGA)" for better visualisation
  ...
  ##### Order the data by MANTA increasing score (to prioritise SVs, based on https://github.com/AstraZeneca-NGS/simple_sv_annotation/blob/master/simple_sv_annotation.py) and then by the highest absolute values for Patient vs [int_cancer] difference
...
if ( params$filter && params$transform == "CPM" ) {
  ...
  y <- DGEList(counts=data,  group=target$Target)
...
# htmlwidgets::saveWidget(as_widget(p), paste0(PlotsDir, "/normalisation.html"), selfcontained = TRUE)
# #######################################################
...
if ( params$plots_mode == "interactive" ) {
  ...
  genes.expr.z <- exprTable( genes = rownames(data), data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "z")
  
  genes.expr.perc <- exprTable( genes = rownames(data), data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "perc")
...
expr_data <- exprTable( genes = rownames(expr_data), data = expr_data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, type = "z")[[2]]
...
```{r fusions_reads, comment = NA, message=TRUE, warning=FALSE, eval = runKallistoChunk}
...
```{r fusions_annot, comment = NA, message=TRUE, warning=FALSE, eval = runKallistoChunk}
...
```{r pizzly_data_cleanup, comment = NA, message=FALSE, warning=FALSE, eval = runKallistoChunk}
...
if ( !is.na(gene) && gene %in% rownames(data) ) {
  ...
  cdfPlot(gene = gene, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, addBoxPlot = TRUE, plot_mode = params$plots_mode)
...
Out of the `r if ( runKallistoChunk ) { nrow(pizzly.fusions.cut) } else { c("0") }` fusion event(s) <span style="color:#ff0000">**`r if ( runKallistoChunk ) { nrow(pizzly.fusions.cut[ pizzly.fusions.cut$geneA_dna_support == "Yes" | pizzly.fusions.cut$geneB_dna_support == "Yes" , ]) } else { c("0") }`**</span> involve **DNA-supported** fusion genes (see [Structural variants] section), <span style="color:#02d653">**`r if ( runKallistoChunk ) { nrow(pizzly.fusions.cut[ pizzly.fusions.cut$reported_fusion == "Yes" , ]) } else { c("0") }`**</span> are **reported in [FusionGDB](https://ccsm.uth.edu/FusionGDB){target="_blank"}**, <span style="color:#0000ff">**`r if ( runKallistoChunk ) { nrow(pizzly.fusions.cut[ pizzly.fusions.cut$fusions_abundant == "Yes" , ]) } else { c("0") }`**</span> contain **highly abundant** fusion transcript(s) and <span style="color:#767689">**`r if ( runKallistoChunk ) { nrow(pizzly.fusions.cut[ pizzly.fusions.cut$fusions_cancer == "Yes" , ]) } else { c("0") }`**</span> involve **[Cancer genes]**.
...
```{r fusions_summary_table, comment = NA, message=FALSE, warning=FALSE, eval = runKallistoChunk}
...
```{r genomic_view_plot_fusions_save, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, eval = runKallistoChunk}
...
```{r genomic_view_table_fusions, comment = NA, message=FALSE, warning=FALSE, eval = runKallistoChunk}
...
```{r genomic_view_table_fusions_save, comment = NA, message=FALSE, warning=FALSE, eval = runKallistoChunk}
##### Save the table as html file
if ( runSVsChunk && nrow(fusion_annot_top) > 0 && params$plots_mode == "interactive" ) {
...
#### `r if ( runKallistoChunk ) { if ( !is.na(pizzly.fusions$geneA.name[1]) ) { paste(pizzly.fusions$geneA.name[1], pizzly.fusions$geneB.name[1], sep="-") }}`
...
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
...
#### `r if ( runKallistoChunk ) { if ( !is.na(pizzly.fusions$geneA.name[2]) ) { paste(pizzly.fusions$geneA.name[2], pizzly.fusions$geneB.name[2], sep="-") }}`
...
#### `r if ( runKallistoChunk ) { if ( !is.na(pizzly.fusions$geneA.name[3]) ) { paste(pizzly.fusions$geneA.name[3], pizzly.fusions$geneB.name[3], sep="-") }}`
...
#### `r if ( runKallistoChunk ) { if ( !is.na(pizzly.fusions$geneA.name[4]) ) { paste(pizzly.fusions$geneA.name[4], pizzly.fusions$geneB.name[4], sep="-") }}`
...
#### `r if ( runKallistoChunk ) { if ( !is.na(pizzly.fusions$geneA.name[5]) ) { paste(pizzly.fusions$geneA.name[5], pizzly.fusions$geneB.name[5], sep="-") }}`
...
sv_genes.expr.z <- exprTable( genes = genes, data = data, sv_data = manta_sv[ , names(manta_sv) %!in% "lof" ], targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z")
...
##### Generate expression summary table for cancer genes from OncoKB and PMCC
sv_genes.expr.perc <- exprTable( genes = genes, data = data, sv_data = manta_sv[ , names(manta_sv) %!in% "lof" ], targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc")
...
immune_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL", "Immune_Cycle_Role")], ext_links = TRUE, type = "z")
...
##### Generate expression summary table for cancer genes from OncoKB and PMCC
immune_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL", "Immune_Cycle_Role")], ext_links = TRUE, type = "perc")
...
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, plot_mode = params$plots_mode, hexcode = "immune_genes")
...
hrd_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], ext_links = TRUE, type = "z")
...
hrd_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], ext_links = TRUE, type = "perc")
...
##### Generate overview boxplot
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, plot_mode = params$plots_mode, hexcode = "hrd_genes")
...
```{r drugs_predictive_fusion_genes, comment = NA, message=FALSE, warning=FALSE, eval = runKallistoChunk}
