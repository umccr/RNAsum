...
bezierPlot <- function(fusion_data, genes_annot) {
  
  ##### Keep only fusions for which both genes have gene symbol available
  fusion_data <- fusion_data[ fusion_data$geneA.name %in% genes_annot$SYMBOL, ]
  fusion_data <- fusion_data[ fusion_data$geneB.name %in% genes_annot$SYMBOL, ]
  
  ##### Get genomic info for fusions genes
  fusion_annot1 <- merge(as.data.frame(fusion_data$geneA.name), genes_annot, by = 1, sort=FALSE)
  fusion_annot2 <- merge(as.data.frame(fusion_data$geneB.name), genes_annot, by = 1, sort=FALSE)
  
  fusion_annot <- cbind(fusion_annot1, fusion_annot2, fusion_data$fusions_abundant, fusion_data$fusions_cancer)
  ...
  fusion_annot <- fusion_annot[ fusion_annot$fusion_data.fusions_abundant == "Yes" | fusion_annot$fusion_data.fusions_cancer == "Yes" , ]
  ...
  ##### ... and the events type
  type <- paste(fusion_annot$fusion_data.fusions_abundant, fusion_annot$fusion_data.fusions_cancer, sep = "-") 
...
exprProfilePlot <- function(data, targets, sampleName, normal, cancer, genes_annot) {
    ...
    group.z <- merge(genes_annot, group.z, by = "row.names", all.x = FALSE)
    group.z <- group.z[, -1]
    
    ###### Change chromosomes X and Y to numbers 23 and 24, respectively
    group.z$SEQNAME[ group.z$SEQNAME  %in% "X" ] <- 23
    group.z$SEQNAME[ group.z$SEQNAME %in% "Y" ] <- 24
    
    ##### Order the data accordingly to genomic postions
    group.z = group.z[order(as.numeric(group.z$SEQNAME),as.numeric(group.z$GENESEQSTART)),]
    
    ##### Label chromosomes
    chr_bins <- data.matrix(summary(as.factor(group.z$SEQNAME)))
    chr_bins<- data.frame(chr_bins,row.names(chr_bins) )
    colnames(chr_bins) = c("count","Chromosome")
    
    data.chr_bins <- data.frame(unique(group.z$SEQNAME))
    ...
    chr_lines <- rep(0, nrow(group.z))
    chr_lines[ chr_annot$count - 1 ] <- 1
    chr_lines[ chr_annot$count ] <- -1
  
    ##### Prepare data for plotting with plotly
    data2plot <- data.frame(group.z[ , c("SYMBOL", "Normal", cancer, "Patient" )], chr_lines)
    names(data2plot)[names(data2plot)=="SYMBOL"] <- "Gene"
    
    ##### Use the genomic positions order for bars
    data2plot$Gene <- factor(data2plot$Gene, levels = data2plot$Gene)
    
    ##### Report only genes with Z-scores > 1 or < -1
    data2plot[ any(data2plot[, c(2:4) ] < 1 && data2plot[, c(2:4) ] > -1) ] <- 
    
    
    
    p <- plot_ly(data2plot, x = ~Gene, y = ~Patient, type = 'bar', name = "mRNA expression (mRNA)", color = I("red"), width = 800, height = 400) %>%
...
for ( ref in names(ref_datasets.list) ) {
  ...
  ##### Get genes genomic locations (Use Ensembl annotation version 86 (Oct 2016), the most recent as of Oct 2018)
  ...
  gene_pos <- ensembldb::select(edb, keys=keys, columns=c("SEQNAME", "GENESEQSTART", "GENESEQEND"),
keytype="GENEID")
  names(gene_pos)[1] <- "ENSEMBL"
  
  ##### Merge genes genomic coordinates info with their annotation and expression data
  data.annot <- merge(gene_pos, data.annot, by = "ENSEMBL", all.x = FALSE)
  ...
  ref_datasets.list[[ref]][[5]] <- data.annot[, c("SYMBOL", "GENENAME", "ENSEMBL", "ENTREZID", "SEQNAME", "GENESEQSTART", "GENESEQEND")]
}
```
...
pizzly.fusions.annot <- cbind(pizzly.fusions2, fusions_abundant, fusions_cancer)

##### Indicate duplicated rows and flag them as both, those which involve both highly abundant transcripts and cancer genes
pizzly.fusions.annot$fusions_abundant[ dup.index  ] <- "Yes"
pizzly.fusions.annot$fusions_cancer[ dup.index ] <- "Yes"

##### Remove duplicated rows
if ( any(duplicated(pizzly.fusions.annot)) ) {
  
  pizzly.fusions.annot <- pizzly.fusions.annot[ -c(which( duplicated(pizzly.fusions.annot) )), ]
}

##### Re-order data frame to prioritise fusions with highly abundant transcripts and cancer genes
pizzly.fusions.annot <- pizzly.fusions.annot[ rev(with(pizzly.fusions.annot, order(fusions_abundant, fusions_cancer))), ]
```
...
pizzly.fusions.clean <- pizzly.fusions.annot[ , names(pizzly.fusions.annot) %!in% c("geneA.id", "geneB.id", "transcripts.list") ]
...
<details>
<summary>Genomic view</summary>

```{r bezier_plot_fusions, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 4}
##### Generate bezier curves-like plot representing gene fusion events
bezierPlot(pizzly.fusions.annot, ref_datasets.list[[ref]][[5]])
```
