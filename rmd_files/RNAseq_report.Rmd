...
densityPlot <- function(gene, data, main_title, x_title, sampleName, distributions = NULL, scaling = "gene-wise", report_dir) {
  ...
  PlotsDir <- paste(report_dir, "exprDistrPlots", sep = "/")
...
mutCNexprPlot <- function(data, alt_data = FALSE, cn_bottom = cn_bottom, cn_top = cn_top, comp_cancer, type = "z", report_dir) {
  ...
  if ( alt_data ) {
    p <- plot_ly(type='scatter', mode = "markers", width = 800, height = 500, showlegend = FALSE) %>%
  ...
  ##### Generate scatterplot with per-gene expression values (y-axis) and CN values (x-axis)
    ...
    p <- plot_ly(data, x = ~CN, y = ~Expr, text=~Gene, color = ~Gene, type='scatter', mode = "markers", marker = list(size=10, symbol="circle"), width = 800, height = 500) %>%
  ...
  ##### Create directory for the plots
  mutCNexprPlotDir <- paste(report_dir, "mut_cn_expr_plot", sep = "/")
  if ( !file.exists(mutCNexprPlotDir) ) {
    dir.create(mutCNexprPlotDir, recursive=TRUE)
  }
  
  ##### Save interactive plot as html file
  saveWidgetFix(p, file = paste(mutCNexprPlotDir, "mut_cn_expr_plot.html", sep = "/"))
...
fusion_png <- function(geneA, geneB, clinker_results ) {
  ...
  ##### Clean the space
  rm(geneA, geneB, clinker_results, fusion_pdf, fusion_png, fusion)
}
...
```{r myqsl_populate, message=FALSE, warning=FALSE}
##### Initiate MySQL commend to populate RNA-seq portal database
mysql_populate <- paste0("### MySQL command to insert data for sample \"", params$sample_name, "\"\nuse piedb;\nINSERT INTO RNAseq_reports ( ID ,Platform, PatientID, SampleID, Cancer, Type, Project, URL, PMID, Analysis, Summary, Date ) VALUES ( 1000000, \"RNA_seq\"")
...
if ( runPcgrChunk ) {
  ...
  mysql_populate <- paste0(mysql_populate, ", \"", subjectID, "\", \"", params$sample_name, "\", \"", params$dataset , "\", \"", params$sample_type , "\", \"", params$project , "\", \"\"",", \"", params$sample_name, "\", \"Expression profiles"  )
  mysql_populate_update <-  paste0(mysql_populate_update, ", PatientID=\"", subjectID, "\", SampleID=\"", params$sample_name, "\", Cancer=\"", params$dataset , "\", Type=\"", params$sample_type , "\", Project=\"", params$project , "\", URL=\"\"",", PMID=\"", params$sample_name, "\", Analysis=\"Expression profiles"  )
  
} else {
  ref_genes.list[["pcgr"]] <- NULL
  
  ##### Update MySQL commend to populate RNA-seq portal database
  mysql_populate <- paste0(mysql_populate, ", \"\", \"", params$sample_name, "\", \"", params$dataset , "\", \"", params$sample_type , "\", \"", params$project , "\", \"\"",", \"", params$sample_name, "\", \"Expression profiles"  )
  mysql_populate_update <-  paste0(mysql_populate_update, ", PatientID=\"\", SampleID=\"", params$sample_name, "\", Cancer=\"", params$dataset , "\", Type=\"", params$sample_type , "\", Project=\"", params$project , "\", URL=\"\"",", PMID=\"", params$sample_name, "\", Analysis=\"Expression profiles"  )
}
...
for (row in 1:nrow(pizzly.fusion.candidates)){
  if(pizzly.fusion.candidates[row,"geneA.name"] %in% rownames(ref_genes.list[["genes_cancer"]]) | pizzly.fusion.candidates[row,"geneB.name"] %in% rownames(ref_genes.list[["genes_cancer"]])) {
    ...
    result.cancer_genes <- rbind(result.cancer_genes, data.frame(pizzly.fusion.candidates[row,]))
  }
}
...
if ( nrow(result.cancer_genes) > 0 ) {
  idx <- order(result.cancer_genes$splitcount, result.cancer_genes$paircount, decreasing = TRUE)
  result.cancer_genes <- result.cancer_genes[idx,]
}

##### Sort and filter quantification file on tpm values. First make sure to consider only fusion transcripts re-qunatified by kallistofrom that are actiually in pizzly output file. Then, grep only the transcript ids for fusion genes from quantification #file. Currently filtering on quantiles. Selected 0.9 because that reduces the #final fusion calls to the value we are #interested in (~15)
quant.fusions.only.transcripts <- quant[grep(":", quant$target_id), ]
pizzly.fusion.transcripts <- unlist(strsplit(as.character(pizzly.fusion.candidates[, "transcripts.list"]), "\\;"))
quant.fusions.only.transcripts <- quant.fusions.only.transcripts[quant.fusions.only.transcripts$target_id %in% pizzly.fusion.transcripts, ]
...
##### Let's try using for loop for iterating over pizzly.fusion.candidates dataframe and get transcriptID and fusion gene pair information.
##### Can also filter quant.sorted.filtered$target_id to have only fusion gene target ids (that is two transcripts instead of one, this will increase speed
for (row in 1:nrow(pizzly.fusion.candidates)){
  y <- strsplit(as.character(pizzly.fusion.candidates[row, "transcripts.list"]), "\\;")
  y <- unname(y)
  for (i in 1:length(y[[1]])){
    if (y[[1]][i] %in% quant.sorted.filtered$target_id){
      
      ##### Creating a new dataframe for the filtered pizzly results
      result <- rbind(result, data.frame(pizzly.fusion.candidates[row,]))
    }
  }
}
...
result.other_genes <- pizzly.fusion.candidates[ rownames(pizzly.fusion.candidates) %!in% c(rownames(result.cancer_genes), rownames(deduped.result)), ]
  
##### Ordering pizzly's other genes results on the basis of read count values
##### Skip ordering if no of the fusion genes is a cancer gene
if ( nrow(result.other_genes) > 0 ) {
  idx <- order(result.other_genes$splitcount, result.other_genes$paircount, decreasing = TRUE)
  result.other_genes <- result.other_genes[idx,]
}
...
pizzly.fusions <- pizzly.fusions[ order(pizzly.fusions$reported_fusion, pizzly.fusions$fusions_abundant, pizzly.fusions$splitcount, pizzly.fusions$paircount, pizzly.fusions$fusions_cancer, pizzly.fusions$reported_fusion_geneA, pizzly.fusions$reported_fusion_geneB, decreasing = TRUE), ]

##### Clean the space and return output
rm(quant, deduped.result, pizzly.fusion.transcripts, pizzly.fusion.candidates, quant.sorted.filtered, quant.fusions.only.transcripts, known_translocations.CGI, known_translocations.FusionGDB, result, result.cancer_genes, result.other_genes, trans.pairs)
```
...
##### Clean the space
rm(fusion_annot1, fusion_annot2, pizzly.fusions.annot, fusion_genes_annot)
```

```{r pizzly_data_cleanup, comment = NA, message=FALSE, warning=FALSE, eval = runFusionChunk}
##### Clean the pizzly data first. Remove some columns and rename column names for better presentation
...
pizzly.fusions.cut <- pizzly.fusions.cut[ , c("geneA.name", "geneB.name", "splitcount", "paircount", "geneA_dna_support", "geneB_dna_support", "reported_fusion", "FGID", "fusions_abundant", "fusions_cancer", "reported_fusion_geneA", "reported_fusion_geneB") ]
```
...
sunburst_plot_all <- plot_ly(sunburst.all.df, ids = ~ids, labels = ~labels, parents = ~parents, values = ~values, type = 'sunburst', width = 600, height = 600)
...
##### Save interactive plot as html file
saveWidgetFix(sunburst_plot_all, file = paste(summaryPlotsDir, "sunburst_plot_all.html", sep = "/"))
saveWidgetFix(sunburst_plot, file = paste(summaryPlotsDir, "sunburst_plot.html", sep = "/"))
...
* **`r paste(ref_ext_cancer, ext_cancer_group, sep=" ")`** samples from [The Cancer Genome Atlas](https://cancergenome.nih.gov/abouttcga){target="_blank"} project ([related publication](https://www.nature.com/articles/ng.2764){target="_blank"})
...
Scree plot presenting the fraction of total variance (y-axis) attributed to each PC (x-axis) before and after batch effects correction. The PCs are ordered by decreasing order of contribution to total variance. 
...
NOTE: for confidentiality reasons, the start of the timeline (*x-axis*) projecting patient's treatment regimens (*y-axis*) is set 1st January 2000, but the treatments lengths are preserved.
...
##### Update MySQL commend to populate RNA-seq portal database
mysql_populate <- paste0(mysql_populate, ",Treatment timeline")
mysql_populate_update <- paste0(mysql_populate_update, ",Treatment timeline")
```
...
rm(counts_vs_transformed, data_transformation_nonfiltered, data_transformation_filtered, data_nonnormalised, data_normalised, treatment_timeline, sunburst_plot, sunburst_plot_all, data.annot)
```
...
4. Involvement of **cancer gene(s)** (see [Cancer genes] section)
...
if ( runFusionChunk ) {
  ...
  ##### Filter out fusions that are with < 2 split reads and < 2 pair reads and are not supported by genomic data, are nor reported, don't involve abundant transcripts and don't involve cancer genes
  pizzly.fusions.cut <- pizzly.fusions.cut %>% dplyr::filter(splitcount > 1 | paircount > 1 & geneA_dna_support != "-" | geneB_dna_support != "-" | reported_fusion != "-" | fusions_abundant != "-" | fusions_cancer != "-")
  ...
  names(pizzly.fusions.table) <- c("Gene A", "Gene B", "Split count", "Pair count", "DNA support (gene A)", "DNA support (gene B)", "Reported fusion", "Abundant transcript(s)", "Cancer gene(s)", "Fusion gene (A)", "Fusion gene (B)")
...
if ( params$save_tables ) {
  ...
  saveWidgetFix(widget=pizzly.fusions.summary, file=paste(fusionsTableDir, "pizzly.fusions.summary.html", sep = "/"), selfcontained=TRUE)
}

##### Clean the space and return output
rm(pizzly.fusions.table, pizzly.fusions.summary)
```
...
if ( runSVsChunk && nrow(fusion_annot_top) > 0 && params$save_tables ) {
  saveWidgetFix(widget=pizzly.fusions.genomicView, file=paste(fusionsTableDir, "pizzly.fusions.genomicView.html", sep = "/"), selfcontained=TRUE)  
}

##### Clean the space and return output
rm(pizzly.fusions.genomicView)
```
...
<span style="color:#ff0000">NOTE</span>: the *Fusion gene visualisation* is not available for gene pairs for which no junctions were found by [clinker](https://github.com/Oshlack/Clinker){target="_blank"}.
...
targets <- ref_dataset.list[[dataset]][["sample_annot"]]
data <- ref_dataset.list[[dataset]][["data_to_report"]]
genes <- as.character(pizzly.fusions$geneA.name)
...
genes <- c(genes, as.character(pizzly.fusions$geneB.name))

##### Collect info and plots for each of the top fusions
for( i in 1:genes_no ) {
  
  geneA <- as.vector(pizzly.fusions$geneA.name[i])
  geneB <- as.vector(pizzly.fusions$geneB.name[i])
  
  ##### List all fusion transcript candidates for given fusion event along with corresponding breakpoints
  fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
    
    output_transcripts[[i]] <- fusion_reads.selected %>% 
      transform(transcripts.list = strsplit(as.character(transcripts.list), ";")) %>%
      unnest(transcripts.list) %>%
      dplyr::select(transcripts.list)
...
for( i in 1:genes_no ) {
  ...
  cat("\n#### ", paste(pizzly.fusions$geneA.name[i], pizzly.fusions$geneB.name[i], sep="-"), "\n")
  ...
  if ( file.exists(paste0(dataDir, "/clinker/", geneA, "_", geneB, ".pdf")) ) {
    ...
    ##### List fusion events for which Clinker plot is available (for PIEdb portal)
    clinker_fusions <- c(clinker_fusions, paste0(geneA, "-", geneB))

    ##### Convert the fusion clinker plots from pdf to png and present it in the report
    fusion_png(geneA, geneB, paste(dataDir, "clinker", sep="/") )
    cat(paste0("![](", paste(dataDir, "clinker", paste0(geneA, "_", geneB), sep="/"), ".png)"), "\n")
    cat("\n***\n")
  }
    
  ##### List all fusion transcript candidates for given fusion event along with corresponding breakpoints
  cat("\n<details>\n")
  cat("\n<summary>Fusion transcripts list</summary>\n")
  cat("<font size=\"2\">\n")
  cat(paste0("\n", paste0(unlist(output_transcripts[[i]]), collapse = "<br />"), "\n"))
  cat("\n</font>\n")
  cat("\n</details>\n")
  cat("\n***\n")
...
write.table(prepare2write(clinker_fusions), file = paste0(params$report_dir, "/results/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.clinker_fusions.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=FALSE, append = FALSE )
...
Section overlaying the mRNA expression data for **[Cancer genes]** with per-gene somatic copy-number (CN) data (from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}), as well as SNVs/indels and SVs data, if available.

CN information for this sample is **`r if ( runPurpleChunk ) { c("AVAILABLE") } else { c("NOT AVAILABLE") }`**.

### - Genomic view

`r if (runPurpleChunk) { length(ref_dataset.list[[dataset]][["expr_mut_cn_data_all"]][,1])} else { cat("0") }` genes with available CN data are presented in the genomic context. **`r if (runPurpleChunk) { length(ref_dataset.list[[dataset]][["expr_mut_cn_data"]][,1]) } else { length(NULL) }`** of them (indicated by various colours) are [Cancer genes] and are gained `r if ( runPurpleChunk ) { paste0("(CN values > ", cn_top, ")") }` or lost `r if ( runPurpleChunk ) { paste0("(CN values < ", cn_bottom, ")") }`. All other genes are marked in <span style="color:#808080">gray</span>.
...
if ( runFusionChunk ) {
  ...
  data$Alterations[ data$Gene %in% pizzly.fusions.cut$geneA.name  ] <- paste0( data$Alterations[ data$Gene %in% pizzly.fusions.cut$geneA.name  ], "; Fusion")
  data.sub$Alterations[ data.sub$Gene %in% pizzly.fusions.cut$geneA.name  ] <- paste0( data.sub$Alterations[ data.sub$Gene %in% pizzly.fusions.cut$geneA.name  ], "; Fusion")
  data$Alterations[ data$Gene %in% pizzly.fusions.cut$geneB.name  ] <- paste0( data$Alterations[ data$Gene %in% pizzly.fusions.cut$geneB.name  ], "; Fusion")
  data.sub$Alterations[ data.sub$Gene %in% pizzly.fusions.cut$geneB.name  ] <- paste0( data.sub$Alterations[ data.sub$Gene %in% pizzly.fusions.cut$geneB.name  ], "; Fusion")
}
...
Out of the `r length(ref_dataset.list[[dataset]][["expr_mut_cn_data_all"]][ ref_dataset.list[[dataset]][["expr_mut_cn_data_all"]]$CN < cn_bottom | ref_dataset.list[[dataset]][["expr_mut_cn_data_all"]]$CN > cn_top, ][,1])` genes within gained `r if ( runPurpleChunk ) { paste0("(CN values > ", cn_top, ")") }` or lost `r if ( runPurpleChunk ) { paste0("(CN values < ", cn_bottom, ")") }` regions `r if (runPurpleChunk) { length(ref_dataset.list[[dataset]][["expr_mut_cn_data"]][,1]) } else { length(NULL) }` are [Cancer genes]. The expression of **`r if (runPurpleChunk) { length(which(ref_dataset.list[[dataset]][["expr_mut_cn_data"]]$Gene %in% rownames(ref_dataset.list[[dataset]][["data_to_report"]]))) } else { length(NULL) }`** of these genes was reliably measured in patient's sample. The remaining `r if (runPurpleChunk) { length(which(cn_genes %!in% rownames(ref_dataset.list[[dataset]][["data_to_report"]]))) } else { length(NULL) }` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).

#### Gains {.tabset}

Table summarising the **mRNA expression** values in cancer and patient samples for genes with **CN** values > `r if ( runPurpleChunk ) { cn_top } else { c("(NA)") }` (**gains**), based on patient's genomic data (from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}), and mutation status if available (from [PCGR](https://github.com/sigven/pcgr){target="_blank"}).
...
if ( runPcgrChunk && runPurpleChunk ) {
  mut_cn_expr_genes.expr.gains.z <- exprTable( genes = genes_gains, data = data, cn_data = cn_data, cn_decrease = TRUE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
  
##### Generate expression summary table for per-gene expression values and CN values
} else if ( runPurpleChunk ) {
  mut_cn_expr_genes.expr.gains.z <- exprTable( genes = genes_gains, data = data, cn_data = cn_data, cn_decrease = TRUE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
}
...
##### Save the expression table as html file
if ( params$save_tables ) {
  saveWidgetFix(widget=mut_cn_expr_genes.expr.gains.z[[1]], file=paste(exprTableDir, "mut_cn_expr_genes.expr.gains.z.html", sep = "/"), selfcontained=TRUE)
}
```
...
##### Generate expression summary table for per-gene expression values CN values and mutation status info (colours)
if ( runPcgrChunk && runPurpleChunk ) {
  mut_cn_expr_genes.expr.gains.perc <- exprTable( genes = genes_gains, data = data, cn_data = cn_data, cn_decrease = TRUE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)
  
##### Generate expression summary table for per-gene expression values and CN values
} else if ( runPurpleChunk ) {
  mut_cn_expr_genes.expr.gains.perc <- exprTable( genes = genes_gains, data = data, cn_data = cn_data, cn_decrease = TRUE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)
}
...
##### Save the expression table as html file
if ( params$save_tables ) {
  saveWidgetFix(widget=mut_cn_expr_genes.expr.gains.perc[[1]], file=paste(exprTableDir, "mut_cn_expr_genes.expr.gains.perc.html", sep = "/"), selfcontained=TRUE)
}

##### Clean the space
rm(mut_cn_expr_genes.expr.gains.perc, cn_data)
```
...
Table summarising the **mRNA expression** values in cancer and patient samples for genes with **CN** values < `r if ( runPurpleChunk ) { cn_bottom } else { c("(NA)") }` (**losses**), based on patient's genomic data (from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}), and mutation status if available (from [PCGR](https://github.com/sigven/pcgr){target="_blank"}).
...
if ( runPcgrChunk && runPurpleChunk ) {
  mut_cn_expr_genes.expr.losses.z <- exprTable( genes = genes_losses, data = data, cn_data = cn_data, cn_decrease = FALSE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
  
##### Generate expression summary table for per-gene expression values and CN values
} else if ( runPurpleChunk ) {
  mut_cn_expr_genes.expr.losses.z <- exprTable( genes = genes_losses, data = data, cn_data = cn_data, cn_decrease = FALSE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
}
...
##### Save the expression table as html file
if ( params$save_tables ) {
  saveWidgetFix(widget=mut_cn_expr_genes.expr.losses.z[[1]], file=paste(exprTableDir, "mut_cn_expr_genes.expr.losses.z.html", sep = "/"), selfcontained=TRUE)
}
```
...
##### Generate expression summary table for per-gene expression values CN values and mutation status info (colours)
if ( runPcgrChunk && runPurpleChunk ) {
  mut_cn_expr_genes.expr.losses.perc <- exprTable( genes = genes_losses, data = data, cn_data = cn_data, cn_decrease = FALSE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)[[1]]
  
##### Generate expression summary table for per-gene expression values and CN values
} else if ( runPurpleChunk ) {
  mut_cn_expr_genes.expr.losses.perc <- exprTable( genes = genes_losses, data = data, cn_data = cn_data, cn_decrease = FALSE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], oncokb_annot = ref_genes.list[["genes_oncokb"]], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)[[1]]
}
  
##### Present the expression, CN and mutation data summary table
mut_cn_expr_genes.expr.losses.perc

##### Save the expression table as html file
if ( params$save_tables ) {
  saveWidgetFix(widget=mut_cn_expr_genes.expr.losses.perc, file=paste(exprTableDir, "mut_cn_expr_genes.expr.losses.perc.html", sep = "/"), selfcontained=TRUE)
}

##### Clean the space and return output
rm(cn_data, mut_cn_expr_genes.expr.losses.perc)
```
...
```{r cdf_plot_mut_cn_expr_gains, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, eval = runPcgrChunk, results="asis"}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level for the genes of interest in the context of the overall mRNA expression distribution
output_cdf <- list()
output_density <- list()
...
```{r cdf_plot_mut_cn_expr_losses, echo=FALSE, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, eval = runPcgrChunk, results="asis"}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level for the genes of interest in the context of the overall mRNA expression distribution
output_cdf <- list()
output_density <- list()
...
Out of the `r length(unique(unlist(ref_genes.list[["genes_immune"]]$SYMBOL)))` immune markers the expression of **`r length(which(ref_genes.list[["summary"]]$Immune %in% rownames(ref_dataset.list[[dataset]][["data_to_report"]])))`** was reliably measured in patient's sample. The remaining `r length(which(ref_genes.list[["summary"]]$Immune %!in% rownames(ref_dataset.list[[dataset]][["data_to_report"]])))` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
##### Update MySQL commend to populate RNA-seq portal database
mysql_populate <- paste0(mysql_populate, ",Cancer genes")
mysql_populate_update <- paste0(mysql_populate_update, ",Cancer genes")
...
```{r myqsl_populate_finalise, message=FALSE, warning=FALSE}
##### Finalise and write into a file the MySQL commend to populate RNA-seq portal database
