...
output:
  ...
  rmdformats::material:
    highlight: kate
params:
  report_dir: '/Users/kanwals/Documents/UMCCR/data/RNAseq-report/CCR180038_SV18T002P006_RNA'
  sample_name: 'CCR180038_SV18T002P006_RNA'
  count_file: '/Users/kanwals/Documents/UMCCR/data/RNAseq-report/CCR180038_SV18T002P006_RNA/CCR180038_SV18T002P006_RNA-ready.counts'
  tissue: 'pancreas'
  ...
  civic_gene_summaries: '../data/CIViC/01-Oct-2018-GeneSummaries.tsv'
  ...
  civic_variant_group: '../data/CIViC/01-Oct-2018-VariantGroupSummaries.tsv'
---
...
exprTable <- function(genes, data, targets, sampleName, normal, cancer, genes_annot = NULL, OncoKB_genes = NULL) {
  ...
  if ( !is.null(OncoKB_genes) ) {
    ...
    for ( gene in genes) {
      ...
      if ( gene %in% rownames(OncoKB_genes) ) {
...
civicDrugTable <- function(genes, civic_gene_summaries, civic_var_summaries, civic_clin_evid, civic_variant_group) {
  
  ##### Initialize data fmrae to the about drug-target info from CIViC
  drug.info <- setNames(data.frame(matrix(ncol = 16, nrow = 0)), c("Gene", "Variant", "Disease", "phenotypes", "drugs", "nct_ids", "evidence_level", "evidence_type", "evidence_direction", "clinical_significance", "rating", "pubmed_id", "variant_origin", "representative_transcript", "representative_transcript2", "last_review_date"))

  ##### Loop thourgh each gene and check if they are druggable
  for ( gene in genes) {
    
    ##### Get summary info about druggable genes
    if ( gene %in% civic_gene_summaries$name ) {
      
      gene.info <- civic_gene_summaries[ civic_gene_summaries$name == gene , ]
      ...
      clin.evid.info$evidence_type[ clin.evid.info$gene==gene] <- paste0("<a href='", clin.evid.info$evidence_civic_url[ clin.evid.info$gene==gene], "' target='_blank'>", clin.evid.info$evidence_type, "</a>")
      
      ##### Provide link to CIViC gene summary
      clin.evid.info$gene_civic_url[ clin.evid.info$gene==gene] <- paste0("<a href='", clin.evid.info$gene_civic_url[ clin.evid.info$gene==gene], "' target='_blank'>", gene, "</a>")
      names(clin.evid.info)[ names(clin.evid.info) =="gene_civic_url" ] <- "Gene"
      
      ##### Provide link to CIViC variants summary
      clin.evid.info$variant_civic_url[ clin.evid.info$gene==gene] <- paste0("<a href='", clin.evid.info$variant_civic_url[ clin.evid.info$gene==gene], "' target='_blank'>", clin.evid.info$variant, "</a>")
      ...
      for ( nct_id in clin.evid.info$nct_ids[ clin.evid.info$gene==gene] ) {
      ...
      clin.evid.info$pubmed_id[ clin.evid.info$gene==gene] <- paste0("<a href='https://www.ncbi.nlm.nih.gov/pubmed/", clin.evid.info$pubmed_id[ clin.evid.info$gene==gene], "' target='_blank'>", clin.evid.info$pubmed_id, "</a>")
      
      ##### Provide link to Disease Ontology
      clin.evid.info$doid[ clin.evid.info$gene==gene] <- paste0("<a href='http://www.disease-ontology.org/?id=DOID:", clin.evid.info$doid[ clin.evid.info$gene==gene], "' target='_blank'>", clin.evid.info$disease, "</a>")
  ...
  names(drug.info) <- c("Gene", "Variant","Disease", "Phenotypes", "Drugs", "Clinical trials", "Evidence level", "Evidence type", "Evidence direction", "Clinical significance", "Trust rating", "PubMed ID",  "Variant origin", "Representative transcript", "Representative transcript 2", "Review date")
  ...
  escape = FALSE) %>%
    ...
    DT::formatStyle(columns = "Evidence level", 
                    backgroundColor = DT::styleEqual(c("A", "B", "C", "D", "E"), c("mediumseagreen", "deepskyblue", "mediumpurple", "darkorange", "coral")) )  %>%
...
caner_genes_annot.list[["civic_gene_summaries"]] <- read.table(params$civic_gene_summaries, sep="\t", as.is=TRUE, header=TRUE, row.names=NULL, quote="", fill = TRUE)
...
caner_genes_annot.list[["civic_variant_group"]] <- read.table(params$civic_variant_group, sep="\t", as.is=TRUE, header=TRUE, row.names=NULL, quote="", fill = TRUE)
```
...
* `r nrow(y.filtered$counts) - nrow(data.annot)` genes were ignored due to lack of [HGNC](https://www.genenames.org/){target="_blank"}-apporved gene symbol
...
Section summarising the key findings (clincially relevant dysregulated genes and pathways, gene fusion events etc.)
...
pizzly.fusions$order2 <- rownames(pizzly.fusions) %in% rownames(result.sorted.cancer_genes)
...
NOTE: currently, displaying the top 3 hits from pizzly filtering output (red section in [Fusion events prioritisation] table).
...
```{r gene_table_fusions_2, comment = NA, message=FALSE, warning=FALSE, eval = FALSE}
fusion.df <- ref_datasets.list[[ref]][[5]]
fusion.df <- rbind(fusion.df[fusion.df$SYMBOL==geneA, ], fusion.df[fusion.df$SYMBOL==geneB, ])
exprTable(fusion.df$SYMBOL, data, target, params$sample_name, params$normal_group, params$cancer_group, ref_datasets.list[[ref]][[5]][, c(1,3)], ref_genes.list[["oncokb_genes"]][,c(1:2)])[[1]]
```
...
```{r fusions_figs3, comment = NA, message=TRUE, warning=FALSE, eval = FALSE}
##### Convert the fusion image from pdf to png and present it in the report
...
```{r fusions_tables3, comment = NA, message=TRUE, warning=FALSE, eval = FALSE}
##### Present a table with number of reads supporting this fusion
fusion_result_reads <- result_reads[ result_reads$geneA.name == geneA & result_reads$geneB.name == geneB, ]
kable(fusion_result_reads, row.names = FALSE, caption = paste0("Number of reads supporting ", paste(geneA, geneB, sep="-"), " fusion")) %>%
  kable_styling(font_size = 12, "striped", "bordered") %>%
...
fusion_png(pizzly.fusions2$geneA.name[4], pizzly.fusions2$geneB.name[4], paste(dataDir, "clinker", sep="/") )
```
...
### Dysregulated cancer genes

NOTE: currently, only the top 50 cancer genes with the greatest difference in standarised (Z-score) mRNA expression values between patient's sample and the average mRNA expression in samples from normal individuals (see [Cancer genes expression profiles] section)  were screened for suitable drugs.

```{r drugs_cancer_genes, comment = NA, message=FALSE, warning=FALSE}
...
civicDrugTable(genes, caner_genes_annot.list[["civic_gene_summaries"]], caner_genes_annot.list[["civic_var_summaries"]], caner_genes_annot.list[["civic_clin_evid"]], caner_genes_annot.list[["civic_variant_group"]])[[1]]
```
...
### Detected gene fusion events

NOTE: currently, fusion events involving high expression count and cancer genes (red and blue sections in [Fusion events prioritisation] table) were screened for suitable drugs.

```{r drugs_fusion_genes, comment = NA, message=FALSE, warning=FALSE}
##### Generate table with drugs targeting dysregulated cancer genes
ref <- params$tissue
target <- ref_datasets.list[[ref]][[2]]
data <- ref_datasets.list[[ref]][[3]]
genesA <- levels(deduped.result$geneA.name)[c(deduped.result$geneA.name, result.cancer_genes$geneA.name)]
genesB <- levels(deduped.result$geneB.name)[c(deduped.result$geneB.name, result.cancer_genes$geneB.name)]

civicDrugTable(c(genesA, genesB), caner_genes_annot.list[["civic_gene_summaries"]], caner_genes_annot.list[["civic_var_summaries"]], caner_genes_annot.list[["civic_clin_evid"]], caner_genes_annot.list[["civic_variant_group"]])[[1]]
```
