...
combineDatasets <- function(sample_name, sample_counts, ref_data, report_dir, dataset, dataset_name_incl = FALSE) {
  ...
  if ( length(gene_list.missing) > 0 ) {
    if ( isFALSE(dataset_name_incl) ) {
      write.table(prepare2write(gene_list.missing), file = paste0(report_dir, "/", sample_name, dataset_name_incl, ".missing_genes.txt"), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
    } else {
      write.table(prepare2write(gene_list.missing), file = paste0(report_dir, "/", sample_name, ".",  dataset, ".missing_genes.txt"), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
    }
  }
...
for ( dataset in names(ref_dataset.list) ) {
  ...
  write.table(prepare2write(ref_dataset.list[[dataset]][["data_to_report"]]), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".combined_data_processed.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(rownames(ref_dataset.list[[dataset]][["data_to_report"]])), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".combined_data_processed.genes.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(ref_dataset.list[[dataset]][["sample_annot"]]), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".sample_annot.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
}
...
##### Clean the space and return output
rm(quant, quant.fusions.only.transcripts, known_translocations.CGI, known_translocations.FusionGDB, result, result.cancer_genes, result.other_genes)
...
write.table(prepare2write(clinker_fusions), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".clinker_fusions.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
```
