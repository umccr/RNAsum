...
##### Record all genes of interest, including...
...
##### Identify genes that appear in more then two lists
if ( runFusionChunk || runPcgrChunk || runPurpleChunk || runSVsChunk ) {
  
  alt_genes <- sort(table(unlist(ref_genes.list[["summary"]])), decreasing = TRUE)
  alt_genes <- alt_genes[ alt_genes > 1 ]
  
  alt_genes.list <- ref_genes.list[["summary"]]
  
  for ( i in 1:length(ref_genes.list[["summary"]]) ) {
    
    alt_genes.list[[i]] <- alt_genes[ names(alt_genes) %in% alt_genes.list[[i]] ]
  }
  
  ##### Don't show cancer genes list (too long)
  alt_genes.list$Cancer <- NULL
  ...
  for ( i in 1:length(alt_genes.list) ) {
    
    sunburst.df <- rbind( sunburst.df , data.frame(ids = paste(names(alt_genes.list)[i], names(alt_genes.list[[i]]), sep = " - "), 
               labels = paste0("\t\t", names(alt_genes.list[[i]]), "\t\t"),
               parents = rep(names(alt_genes.list)[i], length(alt_genes.list[[i]])),
               values = as.numeric(alt_genes.list[[i]])
               ) )
  }
  
  sunburst_plot <- plot_ly(sunburst.df, ids = ~ids, labels = ~labels, parents = ~parents, values = ~values, type = 'sunburst')
}
...
Genes listed in at least two report sections are summarised in the sunburst plot below. The number next to each gene indicates the number of times it appears cross the following report sections: [Mutated genes], [Fusion genes], [Structural variants], [CN altered genes], [Immune markers] or [HRD genes]. That number is also reflected by the width of corresponding branch. Click on the category of interest to expand corresponding branches.

```{r findings_summary, comment = NA, message=FALSE, warning=FALSE, fig.width = 6, fig.height = 6 }
##### Present findings summary in sunburst plot
sunburst_plot
```
...
Scatterplot comparing the per-gene difference in **mRNA expression** between patient's sample and cancer individuals (*y-axis*), **CN values** (*x-axis*, from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}). If the mutation status information is available then the genes's colours correspond to the variant(s) consequence (from [PCGR](https://github.com/sigven/pcgr){target="_blank"}). Genes with CN values > 3 or < 0.5 are annotated. NOTE: only [Cancer genes], [Mutated genes] with [tier](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} 1-`r params$pcgr_tier` variants (if mutation information is available), genes involved in [Fusion genes] and [Structural variants] (if SV information is available) are presented.
