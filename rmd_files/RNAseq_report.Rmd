...
```{r fusion_1_table, comment = NA, message=TRUE, warning=FALSE}
##### List all fusion transcript candidates for given fusion event
...
kable(fusion_reads.selected, row.names = FALSE, col.names = "", caption = paste0("List of ", paste(geneA, geneB, sep="-"), " fusion transcripts")) %>%
...
```{r fusion_2_table, comment = NA, message=TRUE, warning=FALSE}
##### List all fusion transcript candidates for given fusion event
fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
...
```{r fusion_3_table, comment = NA, message=TRUE, warning=FALSE}
##### List all fusion transcript candidates for given fusion event
fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
...
```{r fusion_4_table, comment = NA, message=TRUE, warning=FALSE}
##### List all fusion transcript candidates for given fusion event
fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
...
write.table(prepare2write(clinker_fusions), file = paste0(params$report_dir, "/", params$sample_name, params$dataset_name_incl, ".RNAseq_report.clinker_fusions.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
```
...
```{r fusion_5_table, comment = NA, message=TRUE, warning=FALSE}
##### List all fusion transcript candidates for given fusion event
fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
...
if ( length(genes_losses) == 0 ) {
  genes_losses <- NULL
  genes_losses_no <- 0
} else if ( length(genes_losses) > 10 ) {
  genes_losses_no <- 10
} else {
  genes_losses_no <- length(genes_losses)
}
  
if ( genes_gains_no + genes_losses_no > 10 ) {
  limit_genes <- TRUE
} else {
  limit_genes <- FALSE
}
...
**`r length(mut_genes.expr.z[[2]]$SYMBOL)`** genes with PCGR [tier](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} 1-`r params$pcgr_tier` variants were screened for suitable drugs (see [Mutated genes] section).
...
**`r if ( runPurpleChunk ) { length(cn_genes) } else { length(NULL) }`** genes with CN values > `r if ( runPurpleChunk ) { cn_top } else { c("(NA)") }` (**gains**) or < `r if ( runPurpleChunk ) { cn_bottom } else { c("(NA)") }` (**losses**, see [CN altered genes] section).
