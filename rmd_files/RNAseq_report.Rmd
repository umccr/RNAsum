...
cat(nrow(ref_genes.list[["genes_pmcc"]]), "PMCC panel genes are present in the expression data\n\n")

##### Subset the expression data to include only the PMCC genes
data <- data[ rownames(data) %in% unlist(ref_genes.list[["genes_pmcc"]]), ]
ref_datasets.list[[ref]][[6]] <- data

##### Initiate dataframe for expression mean values in each group
group.mean <- as.data.frame(matrix(NA, ncol = length(unique(target$Target)), nrow = nrow(data)))
colnames(group.mean) <- unique(target$Target)
rownames(group.mean) <- sort(rownames(data))

for ( group in unique(target$Target) ) {

  group.stats <- exprGroupStats(data[rownames(group.mean), ], target, group)
  group.stats <- group.stats[order(rownames(group.stats)), ]
  group.mean[, group] <- round(group.stats$mean, digits=2)
      
}

##### Order dataframe with gene annotations
genes.annot <- ref_datasets.list[[ref]][[5]][order(ref_datasets.list[[ref]][[5]]$SYMBOL), c(1:3)]

##### Merge the dataframe with groups mean expression values and gene annotations
group.mean <- merge(genes.annot, group.mean, by="row.names", all = FALSE, sort = FALSE)[, -1]
group.mean <- group.mean[ order(group.mean[, params$sample_name],  decreasing = TRUE), ]

##### Define colours for cells background (using expression values quantiles) for each group
##### Initiate dataframe for expression mean values in each group
brks.q <- as.data.frame( matrix(NA, ncol = length(unique(target$Target)), nrow = length(seq(.05, .95, .0005)) ))
colnames(brks.q) <- unique(target$Target)
clrs.q <- as.data.frame( matrix(NA, ncol = length(unique(target$Target)), nrow = length(seq(.05, .95, .0005))+1 ))
colnames(clrs.q) <- unique(target$Target)

for ( group in unique(target$Target) ) {

  brks.q[[group]] <- quantile(group.mean[, unique(target$Target)[1]], probs = seq(.05, .95, .0005), na.rm = TRUE)
...
DT::datatable( data = group.mean, rownames = FALSE, options = list(pageLength = 10), caption = htmltools::tags$caption(
        style = 'caption-side: top; text-align: center; color:black; 
        font-size:10% ;'),
      filter=list(position = 'top') ) %>%
  DT::formatStyle( columns = names(group.mean), `font-size` = '12px' ) %>%
  
  ##### Colour cells according to the expression values quantiles in each group
  DT::formatStyle(columns = unique(target$Target)[1], 
                  backgroundColor = DT::styleInterval(brks.q[[unique(target$Target)[1]]], clrs.q[[unique(target$Target)[1]]])) %>%
  DT::formatStyle(columns = unique(target$Target)[2], 
                  backgroundColor = DT::styleInterval(brks.q[[unique(target$Target)[2]]], clrs.q[[unique(target$Target)[2]]])) %>%
  DT::formatStyle(columns = unique(target$Target)[3], 
                  backgroundColor = DT::styleInterval(brks.q[[unique(target$Target)[3]]], clrs.q[[unique(target$Target)[3]]]))
