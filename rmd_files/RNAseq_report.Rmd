...
output:
  ...
  rmdformats::material:
    highlight: kate
params:
  report_dir: '/Users/jmarzec/data/Patients/final/CCR170057_AH17T001P001_MN_RNA/RNAseq_report'
  sample_name: 'CCR170057_AH17T001P001_MN_RNA'
  count_file: '/Users/jmarzec/data/Patients/final/CCR170057_AH17T001P001_MN_RNA/CCR170057_AH17T001P001_MN_RNA-ready.counts'
  tissue: 'pancreas'
  plots_mode: 'static'
  batch: '/Users/jmarzec/data/Patients/umccrised/2016_249_17_AH_P001_1__CCR170057_AH17T001P001'
  ref_data_dir: '../data'
...
# 1. ref_genes.list[["genes_cancer"]] = list of cancer genes derived from OncoKB portal (http://oncokb.org/#/cancerGenes), Peter Mac comprehensive cancer (PMCC) gene panel and UMCCR Cancer Gene list (https://trello.com/c/7j3KFMiL/184-umccr-cancer-genes)

# 2. ref_genes.list[["oncokb_genes"]] = list of cancer genes derived from OncoKB portal (http://oncokb.org/#/cancerGenes) alone (although genes present on the PMCC gene panel are also flagged)

# 3. ref_genes.list[["genes_immune"]] = list of immune reponse markers provided in OmniSeq report (https://trello-attachments.s3.amazonaws.com/5753fc730bff8d2b24f159df/59912803fd0e33e3395d05e7/dc9a81ae11a9d1825ab0028d01dc8c53/Advance-sample-report-card-final.pdf)

# 4. ref_genes.list[["pcgr"]] = list and PCGR annotation of mutated genes in given patient based on PCGR report

# 5. ref_genes.list[["purple"]] = list and PURPLE annotation of copy-number (CN) altered genes in given patient based on PURPLE results

# 6. ref_genes.list[["manta"]] = list and MANTA annotation of structural variants (SVs) with affected genes in given patient based on MANTA results

# 7. ref_genes.list[["pizzly"]] = list and PIZZLY annotation of gene fusion events detected in given patient based on PIZZLY results

# 8. ref_genes.list[["kallisto"]] = genes quantification file from KALLISTO This is required for gene fusion events filtering

# 9. ref_genes.list[["pizzly_manta"]] = list and annotation of gene fusion events detected in given patient based on both PIZZLY and MANTA results
```
...
exprTable <- function(genes, data, cn_data = NULL, sv_data = NULL, cn_decrease = TRUE, targets, sampleName, normal, cancer, genes_annot = NULL, OncoKB_genes = NULL, mut_annot = NULL, fusion_genes = NULL, ext_links = FALSE, type = "z") {
  ...
  if ( !is.null(OncoKB_genes) && length(genes) > 0 ) {
    ...
    group.z <- merge(group.z, OncoKB_genes, by.x="Gene", by.y="row.names", all = TRUE, sort = FALSE)
  }
  ...
  ##### Order the data by MANTA increasing score (to prioritise SVs, based on https://github.com/AstraZeneca-NGS/simple_sv_annotation/blob/master/simple_sv_annotation.py) and then by the highest absolute values for Patient vs Normal difference
...
fusionsBezierPlot <- function(fusion_annot, genes_annot) {
  ...
  ##### ... and the events type
  type <- paste( fusion_annot$pizzly.fusions.annot.fusions_abundant, fusion_annot$pizzly.fusions.annot.fusions_cancer, fusion_annot$geneA_dna_support, fusion_annot$geneB_dna_support, sep = "-") 
  type <- gsub("Yes-Yes-Yes-Yes","Abundant, cancer and DNA-supported gene(s)", type)
  type <- gsub("Yes-Yes---Yes","Abundant, cancer and DNA-supported gene(s)", type)
  type <- gsub("Yes-Yes-Yes--","Abundant, cancer and DNA-supported gene(s)", type)
  ...
  type <- gsub("--Yes-Yes-Yes","Cancer and DNA-supported gene(s)", type)
  type <- gsub("--Yes-Yes--","Cancer and DNA-supported gene(s)", type)
  type <- gsub("--Yes---Yes","Cancer and DNA-supported gene(s)", type)
...
##### Combine PMCC gene panel, OncoKB cancer genes and UMCCR cancer gene list
...
for (row in 1:nrow(pizzly.fusions)){
  if(pizzly.fusions[row,"geneA.name"] %in% ref_genes.list[["genes_cancer"]]$symbol | pizzly.fusions[row,"geneB.name"] %in% ref_genes.list[["genes_cancer"]]$symbol) {
...
pizzly.fusions <- cbind(pizzly.fusions, data.frame(matrix("", ncol = 4, nrow = nrow(pizzly.fusions)), stringsAsFactors = FALSE))
colnames(pizzly.fusions)[-c(1:7)] <- c("Reported", "Effector_gene", "Source", "Cancer_acronym")
...
for ( i in 1:nrow(pizzly.fusions) ) {
  ...
  geneA <- pizzly.fusions$geneA.name[i]
  geneB <- pizzly.fusions$geneB.name[i]
        
  ##### First check if the exact reported gene pairs were detected by pizzly
  if ( paste(geneA, geneB, sep="-") %in% trans.pairs ) {
    
    pizzly.fusions$Reported[i] <- trans.pairs[ trans.pairs %in% paste(geneA, geneB, sep="-") ]
    
    pizzly.fusions[ i, c("Effector_gene", "Cancer_acronym", "Source")] <- known_translocations[ trans.pairs %in% paste(geneA, geneB, sep="-"), c("effector_gene", "cancer_acronym", "source") ]
    
 } else if ( paste(geneB, geneA, sep="-") %in% trans.pairs ) {
    
    pizzly.fusions$Reported[i] <- trans.pairs[ trans.pairs %in% paste(geneB, geneA, sep="-") ]
    
    pizzly.fusions[ i, c("Effector_gene", "Cancer_acronym", "Source")] <- known_translocations[ trans.pairs %in% paste(geneB, geneA, sep="-"), c("effector_gene", "cancer_acronym", "source") ]
  ...
  ##### Now check is at lease one of the pizzly results genes was previously reporeted
    ...
    if ( geneA %in% known_translocations$geneA ) {
      ...
      pizzly.fusions$Reported[i] <- unique(known_translocations$geneA[ known_gene ])
      
      pizzly.fusions[ i, c("Cancer_acronym", "Source")] <- known_translocations[ known_gene, c("cancer_acronym", "source") ]
        
        ##### Flag if it's effector gene
        if ( geneA == known_translocations[ known_gene , "effector_gene" ]  ) {
          
          pizzly.fusions[ i, "Effector_gene"] <- as.character(geneA)
        }
    ...
    ##### Check pizzly genes A and genes B in reported fusions
      ...
      pizzly.fusions$Reported[i] <- unique(known_translocations$geneB[ known_gene ])
    ...
    if ( geneB %in% known_translocations$geneA ) {
      ...
      pizzly.fusions[ i, c("Cancer_acronym", "Source")] <- known_translocations[ known_gene, c("cancer_acronym", "source") ]
        ...
        if ( geneB == known_translocations[ known_gene , "effector_gene" ]  ) {
          ...
          pizzly.fusions[ i, "Effector_gene"] <- as.character(geneB)
        }
...
```{r fusions_reported_annot, comment = NA, message=TRUE, warning=FALSE}
...
pizzly.fusions.cut <- pizzly.fusions.cut[ , c("geneA.name", "geneB.name", "paircount", "splitcount", "geneA_dna_support", "geneB_dna_support", "fusions_abundant", "fusions_cancer", "Reported", "Effector_gene", "Source", "Cancer_acronym") ]

##### Provide links to the external resources (in the "Source" column)
pizzly.fusions.cut$Source <- gsub( "cgc", paste0("<a href='https://cancer.sanger.ac.uk/cosmic/fusion' target='_blank'>cgc</a>"), pizzly.fusions.cut$Source)
pizzly.fusions.cut$Source <- gsub( "biomarker", paste0("<a href='https://www.cancergenomeinterpreter.org/biomarkers' target='_blank'>biomarker</a>"), pizzly.fusions.cut$Source)
```
...
if ( length(manta_pizzly.fusions) > 0 ) {
  ...
  idx <- order(pizzly.fusions.cut$geneA_dna_support, pizzly.fusions.cut$geneB_dna_support, decreasing = TRUE)
...
NOTE: the `r nrow(counts) - nrow(y.filtered$counts)` genes with no/low expression are indicated in <span style="color:#808080">BLANK</span> cells with missing values in *expression summary tables* in [Mutated genes], [Cancer genes], [Structural variants], [Copy-number altered genes] and [Immune Response Markers] sections.
...
[PCGR](https://github.com/sigven/pcgr){target="_blank"} report for this sample `r if ( runPcgrChunk ) { c("is AVAILABLE") } else { c("is NOT AVAILABLE") }`. 
...
mut_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], OncoKB_genes = ref_genes.list[["oncokb_genes"]][, c("Oncogene", "TSG") ], ext_links = TRUE, type = "z")
...
mut_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], OncoKB_genes = ref_genes.list[["oncokb_genes"]], ext_links = TRUE, type = "perc")
...
Out of the `r nrow(ref_genes.list[["genes_cancer"]])` cancer genes the expression of **`r length(which(ref_genes.list[["genes_cancer"]]$symbol %in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]])))`** was reliably measured in patient's sample. The remaining `r length(which(ref_genes.list[["genes_cancer"]]$symbol %!in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]])))` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
cancer_genes.expr.z <- exprTable( genes = ref_genes.list[["genes_cancer"]]$symbol, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]], ext_links = TRUE, type = "z")
...
cancer_genes.expr.perc <- exprTable( genes = ref_genes.list[["genes_cancer"]]$symbol, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]], ext_links = TRUE, type = "perc")
...
The prioritisation of transcriptome-based [pizzly](https://github.com/pmelsted/pizzly){target="_blank"} results is performed in three steps.

1. In the first step, basic idea is to run [kallisto](https://github.com/pachterlab/kallisto){target="_blank"} to quantify the fusion transcripts (reported by [pizzly](https://github.com/pmelsted/pizzly){target="_blank"}) and 
select those which have a decent [Transcripts Per Kilobase Million](http://www.arrayserver.com/wiki/index.php?title=TPM){target="_blank"} (TPM) support.
  * Create a new index based on the transcriptome and the fusion transcripts identified by [pizzly](https://github.com/pmelsted/pizzly){target="_blank"}.
  * Run [kallisto](https://github.com/pachterlab/kallisto){target="_blank"} in normal quantification mode on the expanded index to quantify both normal transcripts 
  and fusions.
  * Select only the fusions with a high [TPM](http://www.arrayserver.com/wiki/index.php?title=TPM){target="_blank"} value as reported by previous step.

2. In the second step, fusion genes reported by [pizzly](https://github.com/pmelsted/pizzly){target="_blank"} (transcriptome-based evidence) are compared with fusion events reported by [manta](https://github.com/sigven/pcgr){target="_blank"} (DNA-based evidence), if available.

3. Finally, fusion events reported by [pizzly](https://github.com/pmelsted/pizzly){target="_blank"} are annotated with cancer genes from *[Peter Mac](https://www.petermac.org/){target="_blank"} comprehensive cancer (PMCC)* panel, *[OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"}* (listed [here](http://oncokb.org/#/cancerGenes){target="_blank"} and available [here](https://github.com/oncokb/oncokb-public/tree/master/data){target="_blank"}) and other resources summarised in *[UMCCR Cancer Genes](https://trello.com/c/7j3KFMiL/184-umccr-cancer-genes){target="_blank"}* trello card.
...
Out of the `r nrow(fusion_annot)` fusion event(s) <span style="color:#02d653">**`r nrow(fusion_annot[ fusion_annot$geneA_dna_support == "Yes" | fusion_annot$geneB_dna_support == "Yes" , ])`**</span>) involve **DNA-supported** fusion genes (see [Structural variants] section), <span style="color:#ff0000">**`r nrow(fusion_annot[ fusion_annot$pizzly.fusions.annot.fusions_abundant == "Yes" , ])`**</span> contain **highly abundant** fusion transcript(s) and <span style="color:#0000ff">**`r nrow(fusion_annot[ fusion_annot$pizzly.fusions.annot.fusions_cancer == "Yes" , ])`**</span> involve **[Cancer genes]**.
...
##### Provide link to OncoKB for genes A
for ( gene in pizzly.fusions.table$geneA.name ) {
  
    if ( gene %in% rownames(ref_genes.list[["oncokb_genes"]]) & ref_genes.list[["oncokb_genes"]][gene, "OncoKB"] == "Yes" ) {
        
      pizzly.fusions.table$geneA.name[ pizzly.fusions.table$geneA.name == gene ] <- paste0("<a href='http://oncokb.org/#/gene/", gene, "' target='_blank'>", gene, "</a>")
    }
}

##### Provide link to OncoKB for genes B
for ( gene in pizzly.fusions.table$geneB.name ) {
  
    if ( gene %in% rownames(ref_genes.list[["oncokb_genes"]]) & ref_genes.list[["oncokb_genes"]][gene, "OncoKB"] == "Yes" ) {
        
      pizzly.fusions.table$geneB.name[ pizzly.fusions.table$geneB.name == gene ] <- paste0("<a href='http://oncokb.org/#/gene/", gene, "' target='_blank'>", gene, "</a>")
    }
}

names(pizzly.fusions.table) <- c("Gene A", "Gene B", "Pair count", "Split count", "DNA support (gene A)", "DNA support (gene B)", "Abundant transcript(s)", "Cancer gene(s)", "Fusion gene(s)", "Effector gene", "Source", "Cancer acronym")

##### Present gene fusion events in a table
DT::datatable( data = pizzly.fusions.table, filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE) %>%
    ...
    DT::formatStyle( columns = colnames(pizzly.fusions.table) %in% "Abundant transcript(s)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'coral')) ) %>%
    DT::formatStyle( columns = colnames(pizzly.fusions.table) %in% "Cancer gene(s)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightblue')) ) %>%
    DT::formatStyle( columns = colnames(pizzly.fusions.table) %in% "DNA support (gene A)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) ) %>%
    DT::formatStyle( columns = colnames(pizzly.fusions.table) %in% "DNA support (gene B)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) )
```

<details>
<summary>Table legend</summary>
<font size="2">
Cells in <span style="color:#02d653">GREEN</span> indicate **DNA-supported** fusion genes (see [Structural variants] section), cells in <span style="color:#ff0000">RED</span> indicate **highly abundant** fusion transcript(s), and those hihglighted in <span style="color:#0000ff">BLUE</span> indicate fusions containing **[Cancer genes]**. Genes known to be involved in gene fusions are flagged based on information provided in [Cancer Genome Interpreter](https://www.cancergenomeinterpreter.org/biomarkers){target="_blank"} (CGI) database. Genes considered to be oncogenes or tumour suppressor genes are indicated by hyperlink to [OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"} database. Fusion events are ordered by **DNA support (gene A/B)**,  **Abundant transcript(s)**, **Cancer gene(s)**, **Split count** and **Pair count** columns.
...
* *biomarker*: gene with mutations known to be biomarkers of response to drugs (from [Cancer Genome Interpreter](https://www.cancergenomeinterpreter.org/biomarkers){target="_blank"})
* *cgc*: gene obtained from the [Cancer Gene Census](https://cancer.sanger.ac.uk/cosmic/fusion){target="_blank"}
...
Gene fusion events involving genes located within detected [Structural variants] (obtained from [Manta](https://github.com/sigven/pcgr){target="_blank"}) are presented in the genomic context. The table at the bottom contains genomic coordingates of individual fusion genes sorted based on their genomic location.

[Manta](https://github.com/sigven/pcgr){target="_blank"} results for this sample `r if ( runSVsChunk ) { c("are AVAILABLE") } else { c("are NOT AVAILABLE") }`. 

```{r bezier_plot_fusions, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3, eval = runSVsChunk}
##### Keep only fusions with abundant transcript(s) or cancer gene(s) involved
fusion_annot_top <- fusion_annot[ fusion_annot$geneA_dna_support == "Yes" | fusion_annot$geneB_dna_support == "Yes" , ]
...
if ( nrow(fusion_annot_top) > 0 ) {
  ...
  fusionsBezierPlot(fusion_annot = fusion_annot_top, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]])
} else {
...
```{r expr_table_fusions, comment = NA, message=FALSE, warning=FALSE, eval = runSVsChunk}
##### Clean the table for better presentation
fusion_annot_top.clean <- fusion_annot_top[, c("SYMBOL", "SEQNAME", "GENESEQSTART", "GENESEQEND", "SYMBOL.1", "SEQNAME.1", "GENESEQSTART.1", "GENESEQEND.1", "geneA_dna_support", "geneB_dna_support", "pizzly.fusions.annot.fusions_abundant", "pizzly.fusions.annot.fusions_cancer") ]
...
names(fusion_annot_top.clean) <- c("Gene A", "Chrom (A)", "Start (A)", "End (A)", "Gene B", "Chrom (B)", "Start (B)", "End (B)", "DNA support (gene A)", "DNA support (gene B)", "Abundant transcript(s)", "Cancer gene(s)")
...
DT::datatable( data = fusion_annot_top.clean, filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800,  escape = FALSE) %>%
    ...
    DT::formatStyle( columns = colnames(fusion_annot_top.clean) %in% "DNA support (gene A)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) ) %>%
    DT::formatStyle( columns = colnames(fusion_annot_top.clean) %in% "DNA support (gene B)", backgroundColor = DT::styleEqual(c("-", "Yes"), c('transparent', 'lightgreen')) )
```

<details>
<summary>Table legend</summary>
<font size="2">
Cells in <span style="color:#02d653">GREEN</span> indicate **DNA-supported** fusion genes (see [Structural variants] section), cells in <span style="color:#ff0000">RED</span> indicate **highly abundant** fusion transcript(s), and those hihglighted in <span style="color:#0000ff">BLUE</span> indicate fusions containing **[Cancer genes]**. Genes known to be involved in gene fusions are flagged based on information provided in [Cancer Genome Interpreter](https://www.cancergenomeinterpreter.org/biomarkers){target="_blank"} (CGI) database. Fusion events are ordered by **genomic coordinates** of **Gene A** and then **Gene B**.
...
Expression profiles for 5 gene fusion events involving DNA-supported fusion genes (see [Structural variants] section), abundant transcripts or cancer genes (see [Cancer genes] section), indicated in <span style="color:#02d653">green</span>, <span style="color:#ff0000">red</span> and <span style="color:#0000ff">blue</span> columns in the [Gene fusion events] table, respectively, and with the highest *Split count* and *Pair count* values. 

<span style="color:#ff0000">NOTE</span>: the *Fusion events visualisation* is not available for gene pairs for which no junctions were found by [clinker](https://github.com/Oshlack/Clinker){target="_blank"}.
...
```{r fusions_tables_1, comment = NA, message=TRUE, warning=FALSE, eval=FALSE}
...
```{r cdf_plot_fusions_1, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5, eval = runPizzlyChunk}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
...
```{r gene_table_fusions_1, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
...
exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "z")[[1]]
```
...
```{r gene_table_fusions_1_perc, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "perc")[[1]]
```
...
```{r fusions_tables_2, comment = NA, message=TRUE, warning=FALSE, eval=FALSE}
##### Present a table with number of reads supporting this fusion
##### Check if the fusion has abundant transcript(s)
fusion_reads.top <- fusion_reads[ fusion_reads$geneA.name == geneA & fusion_reads$geneB.name == geneB, ]
...
```{r cdf_plot_fusions_2, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5, eval = runPizzlyChunk}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
...
```{r gene_table_fusions_2, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
fusion.df <- ref_datasets.list[[tissue]][["gene_annot"]]
fusion.df <- rbind(fusion.df[fusion.df$SYMBOL==geneA, ], fusion.df[fusion.df$SYMBOL==geneB, ])
genes = fusion.df$SYMBOL
...
```{r gene_table_fusions_2_perc, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
exprTable( genes = genes, data = data, targets = targets, sampleName  = params$sample_name, normal  = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes  = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "perc")[[1]]
```
...
```{r fusions_tables_3, comment = NA, message=TRUE, warning=FALSE, eval=FALSE}
##### Present a table with number of reads supporting this fusion
##### Check if the fusion has abundant transcript(s)
fusion_reads.top <- fusion_reads[ fusion_reads$geneA.name == geneA & fusion_reads$geneB.name == geneB, ]
...
```{r cdf_plot_fusions_3, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5, eval = runPizzlyChunk}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
...
```{r gene_table_fusions_3, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
fusion.df <- ref_datasets.list[[tissue]][["gene_annot"]]
fusion.df <- rbind(fusion.df[fusion.df$SYMBOL==geneA, ], fusion.df[fusion.df$SYMBOL==geneB, ])
genes = fusion.df$SYMBOL
...
```{r gene_table_fusions_3_perc, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
exprTable( genes = genes, data = data, targets = targets, sampleName  = params$sample_name, normal  = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes  = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "perc")[[1]]
```
...
```{r fusions_tables_4, comment = NA, message=TRUE, warning=FALSE, eval=FALSE}
##### Present a table with number of reads supporting this fusion
##### Check if the fusion has abundant transcript(s)
fusion_reads.top <- fusion_reads[ fusion_reads$geneA.name == geneA & fusion_reads$geneB.name == geneB, ]
...
```{r cdf_plot_fusions_4, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5, eval = runPizzlyChunk}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
...
```{r gene_table_fusions_4, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
fusion.df <- ref_datasets.list[[tissue]][["gene_annot"]]
fusion.df <- rbind(fusion.df[fusion.df$SYMBOL==geneA, ], fusion.df[fusion.df$SYMBOL==geneB, ])
genes = fusion.df$SYMBOL
...
```{r gene_table_fusions_4_perc, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
exprTable( genes = genes, data = data, targets = targets, sampleName  = params$sample_name, normal  = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes  = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "perc")[[1]]
```
...
```{r fusions_figs_5, comment = NA, message=TRUE, warning=FALSE}
##### Convert the fusion image from pdf to png and present it in the report
...
```{r fusions_tables_5, comment = NA, message=TRUE, warning=FALSE, eval=FALSE}
##### Present a table with number of reads supporting this fusion
##### Check if the fusion has abundant transcript(s)
fusion_reads.top <- fusion_reads[ fusion_reads$geneA.name == geneA & fusion_reads$geneB.name == geneB, ]
...
```{r cdf_plot_fusions_5, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5, eval = runPizzlyChunk}
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode)
...
```{r gene_table_fusions_5, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
fusion.df <- ref_datasets.list[[tissue]][["gene_annot"]]
fusion.df <- rbind(fusion.df[fusion.df$SYMBOL==geneA, ], fusion.df[fusion.df$SYMBOL==geneB, ])
genes = fusion.df$SYMBOL
...
```{r gene_table_fusions_5_perc, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, normal = normal_group, cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], fusion_genes = fusion_annot_reported, ext_links = TRUE, type = "perc")[[1]]
```
...
mRNA expression levels of genes within detected structural variants (SVs), obtained from [Manta](https://github.com/sigven/pcgr){target="_blank"} structural variant caller, in patient's sample and their average mRNA expression in samples from cancer patients and healthy individuals.
...
sv_genes.expr.z <- exprTable( genes = genes, data = data, sv_data = manta_sv[ , names(manta_sv) %!in% c("lof", "SR", "PR") ], targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], ext_links = TRUE, type = "z")
...
##### Generate expression summary table for cancer genes from OncoKB and PMCC
sv_genes.expr.perc <- exprTable( genes = genes, data = data, sv_data = manta_sv[ , names(manta_sv) %!in% c("lof", "SR", "PR") ], targets = targets, sampleName = params$sample_name, normal = normal_group , cancer = cancer_group, genes_annot = ref_datasets.list[[tissue]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], OncoKB_genes = ref_genes.list[["oncokb_genes"]][,c("Oncogene", "TSG")], ext_links = TRUE, type = "perc")
...
<summary>Table legend</summary>
<font size="2">
The <span style="color:#ff0000">RED</span> colour range indicate relatively **high expression** (percentile) values and <span style="color:#0000ff">BLUE</span> colour range indicate relatively **low expression** (percentile) values in individual sample group. The **Patient vs Normal** column illustrates the difference between percentiles in patient sample and cohort of healthy individuals for each fusion gene. Genes considered to be oncogenes or tumour suppressor genes, according to [OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"} database, are also indicated. Genes are ordered by **increasing SV score** and then by **decreasing** absolute values in the **Patient vs Normal** columns. TSG - tumour suppressor gene; SD - standard deviation
...
[PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"} report for this sample `r if ( runPurpleChunk ) { c("is AVAILABLE") } else { c("is NOT AVAILABLE") }`.

### - Expression vs copy-number

Scatterplot comparing the per-gene **mRNA expression** values (*y-axis*, Z-scores), **CN values** (*x-axis*, from [PURPLE](https://anaconda.org/bioconda/hmftools-purple){target="_blank"}). If the mutation status information is available then the genes's colours correspond to the variant(s) consequence (from [PCGR](https://github.com/sigven/pcgr){target="_blank"}). Genes with CN values > 4 or < 0.5 are annotated. NOTE: only [Cancer genes] and [Mutated genes] with [tier 1-3](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} variants (if mutation data is available) are presented.
...
genes <- as.vector(data$Gene[ data$Gene %in% ref_genes.list[["genes_cancer"]]$symbol ])
...
if ( !is.null(ref_genes.list[["pcgr"]]) ) {
  ...
  mutCNexprPlot(data = data, mut_data = TRUE, cn_bottom = 0.5, cn_top = 4, plot_mode = params$plots_mode)
  
} else {
  
  data <- data[ data$Gene %in% genes, ]
  mutCNexprPlot(data = data, mut_data = FALSE, cn_bottom = 0.5, cn_top = 4, plot_mode = params$plots_mode)
}
...
Out of the `r length(ref_datasets.list[[tissue]][["expr_mut_cn_data"]][,1])` genes within gained (CN values > `r params$cn_gain`) or lossed (CN values < `r params$cn_loss`) regions the expression of **`r length(which(rownames(ref_datasets.list[[tissue]][["expr_mut_cn_data"]]) %in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]])))`** was reliably measured in patient's sample. The remaining `r length(which(rownames(ref_datasets.list[[tissue]][["expr_mut_cn_data"]]) %!in% rownames(ref_datasets.list[[tissue]][["combined_data_processed"]])))` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
```{r mut_cn_expr_data_table_gains, comment = NA, message=FALSE, warning=FALSE, eval = runPurpleChunk}
...
Expression profiles for 10 immune response markers with the greatest difference in standarised (Z-score) mRNA expression values between patient's sample and the average mRNA expression in samples from healthy individuals.
...
if ( !is.na(gene) && gene %in% rownames(data) ) {
  ...
  if ( is.na(gene) ) {
    cat(paste("There are", length(immune_genes.expr.z[[2]]$SYMBOL), "immune response marker(s) detected!", sep=" "))
  } else if ( gene %!in% rownames(data) ) {
    cat(paste("No expression data is available for", gene, "!", sep=" "))
  }
}
...
List of drugs targeting variants in identified [Mutated genes], dysregulated [Cancer genes], [Gene fusion events], [Structural variants]-affected genes and [Copy-number altered genes], which can be considered in the treatment decision making process. The clinically actionable aberrations are matched based on information provided by *[clinical interpretations of variants in Cancer](https://civicdb.org/home){target="_blank"}* (CIViC) ([Griffith et al. (2017)](https://www.ncbi.nlm.nih.gov/pubmed/28138153){target="_blank"}). The evidence pertaining to variants effect on therapeutic response is also provided.
...
**`r nrow(fusion_annot_top.clean)`** gene fusion events involving DNA-supported fusion genes (see [Structural variants] section), abundant transcripts or cancer genes (see [Cancer genes] section), indicated in <span style="color:#02d653">green</span>, <span style="color:#ff0000">red</span> and <span style="color:#0000ff">blue</span> columns in the [Gene fusion events] table, respectively, and with the highest *Split count* and *Pair count* values were screened for suitable drugs.
...
data <- ref_datasets.list[[tissue]][["combined_data_processed"]]
genesA <- as.vector(pizzly.fusions[ pizzly.fusions$fusions_abundant == "Yes", ]$geneA.name)
genesB <- as.vector(pizzly.fusions[ pizzly.fusions$fusions_abundant == "Yes", ]$geneB.name)

civicDrugTable(genes = unique(c(genesA, genesB)), civic_var_summaries  = caner_genes_annot.list[["civic_var_summaries"]], civic_clin_evid  = caner_genes_annot.list[["civic_clin_evid"]],  evid_type = "Predictive", var_type = "fusion")[[1]]
```
...
```{r drugs_predictive_fusion_genes_cancer, comment = NA, message=FALSE, warning=FALSE, eval = runPizzlyChunk}
##### Generate table with drugs targeting fusion genes
genesA <- as.vector(pizzly.fusions[ pizzly.fusions$fusions_cancer == "Yes", ]$geneA.name)
genesB <- as.vector(pizzly.fusions[ pizzly.fusions$fusions_cancer == "Yes", ]$geneB.name)
...
**`r if ( runSVsChunk ) { length(unique(manta_sv$gene)) } else { cat("0") }`** genes affected by structural variants (SVs) were screened for suitable drugs (see [Structural variants] section).
...
civicDrugTable(genes, civic_var_summaries  = caner_genes_annot.list[["civic_var_summaries"]], civic_clin_evid  = caner_genes_annot.list[["civic_clin_evid"]],  evid_type = "Predictive", var_type = NULL)[[1]]
```
...
**`r length(ref_datasets.list[[tissue]][["expr_mut_cn_data"]][,1])`** genes with CN values > `r params$cn_gain` (**gains**) and  or < `r params$cn_loss` (**losses**) were screened for suitable drugs (see [Copy-number altered genes] section).
