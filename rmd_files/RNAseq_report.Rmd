...
output:
  ...
  rmdformats::material:
    highlight: kate
params:
  report_dir: '~/Documents/UMCCR/Misc'
  sample_name: 'P006'
  count_file: '~/Documents/UMCCR/data/RNAseq-report/CCR180038_SV18T002P006_RNA/CCR180038_SV18T002P006_RNA-ready.counts'
  tissue: 'pancreas'
  ref_data_dir: '../data'
...
# 1. ref_datasets.list[[tissue]][[1]] = combined read count data (reference datasets + sample data) ("combineDatasets" function output in the "load_ref_data chunk")

# 2. ref_datasets.list[[tissue]][[2]] = combined data samples annotation ("combineDatasets" function output in the "load_ref_data chunk")

# 3. ref_datasets.list[[tissue]][[3]] = transformed, filtered and normalised data (see "data_transformation" and "data_normalisation" chunks)

# 4. ref_datasets.list[[tissue]][[4]] = PCA results

# 5. ref_datasets.list[[tissue]][[5]] = transformed, filtered and normalised data annotated with "SYMBOL", "GENEBIOTYPE", "ENSEMBL", "ENTREZID", "SEQNAME", "GENESEQSTART", "GENESEQEND"

# 6. ref_datasets.list[[tissue]][[6]] = combined expression, mutation and copy-number data
...
exprTable <- function(genes, data, targets, sampleName, normal, cancer, genes_annot = NULL, OncoKB_genes = NULL, tiers = NULL) {
  ...
  #### Add tier information to the expression table
  if(!is.null(tiers)){
  tiers <- tiers[tiers$SYMBOL %in% genes,]
  tiers$TIER <- gsub("TIER ", "", tiers$TIER)
  tiers$TIER <- as.numeric(tiers$TIER)
  #tiers <- tiers[order(tiers$TIER), ]
  #### keep only genes that has the lowest tier value. Applies to the ones that may have multiple mutaions and hence tiers
  tiers <- tiers[!duplicated(tiers$SYMBOL),]  
  rownames(tiers) <- tiers$SYMBOL
  #### merege the tier information with the dataframe
  group.z <- merge(group.z, tiers, by = "row.names", all = FALSE, sort = FALSE)
  group.z <- group.z[, colnames(group.z) %!in% "SYMBOL"]
  group.z <- group.z[order(group.z$TIER, group.z$`Patient vs Normal`),]
  }
...
##### Generate a plot with genome-wide expression profile (Z-score) for each group
exprProfilePlot <- function(genes, data, targets, sampleName, normal, cancer, genes_annot) {
    
    targets.list <- unique(targets$Target)
    
    ##### Keep only gene for which both genes have gene symbol available
    genes <- genes[ genes %in% genes_annot$SYMBOL ]
    genes <- unique(genes)
    
    ##### Initiate list for expression mean values in each group
    group.z <- sapply(targets.list,function(x) NULL)
    
    for ( group in targets.list ) {
      
      group.z[[group]] <- as.data.frame(matrix(NA, ncol = 2, nrow = nrow(data)))
      rownames(group.z[[group]]) <- sort(rownames(data))
      
      group.stats <- exprGroupStats(data[rownames(data), ], targets, group)
      group.stats <- group.stats[order(rownames(group.stats)), ]
      group.z[[group]][,1] <- round(group.stats$z, digits=2)
      names(group.z[[group]]) <- c("Upregulated", "Downregulated")
    }
    ...
    targets.list <- c("Patient", cancer, "Normal")
    
    ##### Merge expression data with gene genomic coordinates info
    for ( group in targets.list ) {
      
      group.z[[group]] <- merge(genes_annot, group.z[[group]], by = "row.names", all.x = FALSE)
      group.z[[group]] <- group.z[[group]][, -1]
      
      ###### Change chromosomes X and Y to numbers 23 and 24, respectively
      group.z[[group]]$SEQNAME[ group.z[[group]]$SEQNAME  %in% "X" ] <- 23
      group.z[[group]]$SEQNAME[ group.z[[group]]$SEQNAME %in% "Y" ] <- 24
      
      ##### Order the data accordingly to genomic postions
      group.z[[group]] = group.z[[group]][order(as.numeric(group.z[[group]]$SEQNAME),as.numeric(group.z[[group]]$GENESEQSTART)),]
    }
    ##### Label chromosomes
    chr_bins <- data.matrix(summary(as.factor(group.z[["Patient"]]$SEQNAME)))
    chr_bins<- data.frame(chr_bins,row.names(chr_bins) )
    colnames(chr_bins) = c("count","Chromosome")
    
    data.chr_bins <- data.frame(unique(group.z[["Patient"]]$SEQNAME))
    ...
    chr_nos <- list( x = chr_annot[, 3], y = min(group.z[["Patient"]]$Upregulated), text = chr_annot$Chromosome, xref = "x", yref = "y", showarrow = FALSE )

    ##### Prepare vector to indicate chromosome boundaries
    chr_lines <- rep(0, nrow(group.z[["Patient"]]))
    chr_lines[ chr_annot$count - 1 ] <- max(group.z[["Patient"]]$Upregulated)
    chr_lines[ chr_annot$count ] <- min(group.z[["Patient"]]$Upregulated)
 
    ##### Initiate list for data to be ploted for each group
    data2plot <- sapply(targets.list,function(x) NULL)
    
    ##### Prepare data for plotting with plotly
    for ( group in targets.list ) {
      
      data2plot[[group]] <- data.frame(group.z[[group]][ , c("SYMBOL", "Upregulated", "Downregulated" )], chr_lines)
      ##### Set the expression value of genes not of interest to 0
      data2plot[[group]]$Upregulated[ data2plot[[group]]$SYMBOL %!in% genes ] <- 0
      
      data2plot[[group]]$GOI <- data2plot[[group]]$SYMBOL
      data2plot[[group]]$GOI[ data2plot[[group]]$SYMBOL %!in% genes ] <- ""
      
      ##### Distinguish up- and dwon-regulated genes and report them in separate columns (for plotly)
      data2plot[[group]]$Downregulated <- data2plot[[group]]$Upregulated
      data2plot[[group]]$Upregulated[ data2plot[[group]]$Upregulated < 0 ] <- 0
      data2plot[[group]]$Downregulated[ data2plot[[group]]$Downregulated >= 0 ] <- 0
      
      names(data2plot[[group]])[names(data2plot[[group]])=="SYMBOL"] <- "Gene"
      
      ##### Use the genomic positions order for bars
      data2plot[[group]]$Gene <- factor(data2plot[[group]]$Gene, levels = data2plot[[group]]$Gene)
    }
    
    ##### Generate genome-wide plot of expression (Z-score) values for Patient
    p1 <- plot_ly(data2plot[["Patient"]], x = ~Gene, y = ~Upregulated, type = 'bar', name = "Upregulated", color = I("red"), text = ~GOI, textposition = 'bottom center', textfont = list( size = 10, color = "black"), constraintext = 'outside', width = 800, height = 400) %>%
        add_trace(y = ~Downregulated, name = "Downregulated", type = 'bar', color = I("blue"), text = ~GOI, textposition = 'top center', textfont = list( size = 10, color = "black"), constraintext = 'outside') %>%
        add_trace(x = ~Gene, y = ~chr_lines, type = 'scatter', name = "Chromosomes", mode = 'lines', line = list(color = "lightgrey", dash = "1px"), hoverinfo = "skip") %>%
        layout( title = "Patient", yaxis = list(title = "mRNA expression (Z-score)"), xaxis = list(title = "Chromosome", showticklabels = FALSE), annotations = chr_nos, barmode = 'group', bargap = 0, margin = list(l=50, r=50, b=100, t=50, pad=4), autosize = F, legend = list(orientation = 'h', y = 1.07, tracegroupgap=0), showlegend=FALSE)
    
    ##### Generate genome-wide plot of expression (Z-score) values for cancer group
    p2 <- plot_ly(data2plot[[targets.list[2]]], x = ~Gene, y = ~Upregulated, type = 'bar', name = "Upregulated", color = I("red"), text = ~GOI, textposition = 'bottom center', textfont = list( size = 10, color = "black"), constraintext = 'outside', width = 800, height = 400) %>%
        add_trace(y = ~Downregulated, name = "Downregulated", type = 'bar', color = I("blue"), text = ~GOI, textposition = 'top center', textfont = list( size = 10, color = "black"), constraintext = 'outside') %>%
        add_trace(x = ~Gene, y = ~chr_lines, type = 'scatter', name = "Chromosomes", mode = 'lines', line = list(color = "lightgrey", dash = "1px"), hoverinfo = "skip") %>%
        layout( title = targets.list[2], yaxis = list(title = "mRNA expression (Z-score)"), xaxis = list(title = "Chromosome", showticklabels = FALSE), annotations = chr_nos, barmode = 'group', bargap = 0, margin = list(l=50, r=50, b=100, t=50, pad=4), autosize = F, legend = list(orientation = 'h', y = 1.07, tracegroupgap=0), showlegend=FALSE)
    
    ##### Generate genome-wide plot of expression (Z-score) values for normal group
    p3 <- plot_ly(data2plot[[targets.list[3]]], x = ~Gene, y = ~Upregulated, type = 'bar', name = "Upregulated", color = I("red"), text = ~GOI, textposition = 'bottom center', textfont = list( size = 10, color = "black"), constraintext = 'outside', width = 800, height = 400) %>%
        add_trace(y = ~Downregulated, name = "Downregulated", type = 'bar', color = I("blue"), text = ~GOI, textposition = 'top center', textfont = list( size = 10, color = "black"), constraintext = 'outside') %>%
        add_trace(x = ~Gene, y = ~chr_lines, type = 'scatter', name = "Chromosomes", mode = 'lines', line = list(color = "lightgrey", dash = "1px"), hoverinfo = "skip") %>%
        layout( title = targets.list[3], yaxis = list(title = "mRNA expression (Z-score)"), xaxis = list(title = "Chromosome", showticklabels = FALSE), annotations = chr_nos, barmode = 'group', bargap = 0, margin = list(l=50, r=50, b=100, t=50, pad=4), autosize = F, legend = list(orientation = 'h', y = 1.07, tracegroupgap=0), showlegend=FALSE)
...
if ( file.exists(paste(dataDir, "pcgr/pcgr-somatic.csv", sep = "/")) ) {
  ...
  ##### Consider only mutations calssified within Tiers 1-3
...
for ( tissue in names(ref_datasets.list) ) {
  ...
  counts <- ref_datasets.list[[tissue]][[1]]
  target <- ref_datasets.list[[tissue]][[2]]
  ...
  ref_datasets.list[[tissue]][[3]] <- y.filtered
}
...
for ( tissue in names(ref_datasets.list) ) {
  ...
  target <- ref_datasets.list[[tissue]][[2]]
  data <- ref_datasets.list[[tissue]][[3]]
  
  ref_datasets.list[[tissue]][[4]] <- pca(data, target)
}
```
...
for ( tissue in names(ref_datasets.list) ) {
  ...
  ref_datasets.list[[tissue]][[3]] <- apply(data.annot[, colnames(data)], 2, as.numeric)
  rownames(ref_datasets.list[[tissue]][[3]]) <- data.annot$SYMBOL
  ref_datasets.list[[tissue]][[5]] <- data.annot[, c("SYMBOL", "GENEBIOTYPE", "ENSEMBL", "ENTREZID", "SEQNAME", "GENESEQSTART", "GENESEQEND")]
}
```
...
expr_data <- ref_datasets.list[[tissue]][[3]]
...
mut_data$CONSEQUENCE <- gsub("_variant", "", mut_data$CONSEQUENCE)
mut_data$CONSEQUENCE <- gsub("_", " ", mut_data$CONSEQUENCE)
...
#cn_data.sub <- cn_data[ cn_data$MaxCopyNumber < cn_data.percent[2] | cn_data$MinCopyNumber > #cn_data.percent[20], ]

##### Keep only altered genes with CN values below 1 (loss) and above 4 (gain)
cn_data.sub <- cn_data[ cn_data$MeanCopyNumber < 1 | cn_data$MeanCopyNumber > 4, ]

##### Subset expression, mutation and copy-number data to include only overlapping genes
genes.intersect <- intersect(intersect(rownames(gene.mut), cn_data.sub$Gene), names(expr_data.z))

gene.mut.sub <- gene.mut[ rownames(gene.mut) %in% genes.intersect, ]
cn_data.sub <- cn_data.sub[ cn_data.sub$Gene %in% genes.intersect, ]
expr_data.z.sub <- expr_data.z[ names(expr_data.z) %in% genes.intersect ]

##### Prepare data frame
ref_datasets.list[[tissue]][[6]] <- data.frame(names(expr_data.z.sub), cn_data.sub$MeanCopyNumber, expr_data.z.sub, gene.mut.sub)
colnames(ref_datasets.list[[tissue]][[6]]) <- c("Gene", "CN", "mRNA", "Mutation")
```
...
fusion_genes_annot <- ref_datasets.list[[tissue]][[5]][ , c("SYMBOL", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND") ]
...
* **`r paste (ref_normal, normal_group, sep=" ")`** samples from [The Genotype-Tissue Expression](https://commonfund.nih.gov/gtex){target="_blank"} (GTEx) program
* **`r paste (ref_cancer, cancer_group, sep=" ")`** samples from [The Cancer Genome Atlas](https://cancergenome.nih.gov/abouttcga){target="_blank"} (TCGA) project
...
mRNA expression levels of genes containing single nucleotide variants (SNVs) or insertions/deletions (indels), obtained from the [PCGR](https://github.com/sigven/pcgr){target="_blank"} report, in patient's sample and their average mRNA expression in samples from cancer patients and healthy individuals.

### Expression summary

From the `r length(unique(ref_genes.list[["pcgr"]]$SYMBOL))` mutated genes the expression of **`r length(which(unique(ref_genes.list[["pcgr"]]$SYMBOL) %in% rownames(ref_datasets.list[[ref]][[3]])))`** was reliably measured in patient's sample. NOTE, only PCGR [tier 1-3](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} variants are reported.
...
mut_genes.expr <- exprTable(unique(ref_genes.list[["pcgr"]]$SYMBOL), data, target, params$sample_name, normal_group , cancer_group, ref_datasets.list[[tissue]][[5]][, c("SYMBOL", "ENSEMBL")], ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER")])
...
<span style="color:#ff0000">NOTE</span>: currently, displaying the top 10 mutated genes with the greatest difference in standarised (Z-score) mRNA expression values between patient's sample and the average mRNA expression in samples from healthy  individuals.
...
cdfPlot(gene, data, target, params$sample_name, normal_group, cancer_group, addBoxPlot = TRUE)
...
## Copy-number altered genes {.tabset}

### Plot

<span style="color:#ff0000">NOTE</span>: the work on this section is in progress. The aim of this section is to overlay the per-gene somatic copy-number variants (CNVs) with mRNA expression data and, where available, mutation status. The scatterplot compares the per-gene **mRNA expression** values (**y-axis**, Z-score) with **CN values** (**x-axis**, from [PURPLE](https://anaconda.org/bioconda/hmftools-purple)). If the mutation status information is available then the genes's colours correspond to the variant(s) consequence (from [PCGR](https://github.com/sigven/pcgr)).

```{r mut_cn_expr_data_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 3}
suppressMessages(library(plotly))
##### Generate scatterplot with per-gene expression values (y-axis), CN values (x-axis) and, where available, mutation status info (colours)
plot_ly(ref_datasets.list[[tissue]][[6]], x = ~CN, y = ~mRNA, color = ~Mutation, text=~Gene, type='scatter', mode = "markers", marker = list(size=10, symbol="circle"), width = 800, height = 600) %>%
...
<span style="color:#ff0000">NOTE</span>: to be added.
...
From the `r nrow(ref_genes.list[["oncokb_genes"]])` cancer genes the expression of **`r length(which(rownames(ref_genes.list[["oncokb_genes"]]) %in% rownames(ref_datasets.list[[ref]][[3]])))`** was reliably measured in patient's sample.
...
cancer_genes.expr <- exprTable(rownames(ref_genes.list[["oncokb_genes"]]), data, target, params$sample_name, normal_group , cancer_group, ref_datasets.list[[tissue]][[5]][, c(1,3)], ref_genes.list[["oncokb_genes"]])
...
# #create a nice table output (with kableExtra, lack of results filtering)
...
#   group_rows("Gene fusion events involving highly abundant transcripts", 1, nrow(deduped.result), label_row_css = "background-color: #E74C3C; color: #fff;") %>%
#   group_rows("Gene fusion events involving cancer genes", (nrow(deduped.result)+1), (nrow(deduped.result) + nrow(result.cancer_genes)), label_row_css = "background-color: #2980B9; color: #fff;") %>%
#   group_rows("Remaining gene fusion events", (nrow(deduped.result) + nrow(result.cancer_genes) +1), nrow(pizzly.fusions2), label_row_css = "background-color: #27AE60; color: #fff;") %>%
...
DT::datatable( data = pizzly.fusions.clean, filter="none", rownames = FALSE, options = list(pageLength = 10, scrollX = TRUE), width = 800,  escape = FALSE) %>%
...
##### Generate bezier curves-like plot representing gene fusion events
fusionsBezierPlot(fusion_annot_top, ref_datasets.list[[tissue]][[5]])
```
...
DT::datatable( data = fusion_annot_top.clean, filter="none", rownames = FALSE, options = list(pageLength = 10, scrollX = TRUE), width = 800,  escape = FALSE) %>%
...
cdfPlot(geneA, data, target, params$sample_name, normal_group, cancer_group)
cdfPlot(geneB, data, target, params$sample_name, normal_group, cancer_group)
...
fusion.df <- ref_datasets.list[[tissue]][[5]]
fusion.df <- rbind(fusion.df[fusion.df$SYMBOL==geneA, ], fusion.df[fusion.df$SYMBOL==geneB, ])
exprTable(fusion.df$SYMBOL, data, target, params$sample_name, normal_group, cancer_group, ref_datasets.list[[tissue]][[5]][, c(1,3)], ref_genes.list[["oncokb_genes"]][,c(1,2,4)])[[1]]
```

<details>
<summary>Table legend</summary>
<font size="2">
The <span style="color:#ff0000">RED</span> colour range indicate relatively **high expression** (Z-score) values and <span style="color:#0000ff">BLUE</span> colour range indicate relatively **low expression** (Z-score) values in individual sample group. The **Patient vs Normal** column illustrates the difference between Z-scores in patient sample and cohort of healthy individuals for each fusion gene. Genes considered to be oncogenes or tumour suppressor genes, according to [OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"} database, are also indicated. Genes are ordered by decreasing values in the "Patient vs Normal" column. TSG - tumour suppressor gene; SD - standard deviation
</font>
</details>

***

<!--This step is skipped for the moment, it's rather redundant.

<details>
<summary>Expression level - genomic view</summary>

```{r expr_plot_fusions_1, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 2.5}
suppressMessages(library(plotly))
##### Generate plots with genome-wide expression profile (Z-score) for each group
fuiosnExprProfile <- exprProfilePlot(c(as.vector(geneA), as.vector(geneB)), data, target, params$sample_name, normal_group, cancer_group, ref_datasets.list[[tissue]][[5]])
...
immune_genes.expr <- exprTable(unlist(ref_genes.list[["genes_immune"]]), data, target, params$sample_name, normal_group , cancer_group, ref_datasets.list[[tissue]][[5]][, c(1,3)])
...
<span style="color:#ff0000">NOTE</span>: currently, only the top 50 cancer genes with the greatest difference in standarised (Z-score) mRNA expression values between patient's sample and the average mRNA expression in samples from healthy  individuals (see [Cancer genes expression profiles] section) were screened for suitable drugs.
