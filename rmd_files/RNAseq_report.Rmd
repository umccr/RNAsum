...
exprTable <- function(genes, data, cn_data = NULL, sv_data = NULL, cn_decrease = TRUE, targets, sampleName, int_cancer, ext_cancer, comp_cancer, genes_annot = NULL, cancer_genes = NULL, mut_annot = NULL, fusion_genes = NULL, ext_links = FALSE, type = "z", scaling = "gene-wise") {
  ...
  names(group.z)[ncol(group.z)] <- paste0("Patient vs ", comp_cancer)
  ...
  if ( length(genes.absent) > 0 ) {
    ...
    NAs.df[ names(NAs.df) %in% paste0("Patient vs ", comp_cancer) ] <- 0
  ...
  ##### Reorder groups
  group.z <- cbind(group.z[ , c(ext_cancer, int_cancer, "Patient")], group.z[, c("SD", paste0("Patient vs ", comp_cancer) )])
  ...
  for ( group in c(targets.list, paste0("Patient vs ", comp_cancer)) ) {
  ...
  if ( !is.null(cn_data) ) {
    ...
    ##### Get the position of "Patient vs [comp_cancer]" column
    col_idx <- grep(paste0("Patient vs ", comp_cancer), names(group.z), fixed = TRUE)
    
    ##### Now place the CN data after the "Patient vs [comp_cancer]" column
    if ( length(genes) > 0 ) {
  ...
  if ( ext_links && length(genes) > 0 ) {
    ...
    ##### Place the external links after the "Patient vs [comp_cancer]" column
    ##### Get the position of "Patient vs [comp_cancer]" column
    col_idx <- grep(paste0("Patient vs ", comp_cancer), names(group.z), fixed = TRUE)
  ...
  if ( !is.null(cn_data) && length(genes) > 0 ) {
    ...
    group.z <- group.z[ order(abs(group.z[, paste0("Patient vs ", comp_cancer)]),  decreasing = TRUE), ]
    group.z <- group.z[ order(group.z[ ,col_idx ],  decreasing = cn_decrease), ]
    
  ##### Order the data by increasing TIER category (to allow filtering based on tier information) and then by the highest absolute values for Patient vs [comp_cancer] difference (to allow filtering based on z-score differences)
  ...
  if ( !is.null(cn_data) ) {
    ...
    dt.table <- DT::datatable( data = group.z[, names(group.z) %!in% "SYMBOL"], filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE) %>%
      ...
      DT::formatStyle(columns = paste0("Patient vs ", comp_cancer), 
                      backgroundColor = DT::styleInterval(brks.q[[paste0("Patient vs ", comp_cancer)]], clrs.q[[paste0("Patient vs ", comp_cancer)]])) %>%
...
##### Get the Patient vs [comp_cancer] Z-scores using exprTable funstion
...
The <span style="color:#ff0000">RED</span> colour range indicate relatively **high expression** (percentile) values and <span style="color:#0000ff">BLUE</span> colour range indicate relatively **low expression** (percentile) values in individual sample group. The **Patient vs `r comp_cancer_group`** column illustrates the difference between percentiles in patient sample and reference cancer cohort for each fusion gene. Genes considered to be oncogenes or tumour suppressor genes, according to [OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"} database, are also indicated. Genes are ordered by **decreasing** absolute values in the **Patient vs `r comp_cancer_group`** column. TSG - tumour suppressor gene; SD - standard deviation across groups
...
The <span style="color:#ff0000">RED</span> colour range indicate relatively **high expression** (percentile) values and <span style="color:#0000ff">BLUE</span> colour range indicate relatively **low expression** (percentile) values in individual sample group. The **Patient vs `r comp_cancer_group`** column illustrates the difference between percentiles in patient sample and reference cancer cohort for each gene. Genes considered to be oncogenes or tumour suppressor genes, according to [OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"} database, are also indicated. Genes are ordered by **increasing SV score** and then by **decreasing** absolute values in the **Patient vs `r comp_cancer_group`** columns. TSG - tumour suppressor gene; SD - standard deviation across groups
