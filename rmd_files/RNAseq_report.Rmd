...
# 5. ref_datasets.list[[dataset]][["pca"]] = PCA results

# 6. ref_datasets.list[[dataset]][["gene_annot_all"]] = gene annotation for combined read count data, containing all input genes. The annotation includes "SYMBOL", "GENEBIOTYPE", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND". "ENSEMBL" is used for rownames

# 7. ref_datasets.list[[dataset]][["gene_annot"]] = gene annotation for transformed, filtered and normalised data. The annotation includes "SYMBOL", "GENEBIOTYPE", "ENSEMBL", "SEQNAME", "GENESEQSTART", "GENESEQEND". "SYMBOL" is used for rownames

# 8. ref_datasets.list[[dataset]][["expr_mut_cn_data"]] = combined expression, mutation and copy-number data
...
pca <- function(data, targets, plot_mode = "static" ) {
  ...
  pca_plot <- plot_ly(data.subset_pca.df, x = ~PC1, y = ~PC2, color = ~Target, text=paste(target$Target, rownames(data.subset_pca.df), sep=": "), colors = targets.colour[[1]], type='scatter', mode = "markers", marker = list(size=10, opacity = 0.7), width = 800, height = 400) %>%
...
##### Calculate sample-wise mean, sd, quantiles and cumulative franctions for expression data
exprGroupsStats_sampleWise <- function(data, targets) {
  ...
  ##### Perform the sample-wise calculations across all groups
  ...
  for ( group in targets.list ) {
    ...
    ##### For groups with > 1 sample get the mean values for each gene
    if ( sum(c(targets$Target %in% group), na.rm = TRUE) > 1 && nrow(data) > 1 )  {
      
      ##### Extract the mean expression values
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], rowMeans(data[ , colnames(data)[ targets$Target %in% group ] ]))
      ...
      ##### Extract the mean Z-scores
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], rowMeans(data.z[ , colnames(data)[ targets$Target %in% group ] ]))

      ##### Extract the mean percentiles
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], rowMeans(data.q[ , colnames(data)[ targets$Target %in% group ] ]))
      
      ##### Extract the cumulative fraction corresponding to the mean Z-score
      ##### First, need to get the position of the Z-score nearest to the mean Z-score, and then extract the cumulative value at this position
      data.z.mean_pos <- apply(data.z, 1, nearest_position, mean(data.z[ , colnames(data)[ targets$Target %in% group, drop = FALSE ] ]))
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], data.cum[ data.z.mean_pos ] )
      
      group_stats.list[[group]] <- as.data.frame(group_stats.list[[group]])
      names( group_stats.list[[group]] ) <- c("mean", "sd", "z", "quantile", "cum")
      
    } else if ( sum(c(targets$Target %in% group), na.rm = TRUE) > 1 && nrow(data) == 1 ) {
      
      ##### Extract the mean expression values
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], mean(data[ , colnames(data)[ targets$Target %in% group, drop = FALSE ] ]))

      ##### Extract the expression sd values
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], sd(data[ , colnames(data)[ targets$Target %in% group, drop = FALSE ] ]))
      
      ##### Extract the mean Z-scores
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], mean(data.z[ , colnames(data)[ targets$Target %in% group, drop = FALSE ] ]))

      ##### Extract the mean percentiles
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], mean(data.q[ , colnames(data)[ targets$Target %in% group, drop = FALSE ] ]))
      
      ##### Extract the cumulative fraction corresponding to the mean Z-score
      ##### First, need to get the position of the Z-score nearest to the mean Z-score, and then extract the cumulative value at this position
      data.z.mean_pos <- nearest_position( data.z, mean(data.z[ , colnames(data)[ targets$Target %in% group, drop = FALSE ] ]))
      group_stats.list[[group]] <- cbind(group_stats.list[[group]], data.cum[ data.z.mean_pos ] ) 
      ...
      ##### Extract the mean cumulative fraction
  ...
  for ( group in targets.list ) {
    ...
    gene_stats.list[[group]]$mean <- data[ , colnames(data)[ targets$Target %in% group ], drop = FALSE ]
...
##### Calculate gene-wise mean, sd, quantiles and cumulative franctions for expression data from specific sample group
exprGroupStats_geneWise <- function(data, targets, target) {
  ...
  names(data.group.df) <- c("mean", "sd", "z", "quantile", "cum")
...
cdfPlot <- function(gene, data, targets, sampleName, int_cancer, ext_cancer, addBoxPlot = FALSE, plot_mode = "static", scaling = "sample-wise") {
  ...
  ##### ... from sample-wise approach 
  if ( scaling == "sample-wise" ) {

    ##### Get stats for each group
    gene.data <- data[ gene, , drop = FALSE]
    group.z.gene <- exprGroupsStats_sampleWise(gene.data, targets)[[1]]
    
    ##### ... and for each sample in individual groups
    gene.stats <- exprGroupsStats_sampleWise(gene.data, targets)[[2]]

    for ( group in targets.list ) {
        
        group.z[[ group]] <- cbind(t(gene.stats[[ group]]$mean), t(gene.stats[[ group]]$z), t(gene.stats[[ group]]$q), t(gene.stats[[ group]]$cum) )
        group.z[[ group]] <- as.data.frame(group.z[[ group]])
        colnames(group.z[[ group]]) <- c("mean", "z", "quantile", "cum")
    }
    
    group.z[[ sampleName ]] <- do.call("rbind", group.z)
    
  ##### ... or from gene-wise approach
  } else {

    group.z[[ sampleName ]] <- exprGroupStats_geneWise(data, targets, sampleName)
    group.z[[ int_cancer ]] <- exprGroupStats_geneWise(data, targets, int_cancer)
    group.z[[ ext_cancer ]] <- exprGroupStats_geneWise(data, targets, ext_cancer)
  ...
  p1 <- plot_ly(group.z[[ sampleName ]], x = ~z, color = I("black")) %>%
    ...
    add_markers(y = group.z.gene[[ ext_cancer ]]$cum, x =  group.z.gene[[ ext_cancer ]]$z,
                ...
                name = ext_cancer,
                marker = list(size = 12, opacity = 0.1, color = "cornflowerblue"),
                showlegend = TRUE) %>%
  
    add_lines(y = group.z[[ ext_cancer ]]$cum, x = group.z[[ ext_cancer ]]$z, opacity = 0.1,
...
glanceExprPlot <- function(genes, data, targets, sampleName, int_cancer, ext_cancer, plot_mode = "static", hexcode, scaling = "sample-wise") {
  ...
  ##### Sort genes by the highest values in the "int_cancer" cohort
  int_cancer.medians <- rowMedians( data.z[ genes ,targets$Target==int_cancer ] )
  names(int_cancer.medians) <- genes
  genes <- genes [ order(int_cancer.medians) ]
...
mutCNexprPlot <- function(data, mut_data = FALSE, cn_bottom = 1.5, cn_top = 3, int_cancer, plot_mode = "static") {
  ...
  ##### Generate scatterplot with per-gene expression values (y-axis) (difference between Patient's and [int_cancer] data), CN values (x-axis) and mutation status info (colours)
  if ( mut_data ) {
    ...
    p <- plot_ly(data, x = ~CN, y = ~Z_score_diff, color = ~Mutation, text=~Gene, type='scatter', mode = "markers", marker = list(size=10, symbol="circle"), width = 800, height = 600) %>%
      ...
      layout( xaxis = list(title = "CN values"), yaxis = list(title = paste0("mRNA expression (Z-score [Patient vs ", int_cancer, "])")), margin = list(l=50, r=50, b=50, t=50, pad=4), autosize = F, legend = list( orientation = 'v', x=1, y=0.97, yanchor="top"), showlegend=TRUE)
  ...
  ##### Generate scatterplot with per-gene expression values (y-axis) and CN values (x-axis)
    ...
    p <- plot_ly(data, x = ~CN, y = ~Z_score_diff, text=~Gene, type='scatter', mode = "markers", marker = list(size=10, symbol="circle"), width = 800, height = 600) %>%
      ...
      layout( xaxis = list(title = "CN values"), yaxis = list(title =  paste0("mRNA expression (Z-score [Patient vs ", int_cancer, "])")), margin = list(l=50, r=50, b=50, t=50, pad=4), autosize = F, legend = list( orientation = 'v', y=0.8, yanchor="top"), showlegend=FALSE)
  }
...
exprTable <- function(genes, data, cn_data = NULL, sv_data = NULL, cn_decrease = TRUE, targets, sampleName, int_cancer, ext_cancer, genes_annot = NULL, cancer_genes = NULL, mut_annot = NULL, fusion_genes = NULL, ext_links = FALSE, type = "z", scaling = "sample-wise") {
  ...
  ##### Perform scaling sample-wise
  if ( scaling == "sample-wise" ) {

    ##### Calculate z-score for each group  
    group.stats <- exprGroupsStats_sampleWise(data, targets)[[1]]
  ...
  ##### Perform scaling gene-wise
    ...
    for ( group in targets.list ) {
      ...
      ##### Calculate z-score for each group  
      group.stats <- exprGroupStats_geneWise(data[rownames(group.z), ], targets, group)
  ...
  ##### Calculate Z-score differneces between investigated sample and int_cancer group mean values
  group.z <- cbind(group.z, round((group.z[, sampleName] - group.z[, int_cancer]), digits = 2))
  names(group.z)[ncol(group.z)] <- paste0("Patient vs ", int_cancer)
  
  ##### Add NAs for genes that are absent in the expression matrix. In the "Patient vs [int_cancer]" columns provide "0"s to facilitate interactive sorting the table. These will appear in blank cells in the table
  if ( length(genes.absent) > 0 ) {
    ...
    NAs.df[ names(NAs.df) %in% paste0("Patient vs ", int_cancer) ] <- 0
  ...
  ##### Reorder groups
  group.z <- cbind(group.z[ , c(ext_cancer, int_cancer, "Patient")], group.z[, c("SD", paste0("Patient vs ", int_cancer) )])
  ...
  ##### Define colours for cells background for each group and the patient vs [int_cancer] difference
  ##### Initiate dataframe for expression mean values in each group
  ...
  for ( group in c(targets.list, paste0("Patient vs ", int_cancer)) ) {
  ...
  if ( !is.null(cn_data) ) {
    ...
    ##### Get the position of "Patient vs [int_cancer]" column
    col_idx <- grep(paste0("Patient vs ", int_cancer), names(group.z), fixed = TRUE)
    
    ##### Now place the CN data after the "Patient vs [int_cancer]" column
    if ( length(genes) > 0 ) {
  ...
  if ( ext_links && length(genes) > 0 ) {
    ...
    ##### Place the external links after the "Patient vs [int_cancer]" column
    ##### Get the position of "Patient vs [int_cancer]" column
    col_idx <- grep(paste0("Patient vs ", int_cancer), names(group.z), fixed = TRUE)
  ...
  ##### Order the data by CN values (to allow filtering based on CN information) and then by the highest absolute values for Patient vs [int_cancer] difference (to allow filtering based on z-score differences)
  if ( !is.null(cn_data) && length(genes) > 0 ) {
    
    ##### Get the position of "Patient vs [int_cancer]" column
    col_idx <- grep("Patient (CN)", names(group.z), fixed = TRUE)

    group.z <- group.z[ order(abs(group.z[, paste0("Patient vs ", int_cancer)]),  decreasing = TRUE), ]
    group.z <- group.z[ order(group.z[ ,col_idx ],  decreasing = cn_decrease), ]
    
  ##### Order the data by increasing TIER category (to allow filtering based on tier information) and then by the highest absolute values for Patient vs [int_cancer] difference (to allow filtering based on z-score differences)
  ...
  ##### Order the data by MANTA increasing Tier (to prioritise SVs, based on https://github.com/AstraZeneca-NGS/simple_sv_annotation/blob/master/simple_sv_annotation.py), event type and then by the highest absolute values for Patient vs [int_cancer] difference
  ...
  ##### Otherwise order table by the highest absolute values for Patient vs [int_cancer] difference
  ...
  if ( !is.null(cn_data) ) {
    ...
    dt.table <- DT::datatable( data = group.z[, names(group.z) %!in% "SYMBOL"], filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE) %>%
      ...
      DT::formatStyle(columns = paste0("Patient vs ", int_cancer), 
                      backgroundColor = DT::styleInterval(brks.q[[paste0("Patient vs ", int_cancer)]], clrs.q[[paste0("Patient vs ", int_cancer)]])) %>%
...
ref_datasets.list[[dataset]] <- combineDatasets(params$sample_name, params$count_file, ref_dataset[[dataset]][1])
...
if ( length(sampleID.col) > 0 ) {
  ...
  if ( nrow(treamtent.df) > 0 ) {
    ...
    treatment_timeline <- plot_timeline(event = treamtent.df$Treatment, 
    ...
    plot_timeline(event = treamtent.df$Treatment, 
                start = treamtent.df$Start, 
                end = treamtent.df$End, 
                label = NA, 
                group = treamtent.df$Type,
    ...
    cv_timeline.png <- image_read(paste(PlotsDir, "cv_timeline.png", sep="/"), density = 300, depth = NULL, strip = FALSE)
...
if ( params$filter && params$transform == "CPM" ) {
  ...
  y <- edgeR::DGEList(counts=data,  group=target$Target)
  ...
  keep <- rowSums(edgeR::cpm(y)>1) >= ncol(data)/10
...
```{r data_normalisation, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 12, fig.show="hide" }
...
ref_datasets.list[[dataset]][["combined_data_processed"]] <- y$norm
```
...
for ( dataset in names(ref_datasets.list) ) {
  ...
  ref_datasets.list[[dataset]][["pca"]] <- pca(data, target, plot_mode = params$plots_mode)
}
...
for ( dataset in names(ref_datasets.list) ) {
  ...
  ##### Get data matrix with gene symbols
  ref_datasets.list[[dataset]][["combined_data_processed"]] <- apply(data.annot[, colnames(data)], 2, as.numeric)
  rownames(ref_datasets.list[[dataset]][["combined_data_processed"]]) <- data.annot$SYMBOL
  ...
  write.table(prepare2write(ref_datasets.list[[dataset]][["combined_data_processed"]]), file = paste0(params$report_dir, "/", params$sample_name, ".", params$dataset, ".combined_data_processed.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
  write.table(prepare2write(rownames(ref_datasets.list[[dataset]][["combined_data_processed"]])), file = paste0(params$report_dir, "/", params$sample_name, ".", params$dataset,  ".combined_data_processed.genes.txt"), sep="\t", quote=FALSE, row.names=FALSE, col.names=TRUE, append = FALSE )
...
if ( params$plots_mode == "interactive" || params$plots_mode == "semi-interactive" ) {
  ...
  genes.expr.z <- exprTable( genes = rownames(data), data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "z", scaling = scaling)
  
  genes.expr.perc <- exprTable( genes = rownames(data), data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "perc", scaling = scaling)
...
expr_data <- ref_datasets.list[[dataset]][["combined_data_processed"]]
targets <- ref_datasets.list[[dataset]][["sample_annot"]]

##### Get the Patient vs [cancer] Z-scores using exprTable funstion
expr_data <- exprTable( genes = rownames(expr_data), data = expr_data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, type = "z", scaling = scaling)[[2]]

expr_data.z <- expr_data[, paste0( "Patient vs ", int_cancer_group)]
...
* `r if (params$filter) { nrow(ref_datasets.list[[dataset]][["combined_data"]]) - nrow(ref_datasets.list[[dataset]][["combined_data_processed"]]) } else { ("0") }` are either not expressed or their expression level is too low to be detected
* `r if (params$filter) { nrow(ref_datasets.list[[dataset]][["combined_data_processed"]]) - nrow(data.annot) } else { nrow(ref_datasets.list[[dataset]][["combined_data"]]) - nrow(data.annot) }` genes were ignored due to lack of [HGNC](https://www.genenames.org/){target="_blank"}-approved gene symbol

NOTE: the `r if (params$filter) { nrow(ref_datasets.list[[dataset]][["combined_data"]]) - nrow(ref_datasets.list[[dataset]][["combined_data_processed"]]) } else { ("0") }` genes with no/low expression are indicated in <span style="color:#808080">BLANK</span> cells with missing values in *expression summary tables* in [Mutated genes], [Cancer genes], [Structural variants], [CN altered genes] and [Immune markers] sections.
...
```{r data_transformation_filtering_plot, fig.width = 12, fig.height = 5, thumb = list(width = 15, height = 15) }
...
```{r data_normalisation_plot, fig.width = 12, fig.height = 9, thumb = list(width = 15, height = 15) }
data_nonnormalised
data_normalised
```
...
```{r pca_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 8, fig.height = 8 }
ref_datasets.list[[dataset]][["pca"]][[2]]
```
...
```{r scree_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 5 }
ref_datasets.list[[dataset]][["pca"]][[3]]
```
...
```{r treatment_timeline_plot, fig.width = 12, fig.height = 5, thumb = list(width = 15, height = 15), eval = runClinicalChunk }
...
mut_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
...
mut_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)
...
Out of the `r nrow(ref_genes.list[["genes_cancer"]])` cancer genes the expression of **`r length(which(rownames(ref_genes.list[["genes_cancer"]]) %in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]])))`** was reliably measured in patient's sample. The remaining `r length(which(rownames(ref_genes.list[["genes_cancer"]]) %!in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]])))` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
cancer_genes.expr.z <- exprTable( genes = rownames(ref_genes.list[["genes_cancer"]]), data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "z", scaling = scaling)
...
cancer_genes.expr.perc <- exprTable( genes = rownames(ref_genes.list[["genes_cancer"]]), data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]], ext_links = TRUE, type = "perc", scaling = scaling)
...
exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Germline") ], fusion_genes = unique(known_translocations$geneA, known_translocations$geneB ), ext_links = TRUE, type = "z", scaling = scaling)[[1]]
```
...
The <span style="color:#ff0000">RED</span> colour range indicate relatively **high expression** (percentile) values and <span style="color:#0000ff">BLUE</span> colour range indicate relatively **low expression** (percentile) values in individual sample group. The **Patient vs `r int_cancer_group`** column illustrates the difference between percentiles in patient sample and reference cancer cohort for each fusion gene. Genes considered to be oncogenes or tumour suppressor genes, according to [OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"} database, are also indicated. Genes are ordered by **decreasing** absolute values in the **Patient vs `r int_cancer_group`** column. TSG - tumour suppressor gene; SD - standard deviation across groups
...
Out of the `r if ( runSVsChunk ) { length(unique(manta_sv$Gene)) } ` genes affected by `r if ( runSVsChunk ) { nrow(manta_sv) }` SVs, the expression of **`r if ( runSVsChunk ) { length(which(unique(manta_sv$Gene) %in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]]))) }`** was reliably measured in patient's sample. The remaining `r if ( runSVsChunk ) { length(which(unique(manta_sv$Gene) %!in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]]))) }` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
sv_genes.expr.z <- exprTable( genes = genes, data = data, sv_data = manta_sv, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
...
##### Generate expression summary table for cancer genes from OncoKB and PMCC
sv_genes.expr.perc <- exprTable( genes = genes, data = data, sv_data = manta_sv, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)
...
The <span style="color:#ff0000">RED</span> colour range indicate relatively **high expression** (percentile) values and <span style="color:#0000ff">BLUE</span> colour range indicate relatively **low expression** (percentile) values in individual sample group. The **Patient vs `r int_cancer_group`** column illustrates the difference between percentiles in patient sample and reference cancer cohort for each gene. Genes considered to be oncogenes or tumour suppressor genes, according to [OncoKB](http://oncokb.org/#/cancerGenes){target="_blank"} database, are also indicated. Genes are ordered by **increasing SV score** and then by **decreasing** absolute values in the **Patient vs `r int_cancer_group`** columns. TSG - tumour suppressor gene; SD - standard deviation across groups
...
if ( runPcgrChunk && length(cn_genes) > 0 ) {
  ...
  mutCNexprPlot(data = data, mut_data = TRUE, cn_bottom = 0.5, cn_top = 3, int_cancer = int_cancer_group, plot_mode = params$plots_mode)
  
} else if ( length(cn_genes) > 0) {
  
  data <- data[ data$Gene %in% cn_genes, ]
  mutCNexprPlot(data = data, mut_data = FALSE, cn_bottom = 0.5, cn_top = 3, int_cancer = int_cancer_group, plot_mode = params$plots_mode)
...
Out of the `r length(ref_datasets.list[[dataset]][["expr_mut_cn_data"]][,1])` genes within gained (CN values > `r params$cn_gain`) or lossed (CN values < `r params$cn_loss`) regions `r if (runPurpleChunk) { length(cn_genes) } else { length(NULL) }` are either [Cancer genes], [Mutated genes] with [tier](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg){target="_blank"} 1-`r params$pcgr_tier` variants (if mutation information is available) or are involved in [Fusion genes] or [Structural variants] (if SV information is available). The expression of **`r if (runPurpleChunk) { length(which(cn_genes %in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]]))) } else { length(NULL) }`** of these genes was reliably measured in patient's sample. The remaining `r if (runPurpleChunk) { length(which(cn_genes %!in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]]))) } else { length(NULL) }` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
if ( runPcgrChunk && runPurpleChunk ) {
  ...
  mut_cn_expr_genes.expr.gains.z <- exprTable( genes = genes, data = data, cn_data = cn_data, cn_decrease = TRUE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
...
if ( runPcgrChunk && runPurpleChunk ) {
  ...
  mut_cn_expr_genes.expr.gains.perc <- exprTable( genes = genes, data = data, cn_data = cn_data, cn_decrease = TRUE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)
...
if ( runPcgrChunk && runPurpleChunk ) {
  ...
  mut_cn_expr_genes.expr.losses.z <- exprTable( genes = genes, data = data, cn_data = cn_data, cn_decrease = FALSE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "z", scaling = scaling)
...
if ( runPcgrChunk && runPurpleChunk ) {
  ...
  mut_cn_expr_genes.expr.losses.perc <- exprTable( genes = genes, data = data, cn_data = cn_data, cn_decrease = FALSE, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")], cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline") ], ext_links = TRUE, type = "perc", scaling = scaling)
...
Out of the `r length(unique(unlist(ref_genes.list[["genes_immune"]]$SYMBOL)))` immune markers the expression of **`r length(which(unique(unlist(ref_genes.list[["genes_immune"]]$SYMBOL)) %in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]])))`** was reliably measured in patient's sample. The remaining `r length(which(unique(unlist(ref_genes.list[["genes_immune"]]$SYMBOL)) %!in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]])))` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
immune_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL", "Immune_Cycle_Role")], ext_links = TRUE, type = "z", scaling = scaling)
...
##### Generate expression summary table for cancer genes from OncoKB and PMCC
immune_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL", "Immune_Cycle_Role")], ext_links = TRUE, type = "perc", scaling = scaling)
...
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, plot_mode = params$plots_mode, hexcode = "immune_genes", scaling = scaling)
...
Out of the `r length(unique(unlist(ref_genes.list[["genes_hrd"]]$SYMBOL)))` hrd genes the expression of **`r length(which(unique(unlist(ref_genes.list[["genes_hrd"]]$SYMBOL)) %in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]])))`** was reliably measured in patient's sample. The remaining `r length(which(unique(unlist(ref_genes.list[["genes_hrd"]]$SYMBOL)) %!in% rownames(ref_datasets.list[[dataset]][["combined_data_processed"]])))` genes are either not expressed or their expression level is too low to be detected (indicated in <span style="color:#808080">BLANK</span> cells with missing values).
...
hrd_genes.expr.z <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], ext_links = TRUE, type = "z", scaling = scaling)
...
hrd_genes.expr.perc <- exprTable( genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group , int_cancer = int_cancer_group, genes_annot = ref_datasets.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")], ext_links = TRUE, type = "perc", scaling = scaling)
...
##### Generate overview boxplot
if ( !is.null(genes) ) {
  glanceExprPlot(genes = genes, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, plot_mode = params$plots_mode, hexcode = "hrd_genes", scaling = scaling)
