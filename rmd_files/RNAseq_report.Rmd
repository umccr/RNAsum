...
```{r dragen_filtering, comment = NA, message=FALSE, warning=FALSE, eval = runArribaChunk}
##### Read in the arriba fusion calls
...
##### Keep only key columns and add info about Arriba detected fusions and 
fusions <- fusions[ colnames(fusions) %in% c("geneA", "geneB", "Score", "LeftBreakpoint", "RightBreakpoint")]
...
if ( runArribaChunk ) {
  ...
  fusion_annot <- cbind(fusion_annot1, fusion_annot2, fusions.annot[, c("Score", "LeftBreakpoint", "RightBreakpoint")])
}
...
if ( runFusionChunk ) {
  ...
  if ( length(manta_fusions) > 0 ) {
    ...
    if (runArribaChunk) {
      ...
      idx <- order(fusions$geneA_dna_support, fusions$geneB_dna_support, fusions$Dragen, fusions$reported_fusion, decreasing = TRUE)
    }
