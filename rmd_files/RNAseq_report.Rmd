...
exprGroupStats <- function(data, targets, target) {
  ...
  data.group.z <- standardization(data.group.mean)
  
  ##### Calculate cumulative sums and perform range standardization between 0 and 1 
  data.group.cum <- standardization(cumsum(data.group.z))
  
  ##### Organise the data into data frame
  data.group.df <- as.data.frame(cbind( data.group.mean, data.group.sd, data.group.q, data.group.cum))
  names(data.group.df) <- c("mean", "sd", "quantile", "cumulative_fraction")
...
cdfPlot <- function(gene, data, targets, sampleName) {
  ...
  met.expr.cum <- exprGroupStats(data, targets, "PDAC (metastatic)")
  ...
  p <- plot_ly(sample.expr.cum, x = ~mean, color = I("black")) %>%
  
    ##### Add sample data
    add_markers(y = sample.expr.cum.selected$cumulative_fraction, x = sample.expr.cum.selected$mean,
                text = rownames(sample.expr.cum.selected ),
                name = params$sample,
                marker = list(size = 12, color = "black"),
                showlegend = FALSE) %>%
  
    add_lines(y = sample.expr.cum$cumulative_fraction, x = sample.expr.cum$mean, 
              line = list(color = "grey"),
              text = rownames( sample.expr.cum ),
              name = params$sample, showlegend = TRUE) %>%
    
    ##### Add normal pancreas data
    add_markers(y = normal.expr.cum.selected$cumulative_fraction, x =  normal.expr.cum.selected$mean,
                text = rownames( normal.expr.cum.selected ),
                name = "Pancreas (normal)",
                marker = list(size = 12, opacity = 0.5, color = "green"),
                showlegend = FALSE) %>%
  
    add_lines(y = normal.expr.cum$cumulative_fraction, x = normal.expr.cum$mean, opacity = 0.5,
              line = list(color = "green", dash = "dash"),
              text = rownames( normal.expr.cum ),
              name = "Pancreas (normal)", showlegend = TRUE) %>%
    
    ##### Add PDAC data
    add_markers(y = PDAC.expr.cum.selected$cumulative_fraction, x =  PDAC.expr.cum.selected$mean,
                text = rownames( PDAC.expr.cum.selected),
                name = "PDAC",
                marker = list(size = 12, opacity = 0.5, color = "coral"),
                showlegend = FALSE) %>%
  
    add_lines(y = PDAC.expr.cum$cumulative_fraction, x = PDAC.expr.cum$mean, opacity = 0.5,
              line = list(color = "coral", dash = "dash"),
              text = rownames( PDAC.expr.cum),
              name = "PDAC", showlegend = TRUE) %>%
    
    ##### Add metastatic PDAC data
    add_markers(y = met.expr.cum.selected$cumulative_fraction, x =  met.expr.cum.selected$mean,
                text = rownames( met.expr.cum.selected),
                name = "PDAC (metastatic)",
                marker = list(size = 12, opacity = 0.5, color = "red"),
                showlegend = FALSE) %>%
    
    add_lines(y = met.expr.cum$cumulative_fraction, x = met.expr.cum$mean, opacity = 0.5,
              line = list(color = "red", dash = "dash"),
              text = rownames( met.expr.cum),
              name = "PDAC (metastatic)", showlegend = TRUE) %>%
    
    ##### Add quantile lines
    add_lines(y = seq(0,1,0.1), x = rep(quantile(sample.expr.cum$mean, probs=0:4/4)[2], 11), opacity = 0.5,
              line = list(color = "gray", opacity = 0.2, dash = "dash"),
              name = "Q1", showlegend = FALSE) %>%
    
    add_lines(y = seq(0,1,0.1), x = rep(quantile(sample.expr.cum$mean, probs=0:4/4)[3], 11), opacity = 0.5,
              line = list(color = "gray", opacity = 0.2, dash = "dash"),
              name = "Q2", showlegend = FALSE) %>%
    
    add_lines(y = seq(0,1,0.1), x = rep(quantile(sample.expr.cum$mean, probs=0:4/4)[4], 11), opacity = 0.5,
              line = list(color = "gray", opacity = 0.2, dash = "dash"),
              name = "Q3", showlegend = FALSE) %>%
    
    layout(xaxis = list(title = "mRNA expression", zeroline = FALSE, range = c(min(sample.expr.cum$mean), max(sample.expr.cum$mean)+4)),
           yaxis = list(title = "Cumulative fraction"),
           legend = list(orientation = 'v', x = 0.02, y = 1, bgcolor = "white"),
           annotations = list(x = sample.expr.cum.selected$mean, y = sample.expr.cum.selected$cumulative_fraction,
