...
cdfPlot <- function(gene, data, targets, sampleName, int_cancer, ext_cancer, comp_cancer, addBoxPlot = FALSE, plot_mode = "static", scaling = "gene-wise") {
  ...
  if ( addBoxPlot ) {
    ...
    p2 <- plot_ly(gene.expr.df, x= ~Expression, color = ~Group, type = 'box', jitter = 0.3, pointpos = 0, boxpoints = 'all', colors = c("cornflowerblue", "red", "black"), opacity = 0.5, orientation = 'h', width = 800, height = 400, showlegend=FALSE)
  }
...
glanceExprPlot <- function(genes, data, targets, sampleName, int_cancer, ext_cancer, comp_cancer, plot_mode = "static", hexcode, scaling = "gene-wise") {
  ...
  p <- plot_ly( gene.expr.df, x = ~Gene, y = ~Expression, color = ~Group, type = "box", colors = c(I("black"), "red", "cornflowerblue"), opacity=0.3, showlegend = TRUE, width = 800, height = 400 ) %>% 
  ...
  if ( plot_mode == "static" ) {
    ...
    p <- plot_ly( gene.expr.df, x = ~Gene, y = ~Expression, color = ~Group, type = "box", colors = c(I("black"), "red", "cornflowerblue"), opacity=0.3, showlegend = TRUE, width = 800, height = 600 ) %>% 
    add_markers(x = ~Gene[ gene.expr.df$Group %in% "Patient" ], y = ~Expression[ gene.expr.df$Group %in% "Patient" ], color = ~Group[ gene.expr.df$Group %in% "Patient" ], marker = list(size = 7), opacity=1, showlegend = FALSE) %>%
...
ref_dataset <- list( "paad" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_PAAD.txt", sep="/"), "PDAC (UMCCR)", "PDAC (TCGA)"), 
                     "cesc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_CESC.txt", sep="/"), "PDAC (UMCCR)", "CESC (TCGA)"), 
                     "blca" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_BLCA.txt", sep="/"), "PDAC (UMCCR)", "BLCA (TCGA)"),
                     "brca" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_BRCA.txt", sep="/"), "PDAC (UMCCR)", "BRCA (TCGA)"),
                     "thca" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_THCA.txt", sep="/"), "PDAC (UMCCR)", "THCA (TCGA)"),
                     "hnsc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_HNSC.txt", sep="/"), "PDAC (UMCCR)", "HNSC (TCGA)"),
                     "lgg" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_LGG.txt", sep="/"), "PDAC (UMCCR)", "LGG (TCGA)"),
                     "kirc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_KIRC.txt", sep="/"), "PDAC (UMCCR)", "KIRC (TCGA)"),
                     "lusc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_LUSC.txt", sep="/"), "PDAC (UMCCR)", "LUSC (TCGA)"),
                     "luad" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_LUAD.txt", sep="/"), "PDAC (UMCCR)", "LUAD (TCGA)"),
                     "prad" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_PRAD.txt", sep="/"), "PDAC (UMCCR)", "PRAD (TCGA)"),
                     "skcm" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_SKCM.txt", sep="/"), "PDAC (UMCCR)", "SKCM (TCGA)"),
                     "stad" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_STAD.txt", sep="/"), "PDAC (UMCCR)", "STAD (TCGA)"),
                     "lihc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_LIHC.txt", sep="/"), "PDAC (UMCCR)", "LIHC (TCGA)"),
                     "coad" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_COAD.txt", sep="/"), "PDAC (UMCCR)", "COAD (TCGA)"),
                     "kirp" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_KIRP.txt", sep="/"), "PDAC (UMCCR)", "KIRP (TCGA)"),
                     "ov" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_OV.txt", sep="/"), "PDAC (UMCCR)", "OV (TCGA)"),
                     "sarc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_SARC.txt", sep="/"), "PDAC (UMCCR)", "SARC (TCGA)"),
                     "pcpg" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_PCPG.txt", sep="/"), "PDAC (UMCCR)", "PCPG (TCGA)"),
                     "ucec" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_UCEC.txt", sep="/"), "PDAC (UMCCR)", "UCEC (TCGA)"),
                     "laml" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_LAML.txt", sep="/"), "PDAC (UMCCR)", "LAML (TCGA)"),
                     "esca" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_ESCA.txt", sep="/"), "PDAC (UMCCR)", "ESCA (TCGA)"),
                     "gbm" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_GBM.txt", sep="/"), "PDAC (UMCCR)", "GBM (TCGA)"),
                     "tgct" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_TGCT.txt", sep="/"), "PDAC (UMCCR)", "TGCT (TCGA)"),
                     "thym" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_THYM.txt", sep="/"), "PDAC (UMCCR)", "THYM (TCGA)"),
                     "read" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_READ.txt", sep="/"), "PDAC (UMCCR)", "READ (TCGA)"),
                     "kich" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_KICH.txt", sep="/"), "PDAC (UMCCR)", "KICH (TCGA)"),
                     "uvm" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_UVM.txt", sep="/"), "PDAC (UMCCR)", "UVM (TCGA)"),
                     "meso" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_MESO.txt", sep="/"), "PDAC (UMCCR)", "MESO (TCGA)"),
                     "acc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_ACC.txt", sep="/"), "PDAC (UMCCR)", "ACC (TCGA)"),
                     "ucs" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_UCS.txt", sep="/"), "PDAC (UMCCR)", "UCS (TCGA)"),
                     "dlbc" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_DLBC.txt", sep="/"), "PDAC (UMCCR)", "DLBC (TCGA)"),
                     "chol" = c(paste(params$ref_data_dir, "ref_datasets/Datasets_list_CHOL.txt", sep="/"), "PDAC (UMCCR)", "CHOL (TCGA)")
)
...
if ( !is.na(gene) && gene %in% rownames(data) ) {
  ...
  cdfPlot(gene = gene, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, addBoxPlot = TRUE, plot_mode = params$plots_mode, scaling = scaling)
...
suppressMessages(library(plotly))
##### Generate empirical cumulative distribution function (ECDF) plot illustrating mRNA expression level the genes of interest in the context of the overall mRNA expression distribution
cdfPlot(gene = geneA, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling)
cdfPlot(gene = geneB, data = data, targets = targets, sampleName = params$sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, addBoxPlot = FALSE, plot_mode = params$plots_mode, scaling = scaling)
