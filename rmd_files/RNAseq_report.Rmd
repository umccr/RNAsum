...
for( i in 1:genes_no ) {
  ...
  fusion_reads.selected <- ref_genes.list[["pizzly"]][ ref_genes.list[["pizzly"]]$geneA.name == geneA & ref_genes.list[["pizzly"]]$geneB.name == geneB, ]
    ...
    output_transcripts[[length(output_transcripts) + 1]] <- DT::datatable( data = fusion_reads.selected, filter="none", rownames = TRUE, extensions = c('Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', scrollX = FALSE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 400, height = 195, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE) %>%
