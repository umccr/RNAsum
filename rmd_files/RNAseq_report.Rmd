...
params:
  ...
  dataset: 'TEST'
  wts_dir : '../data/test_data/final/test_sample_WTS'
  dragen_rnaseq: NULL
  arriba_rnaseq: NULL
  ...
  drugs: FALSE
  immunogram: FALSE
  wgs_dir: '../data/test_data/umccrised/test_subject__test_sample_WGS'
...
if ( !is.null(params$wts_dir) ) {
  ...
  if ( dir.exists(paste0(dataDir, "/kallisto")) ) {
    ...
    salmon_gene_quant_output <- paste0(dataDir, "/salmon/", list.files(dataDir, pattern=".quant.genes.sf$"))
    salmon_transcript_quant_output <- paste0(dataDir, "/", list.files(dataDir, pattern=".quant.sf$"))
...
if ( !is.null(params$wgs_dir) ) {
  ...
  if ( file.exists(paste(params$wgs_dir, "small_variants", paste0(wgs_dir, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")) ) {
    pcgr_file <- paste(params$wgs_dir, "small_variants", paste0(wgs_dir, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")
  } else if ( file.exists(paste(params$wgs_dir, "..", "work", wgs_dir, "pcgr", paste0(wgs_dir, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")) ) {
    pcgr_file <- paste(params$wgs_dir, "..", "work", wgs_dir, "pcgr", paste0(wgs_dir, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")
  } else if ( file.exists(paste(params$wgs_dir, "pcgr", paste0(wgs_dir, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")) ) {
    pcgr_file <- paste(params$wgs_dir, "pcgr", paste0(wgs_dir, "-somatic.pcgr.snvs_indels.tiers.tsv"), sep = "/")
  } else if ( file.exists(paste(params$wgs_dir, "pcgr", paste0(wgs_dir, "-somatic.pcgr_acmg.grch37.snvs_indels.tiers.tsv"), sep = "/")) ) {
    pcgr_file <- paste(params$wgs_dir, "pcgr", paste0(wgs_dir, "-somatic.pcgr_acmg.grch37.snvs_indels.tiers.tsv"), sep = "/")
  } else {
    runPcgrChunk <- FALSE
  }
  ...
  purple_file_1 <- paste(params$wgs_dir, "purple", paste0(wgs_dir, ".purple.gene.cnv"), sep = "/")
  purple_file_2 <- paste(params$wgs_dir, "purple", paste0(wgs_dir, ".purple.cnv.gene.tsv"), sep = "/")
  runPurpleChunk <- TRUE
  ...
  sv_file_1 <- paste(params$wgs_dir, "structural", paste0(wgs_dir, "-sv-prioritize-manta-pass.tsv"), sep = "/")
  sv_file_2 <- paste(params$wgs_dir, "structural", paste0(wgs_dir, "-manta.tsv"), sep = "/")
  runSVsChunk <- TRUE
...
if ( !is.null(params$dragen_rnaseq) ) {
  dragen_mapping_metrics <- list.files(params$dragen_rnaseq, pattern="\\.mapping_metrics.csv$", full.names = TRUE)
...
Transcriptome summary for patient sample **`r if ( !is.null(params$dragen_rnaseq) && length(dragen_mapping_metrics) == 1 ) {paste0(params$sample_name, params$dataset_name_incl, ", Library size ",total_input_reads_millions)} else {paste0(params$sample_name, params$dataset_name_incl)}`**.
