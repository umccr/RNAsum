...
combineDataets <- function(sample_name, sample_counts, ref_dataset) {
  ...
  if ( length(gene_list.missing) > 0 ) {
    write.table(prepare2write(gene_list.missing), file = paste0(params$project_dir, "/", sample_name,".missing_genes.txt"), sep="\t", quote=FALSE, row.names=TRUE, append = FALSE )
  }
...
## Comparison to various tumours  {.tabset}
