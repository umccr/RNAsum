...
<img src="../man/figures/RNAsum_workflow_updated.png" width="100%" alt="rnasum_workflow">
...
The **read count** data (e.g. [abundance.tsv](/inst/rawdata/test_data/kallisto/abundance.tsv)
...
<img src="img/counts_post-processing_scheme.png" width="40%" alt="counts_processing_scheme">
...
**Enhanced batch assessment**: RNAsum provides specialized gene set options for targeted batch effect evaluation:

1. **Top variable genes** (`gene_set_type = "top_n"`): Uses the most variable genes for comprehensive assessment (default: 2000 genes)
2. **Cancer gene sets** (`gene_set_type = "cancer_genes"`): Focuses on curated cancer gene databases for clinically relevant assessment:
   - UMCCR database: 1,248 cancer genes
   - OncoKB database: 1,019 genes
   - Combined: 1,315 unique genes
3. **Custom gene sets** (`gene_set_type = "custom"`): Allows user-defined gene lists for targeted analysis

The enhanced system includes intelligent gene ID format detection and provides clear guidance when format mismatches occur between sample data (typically Ensembl IDs) and gene databases (gene symbols).
...
<img src="img/Z-score_transformation_gene_wise.png" width="30%" alt="gene_wise_z_transformation_scheme">
...
<img src="img/centering_group_wise.png" width="30%" alt="centering_scheme_group_wise">
...
* Check if the [PCGR](https://github.com/sigven/pcgr) tiered TSV output file
  is available (e.g. </inst/rawdata/test_data/small_variants/TEST-snvs_indels.tiers.tsv>).
...
* Check if the SV annotated TSV output file is available
...
[purple-ex]: </inst/rawdata/test_data/copy_number/TEST.cnv.gene.tsv>
