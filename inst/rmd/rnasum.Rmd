...
ensembl_version <- 86L
library(package = paste0("EnsDb.Hsapiens.v", ensembl_version), character.only = TRUE)
library(package = paste0("BSgenome.Hsapiens.UCSC.hg", ucsc_genome_assembly), character.only = TRUE)
...
dataset_name_incl <- dplyr::if_else(params$dataset_name_incl, glue::glue("_{params$dataset}"), "")
...
sample_name <- paste0(
  params$sample_name,
  dplyr::if_else(dataset_name_incl == "", "", paste0("_", dataset))
)
results_dir <- file.path(params$report_dir, glue::glue("{sample_name}.results"))
RNAsum::mkdir(results_dir) # there is also fs::dir_create
...
ref_dataset.list <- vector("list", length(dataset)) |> rlang::set_names(dataset)
...
arriba_summary_out <- glue::glue("{sample_name}{dataset_name_incl}.RNAseq_report.arriba_fusions.txt")
...
if (runSVsChunk) {
  manta <- sample_data.list[["wgs"]][["manta_tsv"]][["melted"]] |>
    dplyr::select(
      dplyr::contains("tier", ignore.case = TRUE),
...
# TODO: cleanup this clinical info chunk
if (length(sampleID.col) > 0) {
  ...
  if (nrow(treatment.df) > 0) {
    ...
    RNAsum::mkdir(PlotsDir_clininfo)
    ...
    lares::plot_timeline(event = treatment.df$Treatment, start = treatment.df$Start, end = treatment.df$End, label = NA, group = treatment.df$Type, title = "", subtitle = "", save = TRUE, subdir = "clinical_info")
    ...
    png::writePNG(cv_timeline.png, file.path(PlotsDir_clininfo, "treatment_timeline.png"), dpi = 300)
...
ref_genes.list[["genes_cancer"]] <- ref_genes.list[["genes_cancer"]] |>
  dplyr::mutate(
    germ = dplyr::if_else(.data$germ, "Yes", "-"),
    fusion = dplyr::if_else(.data$fusion, "Yes", "-"),
    tumorsuppressor = dplyr::if_else(.data$tumorsuppressor, "Yes", "-"),
    oncogene = dplyr::if_else(.data$oncogene, "Yes", "-")
  )
...
target <- ref_dataset.list[[dataset]][["sample_annot"]] |>
  dplyr::mutate(Target = dplyr::if_else(.data$Target == sample_name, "Patient", .data$Target))
...
# Prepare data frame
data.df <- dplyr::tibble(
  Sample = rownames(target),
  Library_size = as.numeric(base::colSums(comb_data) * 1e-6),
...
cn_data.sub <- cn_data.sub[cn_data.sub$Gene %in% ref_genes.list[["summary"]]$Cancer,]
...
known_translocations.CGI <- ref_genes.list[["cancer_biomarkers_trans"]]
...
known_translocations.FusionGDB <- ref_genes.list[["FusionGDB"]]
...
#####  Note the fusions order, which will be later required for embedding Arriba plots from corresponding pdf booklet pages
...
for (i in 1:nrow(fusions)) {
  ...
  if (paste(geneA, geneB, sep = "-") %in% trans.pairs) {
    ...
    } # TO_DO how is this working in master
...
##### Check if there are any fusions detected at all in dragen fusion output
if (nrow(dragen.fusions) > 0) {
  for (row in 1:nrow(dragen.fusions)) {
    if (dragen.fusions[row, "geneA"] %in% ref_genes.list[["summary"]]$Cancer | dragen.fusions[row, "geneB"] %in% ref_genes.list[["summary"]]$Cancer) {
  ...
  if (runArribaChunk) {
    ...
    if (all(c("GeneALocation", "GeneBLocation", "NumSplitReads", "NumSoftClippedReads", "Score") %in% colnames(dragen.fusions))) {
      ...
      #below is erroring out with non-numeric argument to binary operator error
      #fusions$split_reads <- fusions$split_readsA + fusions$split_readsB
      fusions$split_reads <- as.numeric(fusions$split_readsA) + as.numeric(fusions$split_readsB)
    ...
    ##### No arriba results keep only columns with info from Dragen
  } else {
