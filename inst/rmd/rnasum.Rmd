...
# only support GRCh38
ucsc_genome_assembly <- 38L
ensembl_version <- 86L
ensdb_pkg <- paste0("EnsDb.Hsapiens.v", ensembl_version)
bsgen_pkg <- paste0("BSgenome.Hsapiens.UCSC.hg", ucsc_genome_assembly)
library(conflicted) # checks for pkg function conflicts
library(package = ensdb_pkg, character.only = TRUE, include.only = ensdb_pkg)
library(package = bsgen_pkg, character.only = TRUE, include.only = bsgen_pkg)
library(AnnotationDbi, include.only = c("keys"))
library(ensembldb, include.only = c("select")) # be explicit with ensembldb::select
library(edgeR, include.only = c("DGEList", "cpm", "rpkm", "calcNormFactors"))
library(png, include.only = c("readPNG", "writePNG"))
dplyr_funcs <- c(
  "mutate", "filter", "select", "arrange", "distinct", "left_join", "n",
  "if_else", "rename", "contains", "bind_rows", "pull", "relocate", "row_number",
  "slice"
)
# probably should load all dplyr instead of individual functions
library(dplyr, include.only = dplyr_funcs)
...
```{r prep, echo = FALSE}
# TODO (PD): This needs to be reworked
# Include/exclude the "Code" button allowing to "show"/"hide" code chunks from the report
css_code <- if_else(params$hide_code_btn, ".btn { display: none ;", " ")
...
tx2ensembl <- edb |>
  ...
  rename(tx_name = "TXID", gene_id = "GENEID")
...
# TODO: put all chunks inside a function
...
if (runSVsChunk) {
  manta <- sample_data.list[["wgs"]][["manta_tsv"]][["melted"]] |>
    dplyr::select(
      contains("tier", ignore.case = TRUE),
      "Event", "Genes", "Effect", "Detail", "Start", "End", "AF_BPI", "CNC", "CN",
      "SR_alt", "PR_alt", "Ploidy", "Transcript"
    )
  # TODO: Check if 'dplyr::filter(Genes != "")' makes sense
}

##### Read in reference datasets and merge them with sample data.
# This part outputs a vector with first element containing the merged data and
# second element containing merged targets info.
...
# TODO (PD): cleanup this clinical info chunk
...
tmpg <- ref_genes.list[["genes_oncokb"]] |>
  mutate(
    UMCCR = "No", Oncogene = "-", TSG = "-", Fusion = "-", Germline = "-"
  ) |>
  rename(
    occurrence_num = "# of occurence within resources"
  )
...
gene_info <- edb |>
  ...
  rename("ENSEMBL" = "GENEID", "SYMBOL" = "GENENAME")
...
genes2keep.combined_data <- genes2keep |>
  dplyr::filter(.data$ENSEMBL %in% rownames(ref_dataset.list[[dataset]][["combined_data"]]))
...
##### Clean the space
## edb is actually used downstream (in if-else which probably doesn't get hit), so keep for now
...
summarise_comb_data <- function(f) {
  stopifnot(is.function(f))
  sums_vec <- colSums(ref_dataset.list[[dataset]][["combined_data"]])
  res <- as.numeric(sums_vec * 1e-6) |>
    f() |>
    round(digits = 0)
  res
}
cpm.min <- summarise_comb_data(min)
cpm.max <- summarise_comb_data(max)
...
for (group in targets_mod.list) {
  ...
  if (params$filter && params$transform == "CPM") {
    ...
    y[[group]] <- DGEList(counts = data, group = target$Dataset)

    ##### Keep genes with CPM of at least 1 in more than 10% of samples
    filter.threshold <- 1
    keep <- rowSums(cpm(y[[group]]) > filter.threshold) >= ncol(data) / 10
    ...
    y[[group]]$transformed <- cpm(y[[group]], normalized.lib.sizes = FALSE, log = params$log, prior.count = 0.25)
    y[[group]]$filtered.transformed <- cpm(y[[group]]$filtered, normalized.lib.sizes = FALSE, log = params$log, prior.count = 0.25)
    ...
    y[[group]]$transformed <- rpkm(y[[group]], gene.length = gene.length, normalized.lib.sizes = FALSE, log = FALSE)
...
if (params$filter) {
  ...
  data.df <- tibble(
    Transformed = y[["comb"]]$transformed[, ncol(y[["comb"]]$transformed)],
    Counts = ref_dataset.list[[dataset]][["combined_data"]][, ncol(ref_dataset.list[[dataset]][["combined_data"]])]
  )
  ...
  # potential NULL add_cancer_group just gets ignored
  ...
  ##### After filtering
  plot(density(y[["comb"]]$filtered.transformed[, 1]),
    lwd = 2, xlim = c(den.x[1], max(data.df$Transformed)),
...
for (group in targets_mod.list) {
  if (params$transform == "CPM") {
    ...
    if (params$filter) {
      ...
      y[[group]]$filtered$samples["norm.factors"] <- calcNormFactors(y[[group]]$filtered, method = params$norm)$samples["norm.factors"]
      y[[group]]$norm <- cpm(y[[group]]$filtered, normalized.lib.sizes = TRUE, log = params$log, prior.count = 0.25)
    } else {
      y[[group]]$noNorm <- y[[group]]$transformed
      y[[group]]$samples["norm.factors"] <- calcNormFactors(y[[group]], method = params$norm)$samples["norm.factors"]
      y[[group]]$norm <- cpm(y[[group]], normalized.lib.sizes = TRUE, log = params$log, prior.count = 0.25)
    }
...
legend("topright", legend = plot_legend1, fill = targets.colour[[1]], horiz = TRUE, bg = "transparent", box.col = "transparent")
...
boxplot(y[["comb"]]$noNorm, las = 2, col = targets.colour[[2]], main = "", pch = "", las = 3, xaxt = "n", outline = FALSE)
title(main = "Unnormalised data", ylab = params$transform)
legend("topright", legend = plot_legend1, fill = targets.colour[[1]], horiz = TRUE, bg = "transparent", cex = 0.7, box.col = "transparent")
...
for (dataset in names(ref_dataset.list)) {
  ...
  edb <- base::eval(base::parse(text = paste0("EnsDb.Hsapiens.v", ensembl_version)))
  ...
  gene_info.immune_markers <- merge(gene_info, ref_genes.list[["genes_immune"]]$genes_immune_markers, by = "SYMBOL", all.x = TRUE)

  ##### Keep only immune response markers for which there is available annotation
  ref_genes.list[["genes_immune"]]$immune_markers <- ref_genes.list[["genes_immune"]]$genes_immune_markers[ref_genes.list[["genes_immune"]]$genes_immune_markers$SYMBOL %in% gene_info.immune_markers$SYMBOL, ]

  ##### Add info about immunogram genes
  if (params$immunogram) {
    gene_info.immunogram <- merge(gene_info, ref_genes.list[["genes_immune"]]$genes_immunogram, by = "SYMBOL", all.x = TRUE)
    ...
    ref_genes.list[["genes_immune"]]$immunogram <- ref_genes.list[["genes_immune"]]$genes_immunogram[ref_genes.list[["genes_immune"]]$genes_immunogram$SYMBOL %in% gene_info.immunogram$SYMBOL, ]
...
scaling <- if_else(tolower(params$scaling) == "gene-wise", "gene_wise", "group-wise")
...
expr_data.z <- expr_data.z[expr_genes, ]
...
if (!is.null(sample_data.list$wgs$pcgr_tiers_tsv)) {
  mut_data <- sample_data.list$wgs$pcgr_tiers_tsv
  ...
  ##### If there is no expression value for a specific gene then assume it's not expressed at all and assign the lowest value observed in that sample
  for (gene in unique(mut_data$SYMBOL)) {
    if (gene %!in% rownames(gene.mut)) {
      # Add gene with no expression value to expression data percentiles and z-score values
      ...
      ##### Deal with multiple mutations per gene. Add gene with no expression value to our gene.mut variable
  ...
  ##### Make sure they are all in the same order
...
##### Limit the data to include only cancer genes - not seeing how this will work as ref_genes.list[["genes_cancer"]] has numerical rownames
...
if (nrow(fusion_annot_top) > 0) {
  ...
  eval(parse(text = paste0("data(UCSC.HG", ucsc_genome_assembly, ".Human.CytoBandIdeogram)")))
  cyto.info <- eval(parse(text = paste0("UCSC.HG", ucsc_genome_assembly, ".Human.CytoBandIdeogram")))
