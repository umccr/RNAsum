...
```{r arriba_processing, comment = NA, message=FALSE, warning=FALSE, eval = runArribaChunk}
...
```{r dragen_processing, comment = NA, message=FALSE, warning=FALSE, eval = runDragenFusionChunk}
...
```{r fusions_merge}
# TODO (PD): merge arriba and dragen fusions
# - need to standardise the outputs so that the report table has
# the same columns whether or not any of the tool outputs are missing.
# Potentially use `empty_tbl` here.
```
...
fusion_genes_annot <- ref_dataset.list[[dataset]][["gene_annot_all"]] |>
  dplyr::select("ENSEMBL", "SYMBOL", "SEQNAME", "GENESEQSTART", "GENESEQEND") |>
  tibble::as_tibble()

pd_fusions_annot <- function(fusions, ann) {
  ann <- ref_dataset.list$TEST$gene_annot_all |>
    dplyr::select("ENSEMBL", "SYMBOL", "SEQNAME", "GENESEQSTART", "GENESEQEND") |>
    tibble::as_tibble()
  ...
  fus |>
    ...
    dplyr::left_join(ann, by = c("geneA" = "SYMBOL")) |>
    dplyr::left_join(ann, by = c("geneB" = "SYMBOL"), suffix = c("."))
}
...
fusion_annot1 <- merge(
  fusion_genes_annot,
  fusions.annot[, c("order", "geneA")],
  by = 2, sort = FALSE, all.y = TRUE
)
fusion_annot1 <- fusion_annot1[order(fusion_annot1$order), ]
fusion_annot2 <- merge(
  fusion_genes_annot,
  fusions.annot[, c("order", "geneB")],
  by = 2, sort = FALSE, all.y = TRUE
)
...
if (runDragenFusionChunk && runArribaChunk) {
  fusion_annot <- cbind(
    ...
    fusions.annot |>
      dplyr::select(
        "score",
        "breakpointA", "breakpointB", "discordant_mates",
        "split_reads", "split_readsA", "split_readsB",
        "reported_fusion", "fusions_cancer",
        "reported_fusion_geneA", "reported_fusion_geneB"
      )
  )
  ##### Arriba
  ...
  if (
    all(c("GeneALocation", "GeneBLocation", "NumSplitReads", "NumSoftClippedReads", "Score") %in% colnames(dragen.fusions))) {
    fusion_annot <- cbind(
      fusion_annot1,
      fusion_annot2,
      fusions.annot |>
...
manta_sv <- sample_data.list[["wgs"]][["manta_tsv"]][["melted"]] |>
  dplyr::mutate("Fusion genes" = .data$Genes)

# TODO (PD): basically just interested in duplicating rows based on 2-gene events?
i <- 1
while (i <= nrow(manta_sv)) {
  if (length(strsplit(manta_sv$Genes[i], split = "&", fixed = TRUE)[[1]]) > 1) {
