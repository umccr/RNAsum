...
```{r prep_css, echo = FALSE, eval=FALSE}
...
colnames(arriba.fusions) <- sub("1", "A", colnames(arriba.fusions))
colnames(arriba.fusions) <- sub("2", "B", colnames(arriba.fusions))
...
##### Add 5 columns for info about reported fusions
...
for (i in 1:nrow(fusions)) {
  ...
  if (paste(geneA, geneB, sep = "-") %in% known_translocations$FGname2) {
    ##### provide fusion URL to FusionGDB
    fusions$reported_fusion[i] <- "Yes"

    ##### provide fusion ID from FusionGDB
    fusions$FGID[i] <- known_translocations$FGID[known_translocations$FGname2 %in% paste(geneA, geneB, sep = "-")]

    fusions$reported_fusion_geneA[i] <- "Yes"
    fusions$reported_fusion_geneB[i] <- "Yes"
  } else if (paste(geneB, geneA, sep = "-") %in% known_translocations$FGname2) {
    ##### provide fusion URL to FusionGDB
    fusions$reported_fusion[i] <- "Yes"

    ##### provide fusion ID from FusionGDB
    fusions$FGID[i] <- known_translocations$FGID[known_translocations$FGname2 %in% paste(geneB, geneA, sep = "-")]
...
pd_arriba.fusions <- RNAsum::arriba_process(
  arriba.fusions = sample_data.list[["arriba_tsv"]],
...
rm(arriba.fusions, arriba.cancer_genes)
```
...
colnames(dragen.fusions) <- sub("1", "A", colnames(dragen.fusions))
colnames(dragen.fusions) <- sub("2", "B", colnames(dragen.fusions))
...
if (nrow(dragen.fusions) > 0) {
  ...
  for (i in 1:nrow(dragen.fusions)) {
    ...
    if (paste(geneA, geneB, sep = "-") %in% known_translocations$FGname2) {
      ...
      ##### provide fusion ID from FusionGDB
      dragen.fusions$FGID[i] <- known_translocations$FGID[known_translocations$FGname2 %in% paste(geneA, geneB, sep = "-")]
      ...
      ##### provide fusion ID from FusionGDB
      dragen.fusions$FGID[i] <- known_translocations$FGID[known_translocations$FGname2 %in% paste(geneB, geneA, sep = "-")]
  ...
  pd_dragen.fusions <- RNAsum::dragen_fusions_process(
    dragen.fusions = sample_data.list[["dragen_fusions"]],
    known_translocations = known_translocations,
    genes_cancer = ref_genes.list[["genes_cancer"]][["Gene"]]
  )
  ...
  ##### PD: How about we just parse the Arriba results (if not NULL), then
  ##### parse the Dragen results (if not NULL), then merge the two?
  if (runArribaChunk) {
      ...
      for (i in 1:nrow(dragen.fusions)) {
        if (!is.na(match(dragen.fusions$fusion[i], paste(fusions$geneA, fusions$geneB, sep = "__")))) {
          ...
          fusions <- rbind(
            fusions,
            data.frame(
              geneA = dragen.fusions$geneA[i],
              geneB = dragen.fusions$geneB[i],
              breakpointA = dragen.fusions$LeftBreakpoint[i],
              breakpointB = dragen.fusions$RightBreakpoint[i],
              siteA = dragen.fusions$GeneALocation[i],
              siteB = dragen.fusions$GeneBLocation[i],
              type = "-",
              split_reads = dragen.fusions$NumSplitReads[i],
              split_readsA = "-",
              split_readsB = "-",
              discordant_mates = "-",
              soft_clipped_reads = dragen.fusions$NumSoftClippedReads[i],
              confidence = "-",
              score = dragen.fusions$Score[i],
              FGID = dragen.fusions$FGID[i],
              reported_fusion = dragen.fusions$reported_fusion[i],
              reported_fusion_geneA = dragen.fusions$reported_fusion_geneA[i],
              reported_fusion_geneB = dragen.fusions$reported_fusion_geneB[i],
              effector_gene = dragen.fusions$effector_gene[i],
              fusions_cancer = dragen.fusions$fusions_cancer[i],
              geneA_dna_support = "-",
              geneB_dna_support = "-",
              Arriba = "-",
              Dragen = "Yes"
