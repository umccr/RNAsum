...
ref_dataset.list <- vector("list", length(dataset)) |> purrr::set_names(dataset)
...
if (runPcgrChunk) {
  ref_genes.list[["summary"]]$Mutated <- unique(sample_data.list$wgs[["pcgr_tiers_tsv"]][sample_data.list$wgs[["pcgr_tiers_tsv"]]$TIER %in% c(1:params$pcgr_tier), ]$SYMBOL)

  ##### Include splice region variants
  if (params$pcgr_splice_vars) {
    ref_genes.list[["summary"]]$Mutated <- unique(c(ref_genes.list[["summary"]]$Mutated, sample_data.list$wgs[["pcgr_tiers_tsv"]][grepl("NONCODING.*splice region", paste0(sample_data.list$wgs[["pcgr_tiers_tsv"]]$TIER, ".", sample_data.list$wgs[["pcgr_tiers_tsv"]]$CONSEQUENCE), fixed = FALSE), ]$SYMBOL))
  }
...
if (runFusionChunk) {
  if (runArribaChunk) {
    ref_genes.list[["summary"]]$Fusion <- unique(c(as.character(sample_data.list[["arriba_tsv"]]$gene1), as.character(sample_data.list[["arriba_tsv"]]$gene2)))
  } else {
    ref_genes.list[["summary"]]$Fusion <- NULL
  }

  if (runDragenFusionChunk) {
    ref_genes.list[["summary"]]$Fusion <- unique(c(ref_genes.list[["summary"]]$Fusion, as.character(sample_data.list[["dragen_fusions"]]$gene1), as.character(sample_data.list[["dragen_fusions"]]$gene2)))
  }

  ##### Remove NAs
  if (length(ref_genes.list[["summary"]]$Fusion) > 0) {
...
if (runSVsChunk) {
  ref_genes.list[["summary"]]$SV <- sample_data.list$wgs[["manta_tsv"]][["melted"]]
  ref_genes.list[["summary"]]$SV <- ref_genes.list[["summary"]]$SV[ref_genes.list[["summary"]]$SV$Genes != "", ]$Genes
...
if (runPurpleChunk) {
  ref_genes.list[["summary"]]$CN <- sample_data.list$wgs[["purple_gene_tsv"]]
...
ref_genes.list[["summary"]]$Immune <- unique(ref_genes.list[["genes_immune"]]$genes_immune_markers$SYMBOL)
