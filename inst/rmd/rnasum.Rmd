...
# - instead of having tpairAB and tpairBA, we could have one column where the
# pair is sorted then concatenated. That way matches between tables wouldn't need
# to run over both columns.
```

```{r fusions_annot, comment = NA, message=TRUE, warning=FALSE, eval = runFusionChunk}
##### Annotate fusion genes
pd_fusions_annot <- function(fusions, gene_ann) {
...
fusions_dragen_ann <- pd_fusions_annot(fusions_dragen, gene_ann)
fusions_arriba_ann <- pd_fusions_annot(fusions_arriba, gene_ann)
...
while (i <= nrow(manta_sv)) {
  if (length(strsplit(manta_sv$Genes[i], split = ", ", fixed = TRUE)[[1]]) > 1) {
    ...
    manta_sv$Genes[i] <- strsplit(manta_sv$Genes[i], split = ", ", fixed = TRUE)[[1]][1]
    manta_sv$Genes[i + 1] <- strsplit(manta_sv$Genes[i + 1], split = ", ", fixed = TRUE)[[1]][2]
...
if (runFusionChunk) {
  manta_fusions <- unique(manta_sv[grep(",", manta_sv$"Fusion genes"), ]$Genes)
