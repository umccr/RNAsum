...
manta_sv <- sample_data.list[["wgs"]][["manta_tsv"]][["melted"]] |>
  dplyr::mutate(
    "Fusion genes" = .data$Genes,
    multigene = grepl(", ", .data$Genes)
  ) |>
  dplyr::select("Start", "End", "Type", "Genes", "Fusion genes", "multigene")
...
if (runFusionChunk) {
  ...
  manta_fusions <- manta_fusions[manta_fusions %in% unique(c(fusions[["geneA"]], fusions[["geneB"]]))]

  ##### Flag fusions that were also reported in MANTA
  if (length(manta_fusions) > 0) {
    fusions <- fusions |>
      dplyr::mutate(
        geneA_dna_support = .data$geneA %in% manta_fusions,
        geneB_dna_support = .data$geneB %in% manta_fusions
      )
    ...
    fusions_ann$geneA_dna_support[sort(match(manta_fusions, fusion_annot$SYMBOL), na.last = NA)] <- "Yes"
    fusions_ann$geneB_dna_support[sort(match(manta_fusions, fusion_annot$SYMBOL.1), na.last = NA)] <- "Yes"
