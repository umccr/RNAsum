...
##### Clean the space
rm(dat1, target, data.df, edb, keys)
```
...
assertthat::assert_that(all(names(expr_data.perc) == names(expr_data.z)))
...
cn_data <- cn_data |>
  dplyr::filter(.data$gene %in% names(expr_data.z))

##### Add mutation data if available
if (!is.null(sample_data.list[["wgs"]][["pcgr_tiers_tsv"]])) {
  ##### Remove entries with missing gene symbol (mainly variants in intergenic regions)
  pcgr1 <- sample_data.list$wgs$pcgr_tiers_tsv |>
  ...
  pd_fun1 <- function(pcgr1, expr_data.z, expr_data.perc) {
    ...
    gene.mut2 <- tibble::tibble(
      gene = names(expr_data.z),
      alterations = "None"
    ) |>
      dplyr::mutate(
        csq_hits = pcgr_csq_hits[.data$gene],
        alterations = dplyr::if_else(is.na(.data$csq_hits), .data$alterations, paste0("Mutation: ", csq_hits))
      ) |>
      dplyr::select("gene", "alterations")

    # - get pcgr genes that are not in gene.mut
    # - assign them the min expr_data value
    # - add to expr_data named vectors
    # - add it to gene.mut
    gene.mut3 <- pcgr1 |>
      dplyr::filter(!.data$SYMBOL %in% gene.mut2$gene) |>
      dplyr::select(gene = "SYMBOL") |>
      dplyr::distinct() |>
      dplyr::mutate(alterations = pcgr_csq_hits[.data$gene])

    expr_data3 <- gene.mut3 |>
      dplyr::select("gene") |>
      dplyr::mutate(
        expr_min_z = min(expr_data.z),
        expr_min_perc = min(expr_data.perc)
      )

    expr_data3_perc <- expr_data3 |>
      dplyr::select("gene", "expr_min_perc") |>
      tibble::deframe()
    expr_data3_z <- expr_data3 |>
      dplyr::select("gene", "expr_min_z") |>
      tibble::deframe()

    list(
      gene.mut = dplyr::bind_rows(gene.mut2, gene.mut3),
      expr_data.perc = c(expr_data.perc, expr_data3_perc),
      expr_data.z = c(expr_data.z, expr_data3_z)
    )
  }

  for (i in seq_len(nrow(gene.mut))) {
    ##### Check if any mutations are reported for each gene
    if (rownames(gene.mut)[i] %in% pcgr1$SYMBOL) {
      ##### Deal with multiple mutations per gene
      if (length(pcgr1[pcgr1$SYMBOL %in% rownames(gene.mut)[i], ]$CONSEQUENCE) > 1) {
        gene.mut[rownames(gene.mut)[i], "Alterations"] <- "Mutation: multiple hits"
      } else {
        gene.mut[rownames(gene.mut)[i], "Alterations"] <- paste0("Mutation: ", pcgr1[pcgr1$SYMBOL %in% rownames(gene.mut)[i], ]$CONSEQUENCE)
      }
    }
  }
  ...
  for (gene in unique(pcgr1$SYMBOL)) {
    if (gene %!in% rownames(gene.mut)) {
      ...
      if (length(pcgr1[pcgr1$SYMBOL %in% gene, ]$CONSEQUENCE) > 1) {
        gene.mut <- rbind(gene.mut, "multiple hits")
      } else {
        gene.mut <- rbind(gene.mut, pcgr1[pcgr1$SYMBOL %in% gene, ]$CONSEQUENCE)
      }
...
##### Clean the space
rm(cn_data, cn_data.sub, expr_data, gene.mut, pcgr1, targets, expr_data.z, expr_data.perc, expr_data.z.sub, expr_data.perc.sub, expr_genes, gene.mut.sub, genes.intersect)
```
...
mut_genes.expr.perc <- RNAsum::exprTable(
  ...
  add_cancer = add_cancer_group,
  genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")],
  mut_annot = ref_genes.list[["pcgr"]][, c("SYMBOL", "TIER", "CONSEQUENCE", "VARIANT_CLASS", "AF_TUMOR", "GENOMIC_CHANGE", "PROTEIN_CHANGE")],
  oncokb_annot = ref_genes.list[["genes_oncokb"]],
  cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Fusion", "Germline")],
  scaling = scaling,
  civic_clin_evid = ref_genes.list[["civic_clin_evid"]]
)
...
for (i in 1:genes_no) {
  if (genes_no > 0) {
    ...
    output_table_Z[[i]] <- RNAsum::exprTable(
      ...
      genes_annot = ref_dataset.list[[dataset]][["gene_annot_all"]][, c("SYMBOL", "ENSEMBL")],
      oncokb_annot = ref_genes.list[["genes_oncokb"]],
      cancer_genes = ref_genes.list[["genes_cancer"]][, c("Oncogene", "TSG", "Germline")],
