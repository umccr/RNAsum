...
output:
  ...
  rmdformats::material:
    highlight: kate
params:
  arriba_pdf: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/dragen/arriba/fusions.pdf'
  arriba_tsv: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/dragen/arriba/fusions.tsv'
  batch_rm: TRUE
  clinical_id: NULL
  clinical_info: NULL
  cn_gain: 95
  ...
  dragen_fusions: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/dragen/test_sample_WTS.fusion_candidates.final'
  drugs: FALSE
  filter: TRUE
  hide_code_btn: TRUE
  immunogram: FALSE
  log: TRUE
  manta_tsv: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/umccrised/test_sample_WGS/structural/manta.tsv'
  norm: TMM
  pcgr_splice_vars: TRUE
  pcgr_tier: 4
  pcgr_tiers_tsv: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/umccrised/test_sample_WGS/small_variants/pcgr.snvs_indels.tiers.tsv'
  project: NULL
  purple_gene_tsv: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/umccrised/test_sample_WGS/purple/purple.gene.cnv'
  report_dir: '/Users/kanwals/UMCCR/git/RNAsum/nogit/rnasum'
  salmon: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/dragen/TEST.quant.sf'
  ...
  top_genes: 5
  transform: 'CPM'
  umccrise: '/Users/kanwals/UMCCR/git/RNAsum/inst/rawdata/test_data/umccrised/test_sample_WGS'
---

```{r eval=FALSE}
library(here, include.only = c("here"))
p <- list(
  arriba_pdf = here("inst/rawdata/test_data/dragen/arriba/fusions.pdf"),
  arriba_tsv = here("inst/rawdata/test_data/dragen/arriba/fusions.tsv"),
  batch_rm = TRUE,
  ...
  dataset = "TEST",
  dataset_name_incl = FALSE,
  dragen_fusions = here("inst/rawdata/test_data/dragen/test_sample_WTS.fusion_candidates.final"),
  drugs = FALSE,
  filter = TRUE,
  hide_code_btn = TRUE,
  immunogram = FALSE,
  log = TRUE,
  manta_tsv = here("inst/rawdata/test_data/umccrised/test_sample_WGS/structural/manta.tsv"),
  norm = "TMM",
  pcgr_splice_vars = TRUE,
  pcgr_tier = 4,
  pcgr_tiers_tsv = NULL,
  project = "-",
  purple_gene_tsv = NULL,
  report_dir = here("nogit/rnasum"),
  salmon = here("inst/rawdata/test_data/dragen/TEST.quant.sf"),
  sample_name = "test_sample_WTS",
  sample_source = "-",
  save_tables = TRUE,
  scaling = "gene-wise",
...
rmd <- here::here("inst/rmd/rnasum.Rmd")
render_me <- function() {
  rmarkdown::render(
    rmd,
    params = params
...
# start with more exotic pkgs, then get to core ones
{
  ...
  library(edgeR, include.only = c("DGEList", "cpm", "rpkm", "calcNormFactors"))
  library(ensembldb, include.only = c("select", "lengthOf"))
  library(preprocessCore, include.only = c("normalize.quantiles"))
  library(plotly, include.only = c("plot_ly", "layout", "add_trace"))
  library(htmltools, include.only = c("renderTags"))
  library(png, include.only = c("readPNG", "writePNG"))
  library(rlang, include.only = c("set_names"))
  library(lares, include.only = c("plot_timeline"))
  library(fs, include.only = c("dir_create", "dir_delete"))
  library(tibble, include.only = c("tibble", "add_row"))
  library(glue, include.only = c("glue"))
  library(dplyr) # keep dplyr last
...
knitr::knit_hooks$set(timeit = function(before) {
  if (before) {
    message(cat("\n", as.character(glue::glue("Start: {Sys.time()}")), sep = ""))
    NOW <<- Sys.time()
  } else {
    message(cat(as.character(glue::glue("Stop:  {Sys.time()}"))))
    print((Sys.time() - NOW) |> round(2))
  }
})
...
```{r read_data, comment = NA, message=FALSE, warning=FALSE}
...
##### Will overwrite argument passed to "--clinical_id" flag
...
arriba_summary_out <- glue("{sample_name}{dataset_name_incl}.RNAseq_report.arriba_fusions.txt")
...
runClinicalChunk <- !is.null(sample_data.list[["clinical_info"]])
...
runSVsChunk <- (!is.null(sample_data.list[["wgs"]][["manta_tsv"]]) &&
  nrow(sample_data.list[["wgs"]][["manta_tsv"]][["melted"]] > 0))
```
...
##### Prepare data for the treatment timeline plot
##### Search for row with clinical info for investigated patient
if (!is.null(params$clinical_id)) {
  sampleID.col <- grep(params$clinical_id, sample_data.list[[dataset]][["clinical_info"]])
} else if (!is.null(params$subject_id)) {
  sampleID.col <- grep(params$subject_id, sample_data.list[[dataset]][["clinical_info"]])
} else if (!is.null(subjectID)) {
  sampleID.col <- grep(subjectID, sample_data.list[[dataset]][["clinical_info"]])
}

runClinicalChunk <- FALSE

# TODO (PD): cleanup clinical info chunk
if (length(sampleID.col) > 0) {
  ...
  treatment.types <- make.names(c("NEOADJUVANT REGIMEN", "ADJUVANT REGIMEN", "FIRST LINE REGIMEN", "SECOND LINE REGIMEN", "THIRD LINE REGIMEN"))
  treatment.types_simple <- c("Neoadjuvant", "Adjuvant", "1st line", "2nd line", "3rd line")
  treatment.df <- data.frame(matrix(ncol = 4, nrow = 0))
  colnames(treatment.df) <- c("Treatment", "Type", "Start", "End")

  for (i in 1:length(treatment.types)) {
    ##### Identify treatment column number
    treatment.types.col <- grep(paste0("^", treatment.types[i], "$"), names(clinical_info))

    ##### Check how many treatments of particular type were used
    treatment.types.details <- unlist(strsplit(clinical_info[, treatment.types.col], split = ",", fixed = TRUE))

    ##### Add start and end info for each treatment
    if (any(!is.na(treatment.types.details), na.rm = FALSE)) {
      for (treatment in treatment.types.details) {
        treatment.start <- clinical_info[, treatment.types.col + 1]
        treatment.end <- clinical_info[, treatment.types.col + 2]

        ##### Use current data if treatment is still ongoing
        today <- as.character(Sys.Date())
        treatment.end[is.na(treatment.end)] <- today
        treatment.tmp <- data.frame(treatment, treatment, treatment.types_simple[i], treatment.start, treatment.end)
        treatment.df <- rbind(treatment.df, treatment.tmp)
      }
    }
  }

  if (nrow(treatment.df) > 0) {
    ...
    day0 <- sort(treatment.df$treatment.start, decreasing = FALSE)[1]
    treatments.length <- treatment.df$treatment.end - treatment.df$treatment.start
    treatments.reset <- as.Date("2000-01-01") - day0
    treatment.df$treatment.start <- treatment.df$treatment.start + treatments.reset
    treatment.df$treatment.end <- treatment.df$treatment.start + treatments.length
    names(treatment.df) <- c("Treatment", "Drug", "Type", "Start", "End")

    ##### Create directory for timeline plot
    PlotsDir_clininfo <- file.path(results_dir, "clinical_info")
    dir_create(PlotsDir_clininfo)

    ##### Record the timeline plot. NOTE, the modified dates are used here
    treatment_timeline <- plot_timeline(event = treatment.df$Treatment, start = treatment.df$Start, end = treatment.df$End, label = NA, group = treatment.df$Type, title = "", subtitle = "", save = FALSE)

    ##### Save the plot into png file. NOTE, the modified dates are used here. As default, the plot is saved as "cv_timeline"
    plot_timeline(event = treatment.df$Treatment, start = treatment.df$Start, end = treatment.df$End, label = NA, group = treatment.df$Type, title = "", subtitle = "", save = TRUE, subdir = "clinical_info")
    ...
    writePNG(cv_timeline.png, file.path(PlotsDir_clininfo, "treatment_timeline.png"), dpi = 300)
    # png(paste(PlotsDir_clininfo, "treatment_timeline.png", sep="/"), width = 900, height = 600, pointsize = 0.0001, res=300)
    ...
    # TODO (PD): check this gets deleted
    dir_delete("clinical_info")
    # Ouchy use of rm here
...
for (gene in ref_genes.list[["genes_cancer"]][["symbol"]]) {
  ...
  if (gene %in% tmpg$Hugo_Symbol) {
    tmpg[tmpg$Hugo_Symbol == gene, ]$UMCCR <- "Yes"
    tmpg[tmpg$Hugo_Symbol == gene, ]$Oncogene <- ref_genes.list[["genes_cancer"]]$oncogene[ref_genes.list[["genes_cancer"]]$symbol == gene]
    tmpg[tmpg$Hugo_Symbol == gene, ]$TSG <- ref_genes.list[["genes_cancer"]]$tumorsuppressor[ref_genes.list[["genes_cancer"]]$symbol == gene]
    tmpg[tmpg$Hugo_Symbol == gene, ]$Fusion <- ref_genes.list[["genes_cancer"]]$fusion[ref_genes.list[["genes_cancer"]]$symbol == gene]
    tmpg[tmpg$Hugo_Symbol == gene, ]$Germline <- ref_genes.list[["genes_cancer"]]$germ[ref_genes.list[["genes_cancer"]]$symbol == gene]
    tmpg[tmpg$Hugo_Symbol == gene, "occurrence_num"] <- as.integer(tmpg[tmpg$Hugo_Symbol == gene, "occurrence_num"]) + 1L
  } else {
    ##### Add if not present
    r1 <- tibble(
...
tmpg[tmpg == "No" | tmpg == ""] <- "-"
ref_genes.list[["genes_cancer"]] <- tmpg
# dplyr::select(-c("Gene"))
ref_genes.list[["genes_oncokb"]] <- tmpg |>
...
if (runSVsChunk) {
  ref_genes.list[["summary"]]$SV <- RNAsum::sv_manta_summary(sample_data.list[["wgs"]][["manta_tsv"]][["melted"]])
}
...
# Generate bar-plot for library size. The colours indicate sample groups,
...
max_libsize <- max(data.df[["Library_size"]])
# TODO (PD):
# - Does this need to be interactive? Its size is fairly big (>3MB for test set)
# - Does it need to be a barplot?
# - Should the bars be sorted instead of mixed?

library_size <- data.df |>
  plotly::plot_ly(
    x = ~Sample, y = ~Library_size, color = ~Target,
    colors = targets.colour[[1]], type = "bar",
    width = 800, height = 400
  ) |>
  plotly::layout(
    title = "",
    xaxis = list(tickfont = list(size = 10), title = "", showticklabels = FALSE),
    yaxis = list(title = "Library size (millions)"),
    margin = list(l = 50, r = 50, b = 50, t = 50, pad = 4),
    autosize = FALSE, showlegend = TRUE,
    legend = list(orientation = "h", y = max_libsize, bgcolor = "white")
  )

PlotsDir <- file.path(results_dir, "InputDataPlots")
RNAsum::saveWidgetFix(widget = library_size, file = file.path(PlotsDir, "library_size.html"))
```
...
y <- vector("list", length(targets_mod.list)) |> set_names(targets_mod.list)
...
```{r fusions_arriba_processing, comment = NA, message=FALSE, warning=FALSE, eval = runArribaChunk}
...
```{r fusions_dragen_processing, comment = NA, message=FALSE, warning=FALSE, eval = runDragenFusionChunk}
...
if (runFusionChunk) {
  manta_fusions <- manta_sv[["multigenes"]] |>
...
summary_table.df <- data.frame(matrix(unlist(summary_table.list), nrow = length(summary_table.list), byrow = TRUE), stringsAsFactors = FALSE)
...
##### Add GeneCards links
summary_table.df$Gene <- paste0("<a href='https://www.genecards.org/cgi-bin/carddisp.pl?gene=", summary_table.df$Gene, "' >", summary_table.df$Gene, "</a>")
...
rm(
  ...
  data_nonnormalised, data_normalised, treatment_timeline, data.annot
)
```
...
for (i in 1:genes_no) {
  if (genes_no > 0) {
    ...
    if (geneA %in% rownames(dat1)) {
      ##### CDF plot
      output_cdf_A[[i]] <- cdfPlot(gene = geneA, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, scaling = scaling, report_dir = results_dir)
      ...
      ##### Density plot - expression distribution
      output_density_A[[i]] <- densityPlot(gene = geneA, data = dat1, main_title = "", x_title = "Z-score", sampleName = sample_name, distributions = c("normal", "bimodal"), scaling = scaling)
    }

    ##### Gene B
    if (geneB %in% rownames(dat1) && geneB != geneA) {
      ##### CDF plot
      output_cdf_B[[i]] <- cdfPlot(gene = geneB, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = FALSE, scaling = scaling, report_dir = results_dir)
      ...
      ##### Density plot - expression distribution
      output_density_B[[i]] <- densityPlot(gene = geneB, data = dat1, main_title = "", x_title = "Z-score", sampleName = sample_name, distributions = c("normal", "bimodal"), scaling = scaling)
    }
    ...
    output_table_Z[[i]] <- RNAsum::exprTable(
      ...
      fusion_genes = unique(known_translocations$geneA, known_translocations$geneB),
      scaling = scaling,
      civic_clin_evid = ref_genes.list[["civic_clin_evid"]]
    )[[1]]
...
mRNA expression levels of genes located within detected structural variants (SVs), obtained from [Manta](https://github.com/Illumina/manta) SV caller, in patient's sample and their average mRNA expression in samples from cancer cohorts.
...
for (i in 1:genes_no) {
  if (genes_no > 0 && genes[i] %in% rownames(dat1)) {
    ##### CDF plot
    output_cdf[[i]] <- cdfPlot(gene = genes[i], data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, addBoxPlot = TRUE, scaling = scaling, report_dir = results_dir)
    ...
    ##### Density plot - expression distribution
    output_density[[i]] <- densityPlot(gene = genes[i], data = dat1, main_title = "", x_title = "Z-score", sampleName = sample_name, distributions = c("normal", "bimodal"), scaling = scaling)
  }
}
...
Section overlaying the mRNA expression data with per-gene somatic copy-number (CN) data (from [PURPLE](https://github.com/hartwigmedical/hmftools/tree/master/purity-ploidy-estimator)), as well as SNVs/indels and SVs data, if available.

`r if ( !runPurpleChunk ) { c("CN information for this sample is **NOT AVAILABLE**.") }`

### - Genomic view

`r if (runPurpleChunk) { length(ref_dataset.list[[dataset]][["expr_mut_cn_data_all"]][,1])} else { cat("0") }` genes with available CN data (*y-axis*) are presented in the genomic context (*x-axis*). **`r if (runPurpleChunk) { length(ref_dataset.list[[dataset]][["expr_mut_cn_data"]][,1]) } else { length(NULL) }`** of them (indicated by *various colours*) are [Cancer genes] and are gained `r if ( runPurpleChunk ) { paste0("(CN values >= ", cn_top, ")") }` or lost `r if ( runPurpleChunk ) { paste0("(CN values =< ", cn_bottom, ")") }`. All other genes are marked in [*gray*]{style="color:#808080"} or *black*.
...
##### Add SNVs
if (runPcgrChunk) {
  dat1$Alterations <- as.character(dat1$Alterations)
...
if (runFusionChunk) {
  ...
  dat1$Alterations[dat1$Gene %in% fusions$geneA] <- paste0(dat1$Alterations[dat1$Gene %in% fusions$geneA], "; Fusion")
  data.sub$Alterations[data.sub$Gene %in% fusions$geneA] <- paste0(data.sub$Alterations[data.sub$Gene %in% fusions$geneA], "; Fusion")
  dat1$Alterations[dat1$Gene %in% fusions$geneB] <- paste0(dat1$Alterations[dat1$Gene %in% fusions$geneB], "; Fusion")
...
if (runSVsChunk) {
  ##### Change the alteration type to "fusion" for fusion genes
  dat1$Alterations[dat1$Gene %in% unique(manta_sv$Gene)] <- paste0(dat1$Alterations[dat1$Gene %in% unique(manta_sv$Gene)], "; SV")
  ##### Change the alteration type to "fusion" for fusion genes
...
dat1 <- dat1[dat1$Gene %in% ref_dataset.list[[dataset]][["gene_annot"]]$SYMBOL, ]
...
if (nrow(data.annot) > 0) {
  ...
  # TODO (PD): check if the data should be dat1
...
Scatterplot comparing the per-gene difference in **mRNA expression** of [Cancer genes] between patient's sample and cancer individuals (*y-axis*), and **CN values** (*x-axis*, from [PURPLE](https://github.com/hartwigmedical/hmftools/tree/master/purity-ploidy-estimator)).
...
Table summarising the **mRNA expression** values in cancer and patient samples for genes with **CN** values \>= `r if ( runPurpleChunk ) { cn_top } else { c("(NA)") }` (**gains**), based on patient's genomic data (from [PURPLE](https://github.com/hartwigmedical/hmftools/tree/master/purity-ploidy-estimator)), and mutation status if available (from [PCGR](https://github.com/sigven/pcgr)).
...
Table summarising the **mRNA expression** values in cancer and patient samples for genes with **CN** values =\< `r if ( runPurpleChunk ) { cn_bottom } else { c("(NA)") }` (**losses**), based on patient's genomic data (from [PURPLE](https://github.com/hartwigmedical/hmftools/tree/master/purity-ploidy-estimator)), and mutation status if available (from [PCGR](https://github.com/sigven/pcgr)).
...
if (!is.null(genes)) {
  glanceExprPlot(genes = genes, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, hexcode = "immune_genes", type = "perc", sort = "alphabetically", scaling = scaling, report_dir = results_dir)
} else {
...
##### Generate overview boxplot
if (!is.null(genes)) {
  glanceExprPlot(genes = genes, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, hexcode = "immune_genes", type = "z", sort = "alphabetically", scaling = scaling, report_dir = results_dir)
} else {
  cat("\nNo expression data is available for immune markers!\n")
}
...
webplot(as.data.frame(ref_genes.list[["genes_immune"]]$immunogram.df), data.row = ncol(dat1), main = "", add = FALSE, col = "black")
...
##### Generate overview boxplot
if (!is.null(genes)) {
  glanceExprPlot(genes = genes, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, hexcode = "hrd_genes", type = "perc", sort = "alphabetically", scaling = scaling, report_dir = results_dir)
} else {
...
##### Generate overview boxplot
if (!is.null(genes)) {
  glanceExprPlot(genes = genes, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, hexcode = "hrd_genes", type = "z", sort = "alphabetically", scaling = scaling, report_dir = results_dir)
} else {
  cat("\nNo expression data is available for HRD genes!\n")
}
...
if (!is.null(genes)) {
  glanceExprPlot(genes = genes, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, hexcode = "cancer_genes", type = "perc", sort = "none", scaling = scaling, report_dir = results_dir)
} else {
...
if (!is.null(genes)) {
  glanceExprPlot(genes = genes, data = dat1, targets = targets, sampleName = sample_name, ext_cancer = ext_cancer_group, int_cancer = int_cancer_group, comp_cancer = comp_cancer_group, add_cancer = add_cancer_group, hexcode = "cancer_genes", type = "z", sort = "none", scaling = scaling, report_dir = results_dir)
} else {
  cat("\nNo expression data is available for cancer genes!\n")
}
...
`r if ( params$drugs && runPcgrChunk ) { paste0("**", length(mut_genes.expr.perc[[2]]$SYMBOL), "** genes with [PCGR](https://github.com/sigven/pcgr) [tier](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg) 1-", params$pcgr_tier, " variants were screened for suitable drugs (see [Mutated genes] section).") } else if ( params$drugs && !runPcgrChunk ) { paste0("**0** genes with [PCGR](https://github.com/sigven/pcgr) [tier](https://pcgr.readthedocs.io/en/latest/tier_systems.html#tier-model-2-pcgr-acmg) 1-", params$pcgr_tier, " variants were screened for suitable drugs (see [Mutated genes] section).") }`
