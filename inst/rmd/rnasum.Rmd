...
# start with more exotic pkgs, then get to core ones
{
  ...
  library(bsgen_pkg, character.only = TRUE, include.only = bsgen_pkg)
...
for (gene in names(alt_genes.all)) {
  ...
  summary_table.list[[gene]] <- c(
    ...
    paste0("<a href='https://search.cancervariants.org/#", gene, "' target='_blank'>VICC</a>")
  )
  ...
  if (gene %in% rownames(ref_genes.list[["genes_oncokb"]])) {
    if (ref_genes.list[["genes_oncokb"]][gene, "OncoKB"] == "Yes") {
      summary_table.list[[gene]][length(summary_table.list[[gene]])] <- paste(
        ...
        paste0("<a href='http://oncokb.org/#/gene/", gene, "' target='_blank'>OncoKB</a>"),
        sep = ", "
      )
    }
  }
...
Expression profiles for gene fusion events involving **DNA-supported fusion** genes (see [Structural variants] section), gene fusions **reported in [FusionGDB](https://ccsm.uth.edu/FusionGDB){target="_blank"}** or [**Cancer genes**], indicated in [green]{style="color:#02d653"}, [red]{style="color:#ff0000"} and [grey]{style="color:#767689"} columns in the [Fusion genes] table, respectively, and with the highest *Split count* and *Pair count* values.

[NOTE]{style="color:#ff0000"}: the *visualisation* is available only for fusion genes detected by [Arriba](https://arriba.readthedocs.io/en/latest/){target="_blank"} (see the [- Summary] table).
