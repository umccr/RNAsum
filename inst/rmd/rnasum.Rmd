...
```{r treatment_timeline_plot, comment = NA, message=FALSE, warning=FALSE, fig.width = 12, fig.height = 5, eval = FALSE }
# TODO (PD): change the eval to 'runClinicalChunk' (it just doesn't work when using 'Run All Chunks Above' - see https://github.com/rstudio/rstudio/issues/6675)
...
Out of the `r if ( runSVsChunk ) { length(unique(manta_sv$melted_variants$Genes)) }` genes affected by `r if ( runSVsChunk ) { manta_sv$total_variants }` SVs, the expression of **`r if ( runSVsChunk ) { length(which(unique(manta_sv$melted_variants$Genes) %in% rownames(ref_dataset.list[[dataset]][["data_to_report"]]))) } else { length(NULL) }`** was reliably measured in the patient's sample. The remaining `r if ( runSVsChunk ) { length(which(!unique(manta_sv$melted_variants$Genes) %in% rownames(ref_dataset.list[[dataset]][["data_to_report"]]))) }` genes are either not expressed or their expression level is too low to be detected (indicated in [BLANK]{style="color:#808080"} cells with missing values).
...
genes <- unique(manta_sv$melted_variants$Genes)
...
sv_genes.expr.perc <- RNAsum::exprTable(
  ...
  data = dat1,
  genes = genes,
  sv_data = manta_sv[["melted_variants"]],
  targets = targets,
  sampleName = sample_name,
  ext_cancer = ext_cancer_group,
  int_cancer = int_cancer_group,
