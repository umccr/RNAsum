...
# reference data
ref_dataset_filepaths <- RNAsum::get_refdata(dataset, params$batch_rm)
...
ref_dataset.list[[dataset]] <- RNAsum::combineDatasets(
  ...
  sample_counts = sample_data.list[["counts"]],
...
if (dataset %in% c("PAAD", "PAAD-IPMN", "PAAD-NET", "PAAD-ACC") && params$batch_rm) {
...
runPurpleChunk <- !is.null(sample_data.list[["wgs"]][["cn_gene_tsv"]])
...
# Annotation of copy-number (CN) altered genes in given patient, including only
# those with CN values meeting user-defined thresholds.
if (runPurpleChunk) {
  purple_summary <- sample_data.list$wgs[["cn_gene_tsv"]] |>
...
col1 <- "Diff"
if (!is.null(int_cancer_group) && comp_cancer_group == int_cancer_group) {
...
cn_data <- sample_data.list[["wgs"]][["cn_gene_tsv"]] |>
  dplyr::mutate(
...
sv <- sample_data.list[["wgs"]][["sv_tsv"]] |>
  RNAsum::sv_process()
tmp_sv_genes <- sv$genes_all$Genes
...
if (runFusionChunk) {
  ...
  ##### Flag fusions that were also reported in SV input
...
-   **`r glue("{ref_ext_cancer} {ext_cancer_group}")`** external reference samples from [The Cancer Genome Atlas][tcga] project ([related publication][tcga-pub])
`r if (params$batch_rm) { glue("-   **{ref_int_cancer} {int_cancer_group}** internal reference samples") }`
...
msg <- "None of the transcriptome-based fusion events have supporting evidence from DNA data or was previously reported."
if (nrow(fusion_annot_top) > 0) {
  ...
  fusion_annot_top.circos.pairs <- fusion_annot_top |>
    ...
    tidyr::drop_na() |>
    as.data.frame()
  if (nrow(fusion_annot_top.circos.pairs) > 0) {
    ...
    rm(fusion_annot_top.circos.pairs)
    cat(msg)
  }
} else {
  cat(msg)
...
Out of the `r if ( runSVsChunk ) { nrow(sv$genes_all) }` genes affected by `r if ( runSVsChunk ) { sv$total_variants }` SVs, the expression of **`r if ( runSVsChunk ) { sum(sv$genes_all$Genes %in% rownames(ref_dataset.list[[dataset]][["data_to_report"]])) } else { length(NULL) }`** was reliably measured in the patient's sample. The remaining `r if ( runSVsChunk ) { sum(!sv$genes_all$Genes %in% rownames(ref_dataset.list[[dataset]][["data_to_report"]])) }` genes are either not expressed or their expression level is too low to be detected (indicated in [BLANK]{style="color:#808080"} cells with missing values).
...
Section overlaying the mRNA expression data with per-gene somatic copy-number (CN) data, as well as SNVs/indels and SV data, if available.
...
##### Ensure Alterations column exists (e.g. when PCGR not used or no overlapping genes)
if (!"Alterations" %in% colnames(dat1)) dat1 <- dat1 |> dplyr::mutate(Alterations = "None")
if (!"Alterations" %in% colnames(data.sub)) data.sub <- data.sub |> dplyr::mutate(Alterations = "None")
...
##### Prepare dataframe for CN genomic view plot
...
if (nrow(data.annot) > 0) {
  plot_cnv_manhattan <- function(cnv_data, genes_to_highlight = NULL, title = "",
                                 show_legend = TRUE, point_size = 3, highlight_size = 8,
                                 cn_top = NULL, cn_bottom = NULL) {
    ...
    chr_centers <- plot_data |>
      dplyr::group_by(.data$SEQNAME) |>
      dplyr::summarize(center = (min(.data$pos_cum) + max(.data$pos_cum)) / 2, .groups = "drop") |>
      dplyr::arrange(.data$SEQNAME)
    ...
    chr_bands <- plot_data |>
      ...
      dplyr::arrange(.data$SEQNAME) |>
      dplyr::mutate(
        color = ifelse(seq_len(dplyr::n()) %% 2 == 0, "rgba(220, 220, 220, 0.3)", "rgba(180, 180, 180, 0.3)")
      )
    ...
    for (chr in sort(unique(data_normal$SEQNAME))) {
      ...
      p <- p |>
        plotly::add_trace(
          ...
          text = ~paste0(
            "<b>", Gene, "</b><br>",
            "(", format(GENESEQSTART, big.mark = ","), ", ", round(P, 2), ") chr", chr_label(SEQNAME), "<br>",
            "Gene: ", Gene, "<br>",
            "Diff_Z_score: ", round(Diff_Z_score, 2), "<br>",
            "Diff_Perc: ", round(Diff_Perc, 1), "<br>",
...
Scatterplot comparing the per-gene difference in **mRNA expression** of [Cancer genes] between patient's sample and cancer individuals (*y-axis*), and **CN values** (*x-axis*).
...
Table summarising the **mRNA expression** values in cancer and patient samples for genes with **CN** values \>= `r if ( runPurpleChunk ) { cn_top } else { c("(NA)") }` (**gains**), based on patient's genomic data, and mutation status if available (from [PCGR](https://github.com/sigven/pcgr)).
...
Table summarising the **mRNA expression** values in cancer and patient samples for genes with **CN** values =\< `r if ( runPurpleChunk ) { cn_bottom } else { c("(NA)") }` (**losses**), based on patient's genomic data, and mutation status if available (from [PCGR](https://github.com/sigven/pcgr)).
...
The individual box(es) represent the `r if ( !is.null(int_cancer_group) && int_cancer_group == comp_cancer_group ) { paste0(int_cancer_group, " and") } else { cat("") }` `r ext_cancer_group` `r if ( !is.null(add_cancer_group) ) { paste0("and ", add_cancer_group) } else { cat("") }` reference cancer cohort(s), and the **BLACK** dots indicate expression (percentile) values for each gene in the patient sample. Genes are ordered **alphabetically**.
...
```{r drugs_predictive_fusion_genes, comment = NA, message=FALSE, warning=FALSE, eval = params$drugs}
...
```{r drugs_predictive_hrd_genes, comment = NA, message=FALSE, warning=FALSE, eval = params$drugs}
##### Generate table with drugs targeting mutated cancer genes
