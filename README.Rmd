...
Recommended for Mac users and anyone who wants a self-contained environment:
...
Use the conda lock file for a pinned, reproducible install. Supported platforms: `linux-64`, `linux-aarch64`, `osx-64`, `osx-arm64`.
...
`inst/scripts/run.sh`. It pulls the image, mounts an
input and output directory, and runs the WGS + WTS example. All three of
`VERSION` (image tag, see [tags](https://github.com/umccr/RNAsum/tags)), `OUTDIR`
(where the report is written), and `INDIR` (input data directory) are
required. Point `INDIR` at the bundled test data under `inst/rawdata/test_data` or at your own inputs:
