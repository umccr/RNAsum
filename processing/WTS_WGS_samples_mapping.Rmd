...
for ( sample in SAMPLES ) {
  ...
  # Get the "subject_id"
  subject.id <- template[ template$sample_name == sample , ]$subject_id
  ...
  if ( nrow(WGS_subset) > 1 ) {
    ...
    while ( length(WGS_sample_match) == 0 ) {
      ...
      WGS_sample_match <- agrep(sample, WGS_subset$sample_name, ignore.case = TRUE, value = FALSE, max.distance = 0.9)
    }
    
    # Report all WGS samples is these match with the lowest possible maximum distance
    SAMPLES_match[[ sample ]] <- WGS_subset[ WGS_sample_match, ]
    
  } else {
    
    SAMPLES_match[[ sample ]] <- WGS_subset
  }
}
