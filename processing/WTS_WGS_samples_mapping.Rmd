...
First, for each queried WTS sample get the `subject_id`, list WGS (column `type`) samples with the corresponding `subject_id` and,  in case there are multiple WGS samples listed for given `subject_id`, use pattern matching to get the WGS sample that best matches with the queried WTS `sample_name`. To do this, for each *WGS `sample_name`* search for the the *overlap* with *WTS `sample_name`* and then compute the *similarity score* between the *`sample_name` overlap* and the full *WGS `sample_name`*. Multiply this score by the *square root of length of the overlap* to make sure that the longest overlap will get the highest score. WGS sample with the highest final *similarity score* is mapped with the corresponding WTS sample.
...
# Loop though each euqried sample, get the "subject_id", list WGS ("type") samples with the corresponding "subject_id" and in case there are multiple WGS samples use pattern matching to get the WGS sample that best matches with the queried WTS "sample_name"
for ( sample_WTS in SAMPLES ) {
  ...
  if ( nrow(WGS_subset) > 1 ) {
    ...
    stringsim_scores <- NULL
    
    # For each WGS "sample_name" (actually "targetname") search for the the overlap with WTS sample (after removing "_" to minimise chance that these will contribute to the matching score "stringsim_scores")
    for ( sample_WGS in WGS_subset$targetname ) {
      ...
      # Now compute the similarity scores between the WTS-WGS "targetname" overlap and the full WGS "targetname". Multiply this score by the square root of length of the overlap to make sure that the longest overlap will get the highest score
      stringsim_score <- stringsim(sample_WTGS.overlap[[1]], sample_WGS, method='osa', p=0.1 ) * nchar(sample_WTGS.overlap[[1]])^2
      stringsim_scores <- c( stringsim_scores, stringsim_score)
      
      if ( params$debug ) { cat("\tstringsim_score: ", stringsim_score) }
      
    }
    
    if ( params$debug ) { cat("\nWTS - WGS match: ", sample_WTS, " - ", WGS_subset[ which.max(stringsim_scores), ]$sample_name) }
    
    # Report WGS sample with best match and combine info with WTS sample info
    SAMPLES_match[[ sample_WTS ]] <- cbind(WTS_subset, WGS_subset[ which.max(stringsim_scores), ])
...
DT::datatable( data = SAMPLES_match.table[ , c("project.WTS", "subject_id.WTS", "sample_name.WTS",  "sample_name.WGS", "results.WTS", "results.WGS", "phenotype.WTS", "phenotype.WGS") ], filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE)
```

#### Full table

```{r wts_wgs_templates_table_full, comment = NA, message=FALSE, warning=FALSE}
DT::datatable( data = SAMPLES_match.table, filter="none", rownames = FALSE, extensions = c('Buttons','Scroller'), options = list(pageLength = 10, dom = 'Bfrtip', buttons = c('excel', 'csv', 'pdf','copy','colvis'), scrollX = TRUE, deferRender = TRUE, scrollY = 200, scroller = TRUE), width = 800, caption = htmltools::tags$caption( style = 'caption-side: top; text-align: left; color:grey; font-size:100% ;'), escape = FALSE)
```
